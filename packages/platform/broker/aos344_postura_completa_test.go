...
// stackAOS344 monta a pilha com os dois eixos declarados NO BROKER e no gate.
//
...
func stackAOS344(t *testing.T, comGate bool, classProviders, providerHosts map[string][]string) *stack {
	t.Helper()
...
// posturaSelada é o subconjunto de postura que os dois eventos do broker têm de selar.
type posturaSelada struct {
...
func posturaDoEvento(t *testing.T, s *stack, runID, tipo string) posturaSelada {
	t.Helper()
	var achados []posturaSelada
...
// TestAOS344_OsDoisEventosSelamAPosturaCOMPLETA: sob um regime em que os três valores
...
func TestAOS344_OsDoisEventosSelamAPosturaCOMPLETA(t *testing.T) {
	t.Parallel()
	s := stackAOS344(t, true,
...
// TestAOS344_APosturaPorOMISSAOTambemESelada prova que os campos novos não são
...
func TestAOS344_APosturaPorOMISSAOTambemESelada(t *testing.T) {
	t.Parallel()
	sd := stackAOS344(t, false, nil, nil)
