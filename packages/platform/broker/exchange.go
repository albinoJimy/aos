...
type exchangePayload struct {
	...
	// nesta troca. Faltava (AOS-343): o evento selava dois terços da postura do
...
type deniedPayload struct {
	...
	ProviderPolicy string `json:"provider_policy"`
	// ProviderPolicyShape e ResourceBinding completam a postura selada (AOS-343).
	//
	...
	// negação do gate sela e este evento não selava. Resultado: uma negação da guarda
	// de composição ficava no WORM com UM TERÇO da postura, e quem a lesse não sabia
	// se `provider_policy=enforced` significava imposto ou um curinga que não impõe
	// nada — que é exactamente o que o AOS-342 mediu e fechou noutro sítio.
	//
	// São METADADO DE CONFIGURAÇÃO, não a causa da decisão: viajam ao lado do campo
	// `axis`, que é quem diz o que decidiu, e são selados em QUALQUER eixo — uma
	// negação de capability também corre sob um regime, e omitir a postura aí era o
	// mesmo buraco com outro nome.
	//
	// É a MESMA disciplina do gate, e as duas convergiram por caminhos diferentes: lá
	// a postura ia num sufixo do `Reason` enquanto não havia canal estruturado numa
	// negação, e passou para o `metadata` do [referencemonitor.HookResult] quando o
	// AOS-340 o abriu. Aqui o evento é do BROKER e sempre houve onde declarar campos
	// próprios. Contentores diferentes, mesma regra: a postura acompanha o registo,
	// não a explicação.
	ProviderPolicyShape string `json:"provider_policy_shape"`
	ResourceBinding     string `json:"resource_binding"`
}
