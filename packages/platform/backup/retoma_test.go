...
//
// Este teste fixa o PRIMEIRO caminho: com entropia real a re-tentativa cifra com outra DEK, o
// conteúdo é outro e a ref também — o órfão fica ao lado. (Com detRand, como estava, a DEK repetia-se
// e o teste passava pelo segundo caminho sem o dizer.) O segundo caminho está fixado à parte, em
// TestAOS101_ColisaoNaRefDoSegmentoSoAceitaOMesmoConteudo. E a cadeia que resulta RESTAURA.
func TestAOS101_CicloInterrompidoEntreSegmentoERegistoNaoBloqueiaODestino(t *testing.T) {
	...
	signer := newSigner(t)
	vault := audit.NewInMemoryKeyVault(nil) // a custódia da KEK sobrevive aos arranques
	...
	// O segmento FICOU (órfão) e a cadeia NÃO avançou — nenhum registo de ciclo foi selado. (O
	// outro objecto é a sonda de escrita condicional da construção.)
	if dst.Len() != 2 {
		t.Fatalf("deviam ter ficado a sonda e o segmento orfao; got %d objectos", dst.Len())
	}
	...
	// Sonda + órfão + segmento novo + registo de ciclo: a re-tentativa escreveu NOUTRA ref.
	if dst.Len() != 4 {
		t.Fatalf("a re-tentativa devia ter escrito o segmento numa ref NOVA ao lado do orfao; got %d objectos", dst.Len())
	}

	// E o resultado é uma cadeia que restaura — o órfão não a contamina.
	if ev := restaurarCadeia(t, dst, vault, signer, 1); ev.EventsRestored != 2 {
		t.Fatalf("a cadeia depois de um ciclo interrompido devia restaurar os 2 eventos; got %d", ev.EventsRestored)
	}
}

// DOIS EXPORTADORES SOBRE O MESMO DESTINO COLIDEM NO REGISTO DE CICLO — E O SEGUNDO NÃO ADOPTA.
//
// A referência do registo de ciclo é indexada de propósito: é onde dois escritores se encontram. O
// registo que lá está é autêntico (mesma chave) e continua o MESMO head — num destino condicional,
// isso é verdade para TODOS os escritores reais —, e mesmo assim não é deste exportador: só a sua
// própria escrita ambígua se adopta (TestAOS101_EscritaAmbiguaDoRegistoEAdoptada). Qualquer outro é
// [ErrChainOwned]; um que não verifica é [ErrCycleRecordInvalid].
func TestAOS101_DoisExportadoresNoMesmoDestinoColidemNoRegistoDeCiclo(t *testing.T) {
	...
	expA, err := NewExporter(src, dst, signer, WithKeyVault(vault))
	if err != nil {
		t.Fatalf("NewExporter A: %v", err)
	}
	expB, err := NewExporter(src, dst, signer, WithKeyVault(vault))
	if err != nil {
	...
	if !errors.Is(err, ErrChainOwned) {
		t.Fatalf("B devia ser recusado com ErrChainOwned (outro escritor), e nao adoptar o elo de A; got %v", err)
	}
	if errors.Is(err, ErrSegmentTampered) || errors.Is(err, ErrChainBroken) || errors.Is(err, ErrCycleRecordInvalid) {
		t.Fatalf("uma bifurcacao NAO se deve confundir com adulteracao; got %v", err)
	}
	if ev := restaurarCadeia(t, dst, vault, signer, 1); ev.EventsRestored != 2 {
		t.Fatalf("a cadeia de A devia continuar intacta e restauravel (2 eventos); got %d", ev.EventsRestored)
	}
}

// DOIS NÓS COM LOGS DIFERENTES SOBRE O MESMO DESTINO: O SEGUNDO PÁRA, E NADA SE PERDE NEM SE MISTURA.
//
// A regressão que a revisão adversarial mediu: com a adopção por «continua o nosso head», B adoptava
// o ciclo 1 de A (A1,A2), o cursor de B saltava para 2 e o ciclo seguinte selava só B3..B5 — um
// restauro VERIFICADO com A1,A2,B3,B4,B5, e B1..B2 perdidos em silêncio.
func TestAOS101_DoisExportadoresComFONTESDiferentesNaoSeMisturam(t *testing.T) {
	ctx := context.Background()
	...
	seed(t, srcB, "run-a", 5, "B")
	if _, err := expA.Export(ctx); err != nil {
		t.Fatalf("A, ciclo 1: %v", err)
	}
	for i := 0; i < 2; i++ { // a re-tentativa também não adopta
		if _, err := expB.Export(ctx); !errors.Is(err, ErrChainOwned) {
			t.Fatalf("B (outro log) devia parar com ErrChainOwned na tentativa %d; got %v", i+1, err)
		}
	}
	ev := restaurarCadeia(t, dst, vault, signer, 1)
	if ev.EventsRestored != 2 {
		t.Fatalf("o backup devia ter so os 2 eventos de A, sem eventos de B cosidos; got %d", ev.EventsRestored)
	}
}

// DOIS EXPORTADORES COM KEKs DIFERENTES A COLIDIR: ErrChainOwned, NUNCA ADOPÇÃO.
//
// Adoptar o elo de um escritor com outra custódia deixava a cadeia com KEKs misturadas, e o
// arranque seguinte (que só abre o ÚLTIMO segmento) aceitava-a. Só se adopta a própria escrita, e
// essa foi selada com a nossa KEK.
func TestAOS101_ColisaoComOutraKEKNuncaEAdoptada(t *testing.T) {
	ctx := context.Background()
	src := newSourceStore(t, "board-eu", "eu-west")
	dst := NewInMemoryImmutableStore("eu-west")
	signer := newSigner(t)
	vA := audit.NewInMemoryKeyVault(nil)
	vB := audit.NewInMemoryKeyVault(nil)
	expA, err := NewExporter(src, dst, signer, WithKeyVault(vA))
	if err != nil {
		t.Fatalf("NewExporter A: %v", err)
	}
	expB, err := NewExporter(src, dst, signer, WithKeyVault(vB))
	if err != nil {
		t.Fatalf("NewExporter B: %v", err)
	}
	seed(t, src, "run-a", 2, "m")
	...
	if _, err := expB.Export(ctx); !errors.Is(err, ErrChainOwned) {
		t.Fatalf("B, com outra KEK, devia parar com ErrChainOwned; got %v", err)
	}
	// O arranque de B sobre a cadeia de A continua a ser recusado pela KEK.
	if _, err := NewExporter(src, dst, signer, WithKeyVault(vB)); !errors.Is(err, ErrResumeUnverifiable) {
		t.Fatalf("o arranque de B sobre a cadeia selada com a KEK de A devia ser recusado; got %v", err)
	}
}
...
func TestAOS101_UmCursorADULTERADONoDestinoNaoERetomado(t *testing.T) {
	...
	exp, err := NewExporter(src, origem, signer, WithKeyVault(vault))
	if err != nil {
		t.Fatalf("NewExporter: %v", err)
	}
	...
	_, err = NewExporter(src, destinoForjado, signer, WithKeyVault(vault))
