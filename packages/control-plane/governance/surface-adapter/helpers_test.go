...
// assina do seu lado. Implementa [messaging.Signer]. Seeds efémeras deterministas.
...
// approvalStep descreve o que a [seqSource] devolve: quem aprova e se aprova.
...
// transporte out-of-band de N aprovadores distintos POR CANAL.
...
// aprovadores e a sequência de aprovações. Devolve também o audit MemStore para asserir
// a selagem (prova de que o Channel — não o adaptador — sela). É o "gate" (AOS-095) a
// que o adaptador DEVOLVE a decisão.
...
// spyChannel regista o(s) [risk.ConfirmationRequest] que o adaptador DEVOLVE (via o
// coletor canónico) e responde com respostas canónicas. Prova que a identidade do
// aprovador chega ao gate e que o pedido leva o efeito concreto resolvido.
...
// --- Construtores de cards canónicos ----------------------------------------------

// reversibleCard constrói um card REVERSÍVEL (danger reversível, ex.: post com undo):
// DualControlRequired=false, uma aprovação basta em qualquer canal capaz.
func reversibleCard(t *testing.T) approvalcard.ApprovalCard {
	t.Helper()
	...
	card, err := approvalcard.BuildCard(req, approvalcard.WithRequestID("card-rev"))
	if err != nil {
		t.Fatalf("BuildCard reversivel: %v", err)
	}
	return card
}

// irreversibleCard constrói um card IRREVERSÍVEL (danger, ex.: delete): exige
// DUAL-CONTROL (dois aprovadores distintos).
func irreversibleCard(t *testing.T) approvalcard.ApprovalCard {
	t.Helper()
	req := risk.ConfirmationRequest{
		Class:        risk.ClassDanger,
		Irreversible: true,
	...
	card, err := approvalcard.BuildCard(req, approvalcard.WithRequestID("card-irr"))
	if err != nil {
		t.Fatalf("BuildCard irreversivel: %v", err)
	}
	return card
}

// allPlatforms é a lista das três superfícies suportadas.
func allPlatforms() []Platform { return []Platform{PlatformSlack, PlatformTelegram, PlatformDesktop} }

// renderOn renderiza um card na plataforma dada (falha o teste em erro).
func renderOn(t *testing.T, p Platform, card approvalcard.ApprovalCard) RenderedCard {
	t.Helper()
	r, err := RendererFor(p)
	if err != nil {
...
// hasAction indica se a render oferece uma acção do kind dado.
func hasAction(rc RenderedCard, kind ActionKind) bool {
