...
func TestGatewayModelClient_EndToEnd(t *testing.T) {
	...
	for _, model := range []string{"gpt-4o", "gpt-4o-mini"} {
		...
		mc, err := newGatewayModelClient(verifier, srv.URL, model, "", defaultModelGatewayRegion, defaultModelGatewayBoard, nil, nil, nil, nil, false, nil, 0)
		if err != nil {
...
func TestModelGateway_NoThroughputFuse(t *testing.T) {
	...
	verifier, credCtx := aos278ModelIdentity(t)
	mc, err := newGatewayModelClient(verifier, srv.URL, "gpt-4o-mini", "", defaultModelGatewayRegion, defaultModelGatewayBoard, nil, nil, nil, nil, false, nil, 0)
	if err != nil {
...
func TestExternalAllowlist_AllowsNonEmbeddedModel(t *testing.T) {
	...
	verifier, credCtx := aos278ModelIdentity(t)
	mc, err := newGatewayModelClient(verifier, srv.URL, "kimi-for-coding", "", "eu", "board-kimi", pol, nil, nil, nil, false, nil, 0)
	if err != nil {
		t.Fatalf("newGatewayModelClient: %v", err)
	}
