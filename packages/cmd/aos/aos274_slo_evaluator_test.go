...
func TestAOS274_MetricsExpoeSLOsEAlertas(t *testing.T) {
	...
	for _, want := range []string{
		...
		// O `avaliavel="1"` diz que a regra foi mesmo AVALIADA nesta passagem; o `produtor="1"`
		// diz que este binario compoe quem alimenta o SLI. Sao factos DIFERENTES (achado D): sem
		// o segundo, um `avaliavel="0"` de um no sem trafego lia-se como "nunca pode disparar".
		`aos_alert_firing{alert="mediation_overhead_high",catalog="mediation",severity="critical",sli="mediation_overhead_p95",runbook="RB-04",owner="DevOps/SRE",runbook_orphan="0",avaliavel="1",produtor="1"} 1`,
