...
### Estado
**MECANISMO FECHADO** (vaga 4 EPIC-19); pacote `packages/control-plane/governance/plan-approval/` estendido de forma aditiva (23 testes antigos verdes + novos; `go.mod` intacto; zero import do orchestrator — desacoplamento por portas preservado). A fronteira "até L3" é garantida pelo oráculo de autonomia (`mode.Runs()` só auto-aprova a níveis altos; L0–L3 exige gate humano + confirmação assinada). **DECISÃO DO DONO PENDENTE:** a *revisão item-a-item forçada* (≥gray) e o *dual-control por-efeito* (danger) são opt-in `default-OFF`, e `packages/cmd/aos-demo/main.go:215` liga o gate sem eles ⇒ por omissão o nó entregue aprova ≥gray sem revisão nó-a-nó. Ligar por omissão exige montar um `Reviewer` e um canal dual-control no nó (não é flip trivial). Opções levadas ao dono: (A) seguro-por-omissão na lib + wiring; (B) seguro só via wiring de produção; (C) aceitar opt-in. Até decisão, o mecanismo fica disponível mas não imposto por omissão.
