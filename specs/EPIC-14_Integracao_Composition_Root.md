...
- [x] `WithRatifyFreshness`+`WithRatifyNonceStore` ligados no promotion controller. — **FEITO por AOS-206** (achado **DEF-03**; antes `[ ]` POR CUMPRIR após correcção de `[x]` por **AOS-196**). O nó `aos` compõe agora um *promotion controller* real (`packages/cmd/aos/promotion.go`, `PromotionController`, composto no `Bootstrap` passo (5b) e exposto em `Node.Promotion`) que chama a via sancionada `hitl.NewProductionRatificationGate` — freshness + nonce-store durável `hitl.NewEventStoreNonceStore(es)` FORÇADOS. `grep NewProductionRatificationGate` fora de `_test.go` deixou de ser só a declaração; `grep -n "promotion\|Promote" packages/cmd/aos/*.go` não-teste deixou de ser 0. A guarda de fonte `TestNode_UsesSanctionedRatificationPathOnly` prova que o `NewRatificationGate(` cru NÃO é chamado. Ver `### AOS-206` em `specs/EPIC-18_Remediacao_Auditoria_Multiagente_v4.md` §8-bis e o fecho de `DEF-401`/`DEF-402` no registo.
- [x] Uma ratificação re-usada após consumo é `ReasonRatificationReplayed`; fora da janela é `ReasonRatificationStale`. — **FEITO** ao nível do gate (`hitl/ratification.go`), provado em `nonce_store_test.go`/`ratification_test.go`; e agora **observável pelo caminho do nó (`node.Promotion.Promote`), provado em teste** (AOS-206): `TestNodePromotionController_ReplayBlockedThroughNode` sela `ratification_replayed` no WORM do nó após re-submeter a mesma ratificação por `node.Promotion.Promote`. Não há (ainda) superfície de I/O do binário que invoque `Promote`: a submissão de ratificações pelo operador (endpoint/CLI) fica DEFERIDA (depende de AOS-096/pipeline de promoção).
...
- [x] Uso-único durável provado; sem segredos. — **FEITO** (testes de replay/freshness/durabilidade verdes; `secrets.sh` verde). **Wiring de produção FECHADO por AOS-206:** o segundo CA voltou a `[x]` com um chamador de produção real — `packages/cmd/aos/promotion.go` (`PromotionController`), composto no `Bootstrap` passo (5b) e exposto em `Node.Promotion`, pela via sancionada `hitl.NewProductionRatificationGate` (freshness+nonce-store durável FORÇADOS); `DEF-401`/`DEF-402` estão **FECHADO-RESIDUAL** no registo. **Fronteira honesta que ainda vale:** o eval-gate CONCRETO da política de promoção (AOS-114/115) e o loop de auto-modificação ponta-a-ponta continuam DEFERIDOS; e a submissão de ratificações pelo operador (endpoint/CLI) fica DEFERIDA — a prova de ápice é pelo caminho do nó (`node.Promotion.Promote`), provada em teste, não por superfície de I/O do binário.
