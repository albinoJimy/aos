...
- [~] Evidência de sistema: um run real (nó composto com gateway, ou produção) mostra `manifest.model.model_id` preenchido em todos os turnos. É o critério «`model_id` ≠ vazio» do passo 19 do roteiro E2E. *(**NÃO VERIFICADO**: exige um deploy desta versão e um run real do roteiro E2E em produção. Coberto em composição por teste (nó pela API com cliente injectado, e o adaptador real em unidade).)*
