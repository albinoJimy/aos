...
| A postura selada nos dois eventos do broker era parcial — limite declarado no AOS-339, confirmado ao fechar o AOS-342 | **AOS-344** — **implementado** |
...
1. **Só o caminho de sucesso sela.** As negações passam pelo `MediationRecord` do Reference Monitor,
   ...
   **ERAM DOIS CAMINHOS, E FECHARAM EM TICKETS DIFERENTES.** A discovery deste ticket encontrou um
   ...
   selado antes. Esse é o **AOS-339**, que lhe dá evento próprio (`credential.exchange.denied`) com
   `provider_policy` incluído. O caminho do **gate** — a negação que o `ScopeGate` toma na mediação
   e que termina no `tool.call.denied` do RM — é o que **este** ticket fecha, na `Reason`.
...
**O caminho do gate não é deste ticket, e está fechado no AOS-332.** Uma negação tomada pelo
`ScopeGate` na mediação passa pelo `MediationRecord`, que não tem campo para `provider_policy`; o
**AOS-332** fecha-a nomeando a postura na `Reason`. Este ticket fecha o caminho da **guarda de
composição**, que o AOS-332 não cobre porque nem sequer produz um `MediationRecord` próprio — a
cadeia já selou um `permit` antes de a guarda correr. Os dois são complementares e nenhum torna o
outro dispensável; ambos aterram nesta ronda.
...
## AOS-344 — A postura selada era parcial, nos dois eventos do broker
...
A razão da negação do **gate** já selava os três eixos desde o AOS-341/AOS-342. Era o caminho do
broker — o `dispatch` — que ficava com um terço.
...
quem diz o que decidiu — a mesma razão pela qual `provider_policy` já era selado em qualquer eixo
desde o AOS-339. É o contraste deliberado com a `Reason` do gate, onde a postura vai **dentro da
frase** que explica a recusa e por isso só acompanha o eixo que realmente decidiu (AOS-332).
