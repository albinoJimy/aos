...
| **EPIC-24 Remediação dos defeitos da auditoria adversarial ES/SBX** *(proposta)* | Epic | Os doze defeitos apurados em `analises/12` sobre o Substrato, mais os dois que a §6 dessa auditoria omitiu (§0.1): a janela de leitura do JetStream que torna um stream ilegível ao fim de ~22,7 h de uptime, o CRC do WAL que não cobre o cabeçalho de comprimento, dois `Open` que colidem em `seq`, um `Flush` falhado que se anuncia transitório, `Healthy()` verde sobre um substrato morto, o hash de seccomp selado por um driver que nunca o recebe, e o mitigante de `tecnica/17` §5.2-f que é falso contra os deployments entregues. **Cinco tickets confirmados por execução.** Não reabre o substrato de sandbox — ver §1 | [EPIC-24_Remediacao_Auditoria_ES_SBX.md](EPIC-24_Remediacao_Auditoria_ES_SBX.md) | ~900 |
