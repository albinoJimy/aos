...
| **EPIC-23 Remediação dos defeitos da auditoria adversarial MEM/REG/GW/BRK** *(proposta)* | Epic | Os oito defeitos apurados em `analises/11` sobre os serviços de plataforma: o digest de `mcp_server` reduzido a três valores, custo zero por `usage` ausente, alarme de crypto-shred mudo com a custódia de referência, canal do broker sem TLS obrigatório, eixo *Provider* sem imposição nem teste, declarações de estado caducadas, e a não-composição de MEM/REG por registar. **Ao contrário da EPIC-22, inclui os latentes** — nenhum ADR os mantém fora do build. Mais AOS-328..338, follow-ups da validação adversarial da própria remediação e da revisão do AOS-333 (§0.3); ver §0 | [EPIC-23_Remediacao_Auditoria_MEM_REG_GW_BRK.md](EPIC-23_Remediacao_Auditoria_MEM_REG_GW_BRK.md) | ~870 |
| **EPIC-24 Remediação dos defeitos da auditoria adversarial ES/SBX** *(proposta)* | Epic | Os doze defeitos apurados em `analises/12` sobre o Substrato, mais os dois que a §6 dessa auditoria omitiu (§0.1): a janela de leitura do JetStream que torna um stream ilegível ao fim de ~22,7 h de uptime, o CRC do WAL que não cobre o cabeçalho de comprimento, dois `Open` que colidem em `seq`, um `Flush` falhado que se anuncia transitório, `Healthy()` verde sobre um substrato morto, o hash de seccomp selado por um driver que nunca o recebe, e o mitigante de `tecnica/17` §5.2-f que é falso contra os deployments entregues. **Cinco tickets confirmados por execução.** Não reabre o substrato de sandbox — ver §1 | [EPIC-24_Remediacao_Auditoria_ES_SBX.md](EPIC-24_Remediacao_Auditoria_ES_SBX.md) | ~1.400 |

**Total: 26 ficheiros referenciados, ~14.500+ linhas. Backlog ratificado/aceite: EPIC-01..17 (AOS-001..189). Propostas por ratificar: EPIC-18 (AOS-190..225), EPIC-19 (AOS-230..244), EPIC-20 (AOS-245..278), EPIC-21 (AOS-288..304), EPIC-22 (AOS-305..311), EPIC-23 (AOS-320..338) e EPIC-24 (AOS-344..353).**
