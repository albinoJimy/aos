...
- [ ] `-race` verde; nenhum guard-test de fronteira alterado sem emenda registada ao ADR-018. — *Guardas de fronteira: **inalterados e verdes** (zero linhas tocadas — `git status` confirma-o), e **nenhuma emenda ao ADR-018 foi necessária**. `-race`: **POR VERIFICAR EM CI**, por decisão do dono (2026-08-30). Exige cgo e a máquina de desenvolvimento não tem toolchain C (`gcc` ausente), pelo que ficam pendentes **dois** gates pela MESMA causa: `ci-test` (`-race` por módulo) e `ci-apex` (que corre `go test -race` sobre `packages/integration`). Nenhum dos dois é afectado por esta entrega — verificado: `packages/integration` não tem, na sua lista de dependências, um único pacote de `control-plane/runlifecycle` ou `cmd/aos-orq`. **O que CI tem de confirmar:** a suite de `control-plane/runlifecycle` sob `-race` — em particular `TestPosse_DisputaConcorrente_UmSoVencedor` (8 goroutines a reclamar o mesmo run) e `TestKeep_*` (renovador em segundo plano + join), que são o código concorrente desta entrega. NOTA para quem lê o veredicto de CI: `ci-sca` pode sair vermelho por falha de DNS a `vuln.go.dev` sem relação com esta entrega — os módulos novos varrem limpos e a baseline é respeitada.*
