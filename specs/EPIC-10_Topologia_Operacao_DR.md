...
**Critérios de Aceitação**
- [x] O Event Store escreve de forma **append-only** e replica por **quorum**; a perda de uma réplica não perde dados nem interrompe escritas (dentro do quorum). — *Append-only imposto pelo SERVIDOR (`deny_delete`/`deny_purge`; delete e purge recusados com `10057`/`10110`), não por convenção do cliente. **Zero perda RECONCILIADO** (`TestAC4_EscritasConfirmadasSobrevivemAMorteDeUmNo`): matou-se o LÍDER do stream a meio de uma sequência de escritas; as 80 confirmadas estão todas presentes e íntegras no log relido por uma ligação nova, verificadas `seq` a `seq` com a carga, e o log não tem eventos que ninguém confirmou. **0 escritas falharam durante o failover.** A primeira versão desta afirmação assentava numa contagem final e foi RETRACTADA — ver adenda 1 §A3 e adenda 5 do relatório.*
- [x] O transporte é **push** (event-driven), não *polling*; os workers recebem eventos sem *busy-wait*. — *`TestContrato_SubscribeEntregaPorPush`, a partir do código do AOS: um evento escrito DEPOIS da subscrição chega ao handler sem polling, e um segundo evento que o filtro exclui NÃO chega (controlo negativo — sem ele o teste passaria com um handler que recebe tudo). O consumidor é EFÉMERO com `deliver_policy: new`, que é a semântica do modelo de referência. **Limite declarado:** sem acks, sem flow control, sem heartbeats — um consumidor DURÁVEL de produção quer os três, e é a configuração em que o push foi medido.*
- [x] Múltiplos workers escrevem eventos e leem para replay **em paralelo**, sem contenção de single-writer. — *Escrita: `TestIntegracao_CASSobContencao` (8 ligações independentes, 1 vencedor e 7 recusados com `seq=0`) e o benchmark, que mede **~3,9× de 1 para 8 streams** (72 → 279 ops/s) — o paralelismo é medido, não afirmado. **Contraste que dá sentido ao AC:** o caminho durável de REFERÊNCIA não escala (54 → 49, dentro do ruído), porque o WAL é um ficheiro com `fsync` sob mutex global. **Ressalva honesta:** a leitura paralela não é serializada por construção (`Read` não toma o mutex por-stream) mas NÃO foi medida em separado.*
- [x] Não existe nenhum ponto único de escrita (SPOF eliminado); teste de falha de nó demonstra continuidade. — *Provado com a morte do LÍDER e reconciliação — ver o AC anterior. Provado também ao nível do NÓ: duas réplicas do `aos` sobre o mesmo Event Store arrancam ambas e disputam o mesmo `expected_seq` com uma só a ganhar (`TestAOS100_DuasReplicasDoNoSobreOMesmoEventStore`).*
- [x] As réplicas **nunca** cruzam a fronteira regional de soberania (ADR-011). — *Entregue por `eventstore/jetstream` (`ComRegiao`/`ComBoardDeSoberania`): o stream é criado com `placement` restrita a servidores que anunciem `server_tags: ["region:<regiao>"]`, e a colocação é **VERIFICADA contra a configuração ARMAZENADA** no arranque. Três modos de falha MEDIDOS contra um cluster real: região que nenhum par serve ⇒ o servidor recusa (`err_code=10005`); ligar-se a um stream **sem** colocação ⇒ aborta (o caso silencioso); fronteira sem região ⇒ aborta antes de ligar. Ligado ao nó (`AOS_EVENTSTORE_NATS_REGION`, OBRIGATÓRIA com `AOS_BOARD_REGIONS`) e ao `aos-orq` (`--nats-region`). **LIMITE DA MEDIÇÃO:** os três servidores do cluster anunciam a mesma região — prova-se que a restrição é pedida, armazenada, verificada e que a sua ausência aborta; a colocação por regiões distintas é lógica do JetStream, não nossa.*
- [x] A ordenação append-only e a integridade do log são preserváveis e verificáveis (base do audit hash-chain, ADR-010). — *Append-only imposto pelo SERVIDOR: apagar uma mensagem e purgar o stream são RECUSADOS (`10057`/`10110`, medido). Ordenação: o `seq` do AOS é gapless desde 1 POR STREAM e sobrevive à numeração global do stream físico — fixado em `TestContrato_SeqDoAOSEGaplessDesdeUm` com dois streams do AOS intercalados no mesmo stream físico, que é o caso onde uma implementação ingénua abriria buracos. Integridade sob falha: a reconciliação do AC1.*
...
*Estado dos Testes Requeridos (2026-09-01):* escrita concorrente multi-worker **FEITO** (`TestIntegracao_CASSobContencao`, 8 escritores: 1 vencedor, 7 recusados); ordenação/integridade append-only **FEITO** (delete/purge recusados pelo servidor, `10057`/`10110`; seq gapless por stream fixado em `TestContrato_SeqDoAOSEGaplessDesdeUm`); conformidade de soberania **FEITO** (`soberania_test.go`, três modos de falha); ***benchmark* de throughput FEITO** (`throughput_test.go`, co-localizado — ver adenda 4 do relatório: o replicado escala ~3,9× de 1 para 8 streams, o caminho durável de referência não escala, e o **replay do replicado é ~113 eventos/s**, que é a fraqueza a corrigir a seguir); **falha de nó com ZERO PERDA — FEITO** (`TestAC4_EscritasConfirmadasSobrevivemAMorteDeUmNo`, 2026-09-01): matou-se o líder do stream a meio das escritas; 80/80 confirmadas presentes e íntegras no log relido por ligação nova, reconciliadas `seq` a `seq` com a carga, sem eventos fantasma, e 0 escritas falharam durante o failover. A afirmação anterior — que assentava numa contagem final — tinha sido retractada, e esta substitui-a.

**Definition of Done**
- [x] Critérios de Aceitação satisfeitos e demonstráveis. — *Os 6 acima, cada um com a evidência e a ressalva ao lado. «Demonstráveis» é literal: as medições correm contra um cluster JetStream real (4 nós), não contra um duplo — ver `docs/reports/medicao-jetstream-arbitragem-2026-08-31.md` e as suas 8 adendas, que incluem as RETRACTAÇÕES de quatro afirmações minhas que não resistiram ao escrutínio.*
- [x] Replicação por quorum e transporte push testados sob falha de nó. — *Quorum: `TestAC4_EscritasConfirmadasSobrevivemAMorteDeUmNo` mata o líder do stream a meio da carga e reconcilia `seq` a `seq` (80/80, sem eventos fantasma). Push: o consumidor durável (`ack_policy: explicit`, `idle_heartbeat`, `flow_control`) retoma depois de morrer o nó a que a subscrição estava ligada — com semântica **at-least-once**, que está declarada no `doc.go` e não é um detalhe de implementação: um consumidor que perca o handle pode receber repetido.*
- [x] Integridade append-only verificada (base do audit WORM, ADR-010). — *Imposta pelo SERVIDOR (`deny_delete`/`deny_purge`): apagar e purgar são recusados com `10057`/`10110`, medido — não é uma convenção do cliente que um cliente diferente pudesse contornar. A ordem por stream é gapless desde 1 mesmo com dois streams do AOS intercalados no mesmo stream físico. **O que este ticket NÃO fecha:** a ponte para a hash-chain do WORM é do AOS-101; aqui garante-se a BASE (imutabilidade + ordem), não o selo.*
- [x] Sem segredos; testes verdes; cobertura não regride — *com uma ressalva que o número obriga a dizer.* Segredos: gate limpo. Testes: 25/25 verdes. Cobertura: **nenhum piso do CI desce** (apex >= 80%, kernel >= 80%, `COVERAGE_GATED_MODULES` — `substrate/eventstore` não está entre os módulos com piso). Mas o número honesto do pacote novo é **`jetstream` a 20,7%** (era 5,6% antes dos testes de lógica pura acrescentados a 2026-09-01), contra 91,1% do `eventstore` e 53,8% do `natsjs`. A razão é deliberada e tem de ficar escrita: **a correcção deste adaptador prova-se contra um substrato real, e um mock mediria o mock** — o CAS atómico entre processos, a recusa de delete/purge e a colocação por soberania não existem num duplo. O que se cobriu sem cluster foi o que não precisa dele: as **decisões de segurança** (validação de subject, derivação de prefixo, os três modos de falha da verificação de colocação, a classificação de recusas, a janela de `LigarRastreador`). O resto está exercitado pelos testes de integração, que correm com `AOS_NATS_URL` definido e são SALTADOS sem ele — pelo que a percentagem do CI subestima, por construção, o que está realmente testado.
- [~] Revisão por dois revisores (P0); ADR-007 referenciado; documentação e `tecnica/10` §3/§6 actualizadas. — *ADR-007 referenciado; `tecnica/10` §3 (o backend replicado deixa de ser promessa e passa a alcançável), §3-bis (as duas topologias e o que muda entre elas) e §6 (a soberania imposta por mecanismos diferentes nos dois substratos) actualizadas. **A revisão está por completar, e digo-o em vez de a marcar:** há UMA aprovação — a do dono, registada a 2026-09-01. A validação adversarial com subagentes independentes encontrou defeitos reais (três panics que matavam o nó, dois vectores de injecção no cliente, todos corrigidos), mas **não é um segundo revisor humano**, e contá-la como tal seria inflacionar a evidência. Quem fizer o merge herda este buraco e deve sabê-lo.*
...
**Estado.** **ABERTO. ⚠️ LIGAR O EXPORTADOR EM PRODUÇÃO ESTÁ BLOQUEADO pelo AOS-453** — não há custódia de KEK que sele segmentos: a de produção (Vault Transit, *key-never-leaves*) faz falhar todos os ciclos, e a de referência (em memória) morre com o processo, pelo que o 2.º arranque recusa a retoma a nomear a KEK. Fora isso, o módulo e o nó estão completos para um destino durável e faltam passos de infra/dono. AC3, AC4 e AC5 `[x]`; AC1 `[~]` (retoma do manifesto FECHADA a 2026-09-26; falta uma implementação durável do `ImmutableStore` e compor destino + chave em produção); AC2 `[~]` (alvo em `seq`, não em instante); AC6 `[~]` (ensaio de restauro sem agendamento nem evidência persistida). O que falta para ligar em produção, e é do dono: (1) escolher e implementar o backend durável da porta cumprindo o contrato do README do módulo (escrita condicional, `ErrNotFound` fiel, object-lock/retenção na região do board); (2) compô-lo em `Config.BackupDestination` — hoje sem superfície de ambiente; (3) provisionar a chave ed25519 de assinatura do backup (`Config.BackupSigningKey`, trust domain próprio, fora do repositório, a MESMA entre arranques — uma chave nova recusa a retoma); (4) o **AOS-453** — uma custódia de KEK que sele segmentos (DEK embrulhada pelo Vault Transit, no molde do resto do DSAR); (5) decidir a retenção do destino sabendo que um prefixo expirado RECUSA a retoma (uma cadeia incremental sem génese não se restaura), pelo que uma retenção finita exige snapshots completos periódicos — fora do âmbito deste ticket.
...
**Critérios de Aceitação**
- [ ] `sealSegment`/`openSegment` usam `audit.KeyWrapper` (`WrapDEK`/`UnwrapDEK`) quando o vault o implementa, e só caem na KEK crua quando não; o formato do segmento distingue os dois sem ambiguidade.
- [ ] Com a custódia Vault do nó (ou o `InMemoryKeyWrapper` de referência a modelá-la) o exportador sela, a retoma de um 2.º processo abre o último segmento, e a cadeia de dois arranques RESTAURA.
- [ ] Uma custódia que não entrega a KEK nem implementa o envelope é recusada na COMPOSIÇÃO do exportador, com erro nomeado, e não ciclo a ciclo.
- [ ] O crypto-shred do titular `backup:<região>` torna os segmentos irrecuperáveis e é verificado, no molde do resto do DSAR.
- [ ] Os banners de composição e do agendador dizem que custódia sela a KEK do backup.
...
**Estado.** **ABERTO** — criado a 2026-09-26 a partir da revisão adversarial do AOS-101; sem implementação.
...
**Critérios de Aceitação**
- [x] Um run tem, em qualquer instante, **um só detentor de lease**, e todas as escritas de ciclo de vida apresentam o **fencing token** corrente; uma escrita com token obsoleto é recusada, não aplicada. — *Entregue e provado **in-process** (`TestPosse_DisputaConcorrente_UmSoVencedor`, `TestEscrita_TokenObsoleto_Recusada`, `TestGrafo_EscritaDeDonoSuperado_Recusada`). **A disputa ENTRE PROCESSOS não é satisfazível com o Event Store de referência** — as réplicas de AOS-100 são cópias in-process e cada `Open` fica com a sua cabeça; medido deterministicamente. Ver **DEF-282** e ADR-023 §4.* **SATISFEITO ENTRE PROCESSOS por AOS-100 (2026-08-31):** com `aos-orq serve --nats` sobre o Event Store replicado, quatro processos REAIS em paralelo sobre o mesmo run dão `vencedores=1 negados-pelo-lease=3 guardados(WAL)=0` — todos admitidos ao store, e o vencedor decidido pelo LEASE (`TestAOS100_NServeEmParaleloSobreOSubstratoReplicado`). Sobre `--wal` o limite mantém-se e o guard de ficheiro continua a impedir a configuração insegura.
...
- [x] Cada partição tem um escritor exclusivo, arbitrado por lease durável (ADR-023) e não por mutex em processo. — *Porta `PosseDeParticao` (um método) injectada no `FileStore` por `ComPosseDeParticao`. **Porta e não dependência:** o `platform/audit` não conhece o `kernel/agent-runtime` onde vive o `LeaseManager`, e importá-lo inverteria a camada — mesmo padrão do `eventstore.Rastreador`. Consultada ANTES de qualquer efeito e FORA do `s.mu`: a porta pode ir à rede, e serializar todas as escritas atrás de uma chamada remota trocaria um defeito de correcção por um de desempenho. **O que NÃO resolve, declarado em `posse.go`:** entre o `Detem` dizer «sim» e a escrita ficar durável há a janela inerente a qualquer lease; fechá-la exige um token de fencing aceite pelo destino, e um ficheiro local não tem onde o validar. Isto dá exclusão entre réplicas BEM COMPORTADAS — a rede de segurança do outro caso é o critério seguinte. **Sem a opção armada nada muda**, fixado em teste próprio para que um default «esperto» no futuro não passe despercebido.*
- [x] A verificação da cadeia (`checkpoint.go`) distingue **adulteração** de **elo em falta por corrida** — um diagnóstico que confunde as duas leva a chamar o segurança quando o problema é de deployment. — *O discriminador é estrutural: se o registo repetido ENCADEIA no elo anterior e o seu `EntryHash` RECOMPUTA, é um segundo ramo bem formado. Tipo próprio `TamperFork` e sentinela `ErrChainForked`, seguindo o precedente do `TamperUnknownSchema` no mesmo ficheiro. **A medição corrigiu o enunciado deste critério:** o fork não aparece na verificação — aparece no REPLAY, e o WORM fica INABRÍVEL, pelo que a frase enganadora é a que o operador lê quando o nó SE RECUSA A ARRANCAR. Por isso o invólucro externo também mudou. **Nada enfraqueceu:** o erro de fork continua a desembrulhar para `ErrTampered`. **O que a distinção NÃO prova**, escrito no código: o `EntryHash` é um hash e não um MAC, logo um adulterador com acesso ao ficheiro consegue fabricar um ramo bem formado — distinguem-se FORMAS, não intenções, e a prova contra um adversário continua a ser o checkpoint assinado.*
- [x] Uma tentativa de escrita numa partição de que a réplica não é dona é **recusada**, não aplicada — e o facto de recusa é observável. — *Recusada antes de haver efeito: nada selado, nada persistido, `audit_seq` não consumido. Observável POR PARTIÇÃO (`RecusasDePosse`): «400 recusas» diz que algo está mal, «400 em `gov.read/run-7`» diz onde, e é a diferença entre um alarme e um diagnóstico; o instantâneo é uma cópia, com teste que o prova. **Fail-closed sobre a INCERTEZA:** se a porta falhar, não se escreve — não conseguir estabelecer a posse não autoriza a escrever. Mas a CAUSA sobrevive no erro, porque «o lease store caiu» e «perdi a posse» têm remédios diferentes.*
- [~] O `GenesisHash` e a atribuição de partições são determinísticos e reconstruíveis: quem verifica a cadeia consegue saber que partições existiam. — *Metade feita, e a metade que falta é a que tem peso. O `GenesisHash` é determinístico (deriva do nome da partição) e `FileStore.Partitions()` enumera as que existem — logo quem verifica CONSEGUE saber. **O que não existe é a ATRIBUIÇÃO:** nada mapeia deterministicamente réplicas→partições. Hoje isso é responsabilidade de quem implementa a porta de posse, e por isso é uma convenção sem guarda. Não se inventou um esquema de atribuição a pretexto deste ticket.*
