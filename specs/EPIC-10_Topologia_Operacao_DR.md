...
- [x] O `GenesisHash` e a atribuição de partições são determinísticos e reconstruíveis: quem verifica a cadeia consegue saber que partições existiam. — *Fechado. O `GenesisHash` já era determinístico (deriva do nome da partição) e `FileStore.Partitions()` enumera as que existem. **A metade que faltava — a ATRIBUIÇÃO — passou a existir** em `packages/platform/audit/atribuicao.go`: `AtribuirParticao(particao, replicas)` é uma função PURA (rendezvous hashing/HRW) que mapeia deterministicamente cada partição a uma réplica, independente da ordem da lista; `MapaDeAtribuicao` reconstrói a posse de todas as partições sem consultar lease store nenhum — é o que torna o critério verificável de fora. A posse deixa de ser convenção do implementador: `AtribuicaoDeterministica` é uma `PosseDeParticao` cuja resposta DERIVA da função, pelo que escritor e verificador chegam à mesma atribuição. Escolheu-se HRW sobre módulo porque só remapeia as partições da réplica que sai (provado em `TestAOS284_AC5_RemapeamentoEMinimoAoRemoverReplica`). **Fronteira mantida:** a atribuição diz quem DEVE deter; a exclusão em tempo real contra partição de rede continua a ser o lease durável (ADR-023) por trás da porta. Provas: `packages/platform/audit/aos284_atribuicao_test.go` (determinismo, independência de ordem, repartição não-degenerada, remapeamento mínimo, reconstrução, fork impedido entre réplicas, fail-closed sem réplicas), `-race` verde.*
...
**Definition of Done**
- [x] Critérios de Aceitação satisfeitos, com o AC1 (existe ou não o problema) respondido por escrito ANTES do código.
- [x] `-race` verde. — *`go test -race -count=1 ./...` no `packages/platform/audit` (suite completa, inclui os AC1/AC3 anteriores e os AC5 novos); gofmt e vet limpos.*
- [x] ~~Se o problema não existir, o ticket fecha com a prova disso~~ — **não se aplica**: o AC1 estabeleceu por medição que o problema EXISTE (ver o bloco no topo do ticket). Este ramo da DoD fica fechado por inaplicável, não por omissão.
- [x] `tecnica/09` e/ou `tecnica/10` actualizados com o modelo de partição. — *`tecnica/10` §3-quater — «Posse das PARTIÇÕES da hash-chain de auditoria»: a porta de posse, a atribuição determinística por rendezvous hashing e a fronteira atribuição↔lease.*
