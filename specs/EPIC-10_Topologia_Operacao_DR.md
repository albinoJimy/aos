...
**Critérios de Aceitação**
- [~] O log é exportado para **backup imutável** de forma contínua, adicional à replicação por quorum. — *A primitiva existe e é INCREMENTAL: cada `Export` lê só o que passou do head anterior, cifra num segmento AES-256-GCM e encadeia-o no manifesto. **O que não existe é o «contínua»:** `Export(ctx)` é UM ciclo, não há laço nem agendador, e — medido — nenhum binário importa `platform/backup`. O que corre de facto no servidor é o `deploy/server/backup.sh` (cron diário), que é outra coisa: cópia do VOLUME, não exportação de segmentos, com RPO de 24h. Enquanto ninguém agendar o exportador, esta linha descreve uma capacidade, não um comportamento.*
...
- [x] A janela de retenção e a periodicidade satisfazem o **RPO ≤ 1 min** dentro de região (cruza com AOS-102). — *`TestRPO_WithinOneMinute`, com relógio injectado e periodicidade de 30s, mede a janela efectiva ao longo de 6 ciclos e mantém-na <= 1 min. É uma propriedade DA PERIODICIDADE CONFIGURADA, e é aí que liga ao critério anterior: como nada agenda o exportador, o RPO efectivo em produção hoje é o do cron do `backup.sh` — **24 horas**, não 1 minuto.*
