...
> **Precisão de contagem (validação de 2026-08-13, actualizada em 2026-08-14):** o âmbito do epic são os **34** tickets `AOS-245..AOS-278`, todos implementados. A tabela §4 tem **36** linhas porque a remediação deste próprio epic **criou dois** tickets, ambos para dar eixo executável a residuais que ela própria nomeou — e **ambos já entregues em 2026-08-13**:
>
> - **AOS-279** (fecha **DEF-276**): o golden-set do planeador passou a correr no gate de CI `evalgate.sh`, fechando a metade do AC2 de AOS-273 que estava declarada «NÃO ENTREGUE».
> - **AOS-280** (fecha **DEF-271**): o estágio de roteamento passou a estar composto no pipeline do GW (`failover` → `routingstage`+`router` com o scoring armado), pelo que a regra 3 do ADR-021 deixou de estar inerte.
>
> Os dois têm secção própria no corpo, acrescentada depois de nascerem na tabela. **34 + 2 = 36 linhas para 34 tickets de âmbito** — a diferença é a remediação, não trabalho por fechar.
...
Selagem periódica out-of-process dos checkpoints do WORM (**DEF-268/DEF-269**, custódia de chave D4/AOS-156) · instância do IdP tenant + adaptador HSM/KMS (costura `crypto.Signer`, EPIC-16) · injecção do broker no executor remoto (**D8-B**) · ~~composição do estágio de roteamento no pipeline do GW (**DEF-271**)~~ **FECHADO em 2026-08-13 por AOS-280**, ficando no seu lugar os residuais **DEF-280-PORTAS** (orçamento e *task-fit* continuam seams — as fontes vivem noutros módulos), **DEF-280-TOKENS** (o `Exchange` não transporta o prompt: a reserva de admissão coordena por pedidos, não por tokens), **DEF-280-NO** (o nó de referência ainda não declara escada, logo o binário do nó continua a rotear só pelo failover) e **DEF-280-REGIAO** (a selagem no WORM cobre a troca de MODELO, não uma mudança só de região) — e ~~**DEF-280-ADR021**~~ **FECHADO em 2026-08-14** (emenda 1.2 do ADR-021, ratificada pelo dono: §5-ter acrescentado, §5-bis marcado SUPERADO) · emissão do veredicto e transporte do payload sem chamador de produção (**DEF-272/273**, eixo AOS-238) · gate de CI `evalgate.sh` a correr os golden-sets do planeador (**DEF-276**, eixo **AOS-279**) · compensação real da saga (**DEF-270**: exige um *produtor* de compensações — o loop nunca popula `activity.Activity.Compensation` — **e** o registo run-scoped no kernel; enquanto faltarem, habilitá-la compensaria o run errado).
...
- **~~⚠️ AOS-251 e AOS-252 têm DUAS implementações independentes.~~ RESOLVIDO — nota histórica (fechada em 2026-08-14).** Houve, à data da W0, uma segunda implementação por committar em worktrees locais (`.claude/worktrees/nice-cartwright-529d11`, branch `claude/compassionate-curie-e2866b`, AOS-251; `.claude/worktrees/admiring-wright-dec4c1`, branch `claude/vibrant-wilson-ce1127`, AOS-252) — nomes de ficheiro diferentes, mesmos ficheiros de produção tocados (`bootstrap.go`, `service.go`, `steer_gates.go`) —, e o aviso mandava **escolher UMA e descartar a outra**, sob pena de compor dois claims de arranque e dois selos terminais sobre a mesma aresta de estado. **A escolha foi feita e é a da W0** (a única com prova de coexistência dos dois mecanismos), que está no trunk. **Nenhuma das duas branches existe no `origin`** — os worktrees eram locais e o trabalho não committado desapareceu com eles. A instrução deixou de ser accionável e fica aqui só como registo de *porque* a aresta de estado tem a forma que tem; quem a ler não tem nada a decidir.
...
### Estado
**FECHADO (com residuais declarados)** — cadeia composta, classificador de produção entregue e provada pela cadeia real; remediação adversarial aplicada (cobertura de preço e config parcial como recusa de arranque, troca de modelo selada no WORM, atribuibilidade do erro de perfil, guard AST alargado a quem decide). Ficam com eixo **quatro** residuais: as duas portas de factor cuja fonte vive noutro módulo (DEF-280-PORTAS), a estimativa de tokens (DEF-280-TOKENS), a declaração da escada no nó de referência (DEF-280-NO) e a selagem de uma mudança só de região (DEF-280-REGIAO). O quinto — **DEF-280-ADR021** — **FECHOU em 2026-08-14** com a ratificação da emenda 1.2 pelo dono (ver o AC acima).
