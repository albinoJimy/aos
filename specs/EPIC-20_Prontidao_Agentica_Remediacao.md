...
| AOS-279 | Golden-set do planeador no gate de CI: ligar `plannerprompt.Evaluate` ao harness de `packages/platform/eval` (ou estender `evalgate.sh` a este conjunto), com o pass-rate a entrar no `AOS_EVAL_REPORT` | test | S-M | P3 | AOS-241, AOS-273 | DEF-276 |
...
- [ ] Os mesmos casos correrem no **gate de CI `evalgate.sh`** — **NÃO ENTREGUE**: esse gate corre outro harness (`packages/platform/eval`, ADR-012/AOS-114) e não conhece o golden-set do planeador. Os casos correm hoje como testes do pacote (`go test` do módulo, que a CI executa), o que é cobertura real mas **não** é o gate que o nome sugere. Residual com eixo em `docs/governance/REGISTO-Deferimentos.md` (**DEF-276**); a fronteira «biblioteca pura, handoff para um CI de staging» já estava declarada por AOS-241 (`plannerprompt/doc.go`) e não é re-clamada aqui.
