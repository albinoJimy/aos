...
| `AOS_WORM_EXPECTED_HEAD` | *(vazio ⇒ **verificação ancorada DESLIGADA**)* | **Piso de frescura** (audit_seq decimal **> 0**) persistido **INDEPENDENTEMENTE** do store: um checkpoint com `AuditSeq` **abaixo** deste piso é um **rollback** e ⇒ `audit.ErrCheckpointStale` (o boot **ABORTA**). ⚠️ É o que fecha o rollback-de-checkpoint — por isso **não** se deriva do próprio checkpoint (seria um no-op que reabriria o vector). Malformada ⇒ **ABORTA** (`ErrBadWormExpectedHead`). Uma das **três** da tripla — **algumas-mas-não-todas** ⇒ **ABORTA** (`ErrWormAnchorIncomplete`), nunca degrada em silêncio para a re-verificação de AOS-221. Material **público**. |
