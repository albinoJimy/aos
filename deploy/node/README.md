...
| Variável | Default | Efeito |
|---|---|---|
| `AOS_EVENTSTORE_NATS` | *(vazio)* | Endereço (`host:porta`) de um cluster **NATS JetStream** que serve de Event Store **replicado por quórum** (AOS-100, ADR-007). **PRECEDE** `AOS_EVENTSTORE_PATH`. **Impacto de conformidade — o mais importante desta tabela:** este substrato **ARBITRA ENTRE PROCESSOS** (o `expected_seq` é imposto pelo servidor, medido), pelo que correr **N réplicas do nó** sobre ele passa a ser a configuração PRETENDIDA e o guard de arranque de AOS-285 deixa de se aplicar ao Event Store. O guard do **WORM mantém-se** — o WORM continua a ser um ficheiro local e dois escritores forkam a hash-chain (AOS-284). **LIMITE POR FECHAR:** este backend ainda **não exprime a fronteira regional de soberania** (falta `placement`); **não usar num board com fronteira declarada** (ADR-011). |
