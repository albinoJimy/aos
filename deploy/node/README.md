...
| `AOS_BREAKER_MAX_COST_MICRO_USD_PER_SEC` | `0` *(**desligado**)* | Tecto de **velocidade de queima** de custo. ⚠️ Desligado por omissão **por escolha, não por esquecimento**: um tecto errado mata runs saudáveis, e o valor certo depende do modelo, do preço e do perfil da carga — que o nó não conhece. Ligue-o quando tiver dados. |
| `AOS_BREAKER_MAX_TOKENS_PER_SEC` | `0` *(**desligado**)* | Tecto de velocidade de queima de tokens. Mesma justificação do anterior. |
