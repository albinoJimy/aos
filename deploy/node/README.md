...
|---|---|---|
| `AOS_MODE` | *(vazio ⇒ modo de **referência**)* | `production` (qualquer caixa) activa a **postura de produção fail-closed**. **Segurança:** é o interruptor que torna obrigatórias **cinco** exigências — `AOS_ISSUER_PUBKEY` (senão `ErrProductionNeedsHardenedIdentity`), `AOS_BOARD_REGIONS` **não-vazio** (senão `ErrProductionNeedsSovereignRead`), **credencial forte da soberania de leitura** — `AOS_SOVEREIGN_OIDC_ISSUER`+`AOS_SOVEREIGN_OIDC_AUDIENCE` (senão `ErrProductionNeedsSovereignAuthority`, AOS-205) **terminação TLS** — `AOS_TLS_CERT_PATH`+`AOS_TLS_KEY_PATH` **ou** `AOS_TLS_EXTERNAL_TERMINATION` (senão `ErrProductionNeedsTLS`, AOS-209) e **Event Store durável** — `AOS_EVENTSTORE_PATH` **ou** `AOS_EVENTSTORE_NATS` (senão `ErrProductionNeedsDurableSubstrate`, AOS-300: sobre o store in-memory o stream `identity.nhi.revoked` morre com o processo e um NHI **revogado volta a ser aceite** ao primeiro restart, em silêncio, enquanto o banner anuncia «revogacao»). Qualquer outro valor ⇒ modo de referência **sem** essas exigências: um nó exposto sem `AOS_MODE=production` não é um nó de produção, é um nó de referência a servir tráfego. |
...
| `AOS_MANDATED_ISSUER_ID` | *(vazio ⇒ **sem emissor automático**; a cunhagem do NHI é manual)* | Identificador do **emissor AUTOMÁTICO** (AOS-427, [ADR-033](../../docs/adr/ADR-033-emissor-automatico-limitado-por-mandato.md)), ex.: `iss:aos-issuer-auto`. É um **segundo trust anchor**, e **não** é confiado por inteiro: os seus tokens **só verificam com um MANDATO embebido**, assinado por um humano de `AOS_MANDATE_SIGNERS`, que cubra o token — humano, agente, classe, `policy_ref`, board, escopo, TTL e janela. Um token fora do mandato ⇒ `E_MANDATE_VIOLATED`; sem mandato ⇒ `E_MANDATE_REQUIRED`; mandato de chave não pinada ⇒ `E_MANDATE_INVALID`. **Segurança:** é o que permite o emissor viver num servidor cujo Vault se destrava sozinho — quem comprometer o **emissor** (o contentor, o token do Vault, a chave transit) só cunha o que o humano assinou. ⚠️ **Não cobre root no host do nó**: esse muda esta configuração e reinicia o nó. **As três variáveis vêm juntas ou nenhuma**: uma sem as outras ⇒ **ABORTA** (`ErrBadMandatedIssuer`). Igual a `AOS_ISSUER_ID` ⇒ **ABORTA**. Não é segredo: é um nome. |
| `AOS_MANDATED_ISSUER_PUBKEY` | *(vazio)* | Pubkey ed25519 do emissor automático em hex (64 hex) — a da chave `aos-issuer-auto` no Vault transit. **Igual a `AOS_ISSUER_PUBKEY` ⇒ ABORTA**: o emissor automático assinaria com o `iss` do manual e o token verificaria **sem mandato** — é a colisão que anula o limite todo. Igual à chave de um humano pinado ⇒ **ABORTA** (quem cunha também escreveria os mandatos). Malformada ⇒ **ABORTA**. Material **público**. |
| `AOS_MANDATE_SIGNERS` | *(vazio)* | Os humanos cujos mandatos o nó aceita: `user_id=hexpubkey,...` — o `user_id` do token, **sem** o prefixo `human:`. A chave é a do **humano** (a de `aos-issuer mandate-sign --key-file`), e **nunca esteve no servidor**: é por ela ser pinada aqui que o mandato limita alguma coisa. Nome repetido, chave repetida entre dois nomes, prefixo `human:` ou pubkey inválida ⇒ **ABORTA**. **Revogar um mandato** mata todos os tokens cunhados sob ele, sem saber os seus `jti`: `POST /nhi/revoke` com `jti=mandate:<id>` (o `<id>` sai no `mandate-sign`). Tirar um humano desta lista e reiniciar invalida **todos** os seus mandatos. Material **público**. |
...
| `AOS_PRIVILEGED_CAPS` | *(vazio/ausente ⇒ TaintGate **INERTE**, retro-compat)* | Lista de **capabilities privilegiadas** (separadas por vírgula, ex. `cap:fs.read,cap:fs.write,cap:net.connect`) que torna a **barreira control/data-plane** (o `TaintGate`, ADR-005/AOS-069) **EFICAZ**: uma tool call privilegiada cuja autorização é **não-confiável** (`taint=untrusted`) é **barrada** — e a autorização é o rótulo do **contexto** do turno que a pediu, cunhado pelo runtime (ADR-034): uma capability privilegiada só pode ser pedida **antes** de o run ler conteúdo não-confiável. Presente com ≥1 capability ⇒ o ápice compõe a via **ENDURECIDA** (`NewProductionHardenedTaint`). **Ausente, vazia, ou só vírgulas/espaços** ⇒ conjunto vazio, `TaintGate` **presente-mas-inerte** (o comportamento de **todo** deployment até AOS-363, preservado): a barreira estrutural fica desligada. Uma variável **definida-mas-vazia** conta como ausente **de propósito** — é como o idioma de deployment desliga uma opção (`${AOS_PRIVILEGED_CAPS:-}`), e tratá-la como erro quebraria esses deployments. ⚠️ **Consequência a saber:** com o `TaintGate` inerte, a única aplicação de taint que resta é a cláusula `context.taint != "untrusted"` que cada regra Cedar do bundle traz — e no bundle de referência a regra `allow_fs_read` **ainda não a tem** (eixo AOS-363 critério 6, por fechar): um `cap:fs.read` sobre dados não-confiáveis passa. Ligar a barreira estrutural — incluindo **`cap:fs.read`** na lista — é o que fecha esse buraco; **só depois da release com o ADR-034**, porque antes dela toda a tool call do modelo sai untrusted e armar `cap:fs.read` proíbe toda a leitura. **Opt-in de propósito:** o estado (activo/inerte) é **declarado no banner** de arranque. Material **público** (nomes de capabilities). |
...
| `AOS_EVENTSTORE_NATS` | *(vazio ⇒ substrato local)* | Event Store **replicado** (AOS-100); move o log para FORA do volume `aos-data` (ver a nota de **Backup**) — ver [Estado durável](#estado-durável--variáveis-de-ambiente-aos-170--aos-180). |
| `AOS_EVENTSTORE_NATS_STREAM` | *(vazio ⇒ `AOS_EVENTS`)* | Nome do stream JetStream. Só lido com `AOS_EVENTSTORE_NATS`. **Conformidade:** um cluster serve mais do que um board — nomes distintos são o que os mantém separados. |
| `AOS_EVENTSTORE_NATS_REGION` | *(vazio ⇒ fronteira dormente)* | **Região da fronteira de soberania** do board (ADR-011). Só lido com `AOS_EVENTSTORE_NATS`. **Conformidade — é o AC5 do AOS-100:** preenchida, o stream é criado com `placement` restrita a servidores que anunciem `server_tags: ["region:<valor>"]`, e a colocação é **verificada contra a configuração armazenada** — ligar-se a um stream **sem** colocação, ou com a de outra região, **ABORTA** o arranque. **Com `AOS_BOARD_REGIONS` declarado esta variável passa a ser OBRIGATÓRIA** e tem de ser a região de um dos boards: um read-path soberano sobre dados que podem estar em qualquer par do cluster é uma contradição servida em silêncio. |
| `AOS_EVENTSTORE_NATS_STREAM` | *(vazio ⇒ `AOS_EVENTS`)* | Nome do stream JetStream. Só lido com `AOS_EVENTSTORE_NATS`. **Conformidade:** um cluster serve mais do que um board — nomes distintos são o que os mantém separados. |
| `AOS_EVENTSTORE_NATS_REPLICAS` | *(vazio ⇒ 3)* | Factor de replicação (**3**/**5**; `1` é só dev). Só lido com `AOS_EVENTSTORE_NATS`. **Segurança:** presente e não-inteiro-positivo ⇒ o nó **RECUSA arrancar** — degradar para R1 anunciaria replicação sem réplicas. |
...
| `AOS_WORM_TRUST_ANCHOR` | *(vazio ⇒ **verificação ancorada DESLIGADA**)* | Pubkey ed25519 (**64 hex chars = 32 bytes**) do **selador dos checkpoints** do WORM, forçada **out-of-band** (AOS-268/AOS-072). Uma das **três** da verificação ancorada no arranque: presente ⇒ o boot corre `audit.VerifyFromCheckpointAtHead` contra o checkpoint assinado, o que **fecha** o que a re-verificação de AOS-221 sozinha não apanha — a **truncatura do tail** e a **reescrita desde a génese**. Malformada ⇒ **ABORTA** (`ErrBadWormTrustAnchor`). ⚠️ **Segurança:** só material **público**; a chave **PRIVADA** sela os checkpoints **FORA** do nó (custódia out-of-process, **DEFERIDA** — DEF-268, eixo D4/AOS-156). Este nó **consome** a âncora, **não** a produz. ⚠️ **AOS-307 aumentou o custo de a deixar desligada:** desde que o arranque **reidrata** os níveis de autonomia da partição `autonomy` (ver `AOS_AUTONOMY_SETTERS`), o WORM deixou de ser só *prova* e passou a ser *entrada de privilégio*. Sem esta âncora a cadeia só é verificada **contra si própria** (`audit.VerifyStore` re-encadeia, sem raiz de confiança fora do ficheiro), pelo que quem tenha escrita no `AOS_WORM_PATH` pode reescrevê-la de forma consistente. A rehidratação **verifica a assinatura de cada alteração de nível** contra `AOS_OPERATORS` (AOS-305/307), o que fecha a elevação por registo forjado; a âncora continua a ser o que fecha a **truncatura do tail** e a **reescrita desde a génese**. Num deployment que ligue `AOS_AUTONOMY_SETTERS`, ligue também as três. |
| `AOS_WORM_CHECKPOINT_FILE` | *(vazio ⇒ **verificação ancorada DESLIGADA**)* | Caminho do **ficheiro montado** com a **âncora assinada** (JSON de `audit.Checkpoint`) — lida out-of-band, **nunca** do próprio WORM mutável, no padrão de `AOS_POLICY_TRUST_ANCHOR`. Presente ⇒ o boot verifica a cadeia contra este checkpoint. Ficheiro inexistente/ilegível ou JSON malformado ⇒ **ABORTA** (`ErrBadWormCheckpoint`). Uma das **três** da tripla (ver `AOS_WORM_TRUST_ANCHOR`). ⚠️ **Lido em DOIS momentos, com estatutos DIFERENTES:** no **arranque**, para a verificação ancorada (fail-closed, é a única leitura **verificada**); e em **cada recolha de `/metrics`**, **em par com o `AOS_WORM_EXPECTED_HEADS_FILE`** e com a mesma validação de **forma** do arranque (todo o checkpoint tem de trazer piso > 0), para publicar `aos_worm_anchor_delivered_age_seconds` (idade da selagem **entregue**) e `aos_worm_anchor_delivered_unreadable`. A segunda leitura **não verifica nada** — nem assinatura nem frescura, que exigem o store composto — e é por isso **assimétrica**: `1` ⇒ o próximo arranque abortaria; `0` **não** garante que arranca. Existe porque a entrega diária substitui estes ficheiros **sem reiniciar o nó**, e sem ela a morte da tarefa de selagem só seria visível 48 h depois de um restart. A leitura tem **tecto** (4 MiB por metade) e recusa o que não for ficheiro regular, porque `/metrics` é anónimo e sem admission. Material **público** (a âncora é assinada, não secreta). |
| `AOS_WORM_EXPECTED_HEADS_FILE` | *(vazio ⇒ **verificação ancorada DESLIGADA**)* | Caminho de um ficheiro **JSON** com o mapa `particao → audit_seq`: o **piso de frescura POR PARTIÇÃO**, persistido **independentemente** do store **e** do ficheiro de checkpoints. Produz-se com `aos-issuer worm-seal --worm <copia-do-backup> --key-file <selador> --heads`. É o que torna o *rollback de checkpoint* fail-closed: um checkpoint **legítimo mas anterior**, reapresentado para mascarar a truncatura do que veio depois, tem `AuditSeq` abaixo do piso e é recusado (`ErrCheckpointStale`). **Guarde-o noutro sítio que não os checkpoints** — juntos, quem trocasse o ficheiro trocava os dois e o piso deixaria de morder. Toda a partição com checkpoint **tem** de trazer piso, senão o arranque aborta. ⚠️ Substituiu `AOS_WORM_EXPECTED_HEAD` (um decimal, que ancorava **uma** partição); a env antiga, se presente, **aborta o arranque** com `ErrWormExpectedHeadObsoleta` em vez de ser ignorada. Material **público**. |
| `AOS_WORM_EXPECTED_HEAD` | *(**OBSOLETA** — presente ⇒ **ABORTA o arranque**)* | Era um `audit_seq` decimal e ancorava **uma** partição; as partições deste WORM nascem **por run** e são muitas. Substituída por `AOS_WORM_EXPECTED_HEADS_FILE`. O nó continua a **lê-la** para a **recusar** em voz alta (`ErrWormExpectedHeadObsoleta`), e não para a usar: ignorá-la deixaria o operador convencido de que ancorou o que não ancorou — a mesma forma de falha que já custou caro noutro eixo (um banner verdadeiro sobre o mecanismo e enganador sobre o efeito). Remova-a e monte o ficheiro de pisos. Material **público**. |
...
| `AOS_ATTESTATION_VERIFIER_URL` | *(vazio ⇒ **attestation DESLIGADA**)* | URL do **componente de autoridade externo** que verifica a **attestation de dispositivo WebAuthn** (AOS-177) — ex.: `http://attestation:8090/verify`. Presente ⇒ liga o `FourEyesGate` ao [`integration.RemoteDeviceAttestationVerifier`] (cliente HTTP **STDLIB**; o descodificador **CBOR** corre no componente externo, **nunca** no nó — ADR-017/zero-dep) e cada perna de aprovação passa a **EXIGIR** `attestationObject`+`clientDataJSON` válidos: o *enforcement* dormente de AOS-177 fica **ACTIVO**. Fail-closed: exige `https` (ou `http` só em loopback), senão o boot **ABORTA** (`ErrRemoteAttestationURL`); um componente indisponível **NEGA** a aprovação (não degrada). ⚠️ **Segurança (AOS-333):** o URL **não pode trazer credenciais embutidas** (`user-info`, `https://user:pass@host`) — o boot **ABORTA**. É uma **QUEBRA DELIBERADA**: o `net/http` converte `user-info` em `Authorization: Basic`, pelo que esta forma funcionava; a recusa mantém-se porque uma credencial num URL de ambiente aparece na tabela de processos, no `inspect` do contentor e em qualquer erro que ecoe o endereço. **Migração (AOS-338):** o par que estava no URL passa a `AOS_ATTESTATION_VERIFIER_BASIC_PATH` — **ficheiro montado** com `utilizador:senha` —, ou use `AOS_ATTESTATION_VERIFIER_TOKEN_PATH` se o componente falar `Bearer`; em alternativa, termine a autenticação no proxy. ⚠️ **ATENÇÃO AO COPIAR:** o `user-info` de um URL é **percent-decoded** pelo `net/http` antes de virar `Basic` — `https://us%40er:p%3Aw@…` enviava o par real `us@er:p:w`. Uma senha com `@` ou `:` **obriga** a percent-encoding no URL, pelo que copiar o literal para o ficheiro dá **outra credencial**: escreva no ficheiro o par **descodificado**. Os dois definidos ao mesmo tempo **ABORTAM** (`ErrRemoteAttestationAuth`). Só tem efeito com `AOS_APPROVERS_FILE`. Material **público** (uma URL). |
| `AOS_AUTHORITY_FILE` | *(vazio ⇒ **defesa-em-profundidade DORMENTE**, sem revogação)* | Ficheiro JSON montado com o **directório de autoridade** externo do `ScopeGate` (AOS-071): `{"revision":N,"subjects":[{"subject":"human:alice","capabilities":["cap:fs.read"]}]}`. O escopo efectivo passa a ser **token ∩ directório**: **restringe** e **REVOGA**, e **nunca amplia** — a ampliação está estruturalmente fora do alcance do gate, que intersecta com o grant **assinado** pelo issuer. ⚠️ **Semântica que engana:** um sujeito **ausente** do directório **NÃO** é restringido (cai na autoridade do token) — é o que torna seguro ligar um directório **parcial**; mas por isso **revogar não é remover**: liste o sujeito com `"capabilities": []` para lhe negar tudo. Vazio ⇒ o gate acaba a re-verificar `capability ∈ token.Scope`, que a identidade já impôs, e **um token válido vale até expirar aconteça o que acontecer à organização**. Fail-closed de config: ficheiro ilegível, esquema inválido, sujeito vazio/**duplicado** ou lista vazia **abortam** o arranque (`ErrBadAuthorityFile`) em vez de deixarem o directório sem efeito. **Não é assinado** (ao contrário do bundle PDP) por só poder restringir: adulterá-lo nega acções — negação de serviço visível e auditável — mas não concede nenhuma. Material **público** (sujeitos + capabilities). |
| `AOS_ATTESTATION_VERIFIER_TOKEN_PATH` | *(vazio ⇒ **sem bearer**)* | Caminho do **ficheiro montado** com um bearer opcional apresentado ao componente de autoridade de attestation, enviado como `Authorization: Bearer`. ⚠️ **Material PRIVADO por FICHEIRO montado** (nunca por variável de ambiente), no padrão de `AOS_DSAR_VAULT_TOKEN_PATH`. Ilegível **ou VAZIO** ⇒ **ABORTA** o boot (`ErrBadAttestationCredential`) — desde **AOS-338**, no molde dos dois Vaults: um ficheiro em branco era lido em silêncio e o nó falava **sem autenticação nenhuma**. **Mutuamente exclusivo** com `AOS_ATTESTATION_VERIFIER_BASIC_PATH`: os dois definidos **ABORTAM** (`ErrRemoteAttestationAuth`), porque só um cabeçalho `Authorization` é enviado e escolher um em silêncio deixaria o operador a crer que a outra credencial está em uso. |
| `AOS_ATTESTATION_VERIFIER_BASIC_PATH` | *(vazio ⇒ **sem basic**)* | Caminho do **ficheiro montado** com o par `utilizador:senha` (molde do `-u` do `curl`) apresentado ao componente de attestation como `Authorization: Basic <base64>` (**AOS-338**). Existe porque o **AOS-333** passou a recusar credenciais embutidas no URL — o `net/http` convertia `req.URL.User` em `Basic`, e essa via **funcionava** para quem tem o verificador atrás de um reverse-proxy com basic-auth; a recusa mantém-se (uma credencial num URL de ambiente aparece na tabela de processos e no `inspect` do contentor) e **este é o caminho que a substitui**. ⚠️ **Material PRIVADO por FICHEIRO montado**, nunca por variável de ambiente (ADR-006). Fail-closed: ficheiro ilegível, vazio ou acima de 8 KiB, par **sem `:`**, par **sem utilizador** antes do `:`, ou com um **caractere de controlo** — todo o controlo excepto `TAB`, que é o critério do `net/http` para um valor de cabeçalho — **ABORTAM** o boot. A senha pode conter `:` (só o primeiro separa) e **espaços nas pontas**: apara-se o terminador de linha do ficheiro e mais nada, para não corromper uma senha que legitimamente acabe em espaço. **O critério de transporte NÃO relaxa**: `https`, ou `http` só em loopback, o mesmo de `AOS_ATTESTATION_VERIFIER_URL` — basic sobre claro é a credencial em claro. Nenhum **valor** é impresso no banner; o banner declara só **qual** o esquema composto. |
| `AOS_CHALLENGE_ISSUANCE` | `0` *(⇒ **frescura por-cerimónia DORMENTE**)* | `1`/`true`/`on` liga a **FRESCURA POR-CERIMÓNIA** do four-eyes (AOS-266, achado F10): o nó **EMITE** o *challenge* de cada perna por `(pedido, aprovador)` com TTL (`POST /runs/{id}/challenge`) e a verificação passa a **EXIGIR** um *challenge* emitido e fresco (*issue-then-consume*). Sem ela, o anti-replay só dá **dedup** (uso-único), **não** frescura: quem detenha a chave de um aprovador pode reapresentar uma prova capturada (incl. uma attestation WebAuthn) num pedido novo. ⚠️ **Muda a CERIMÓNIA:** o cliente passa a obter o *challenge* do nó (`aos-issuer approve-sign --challenge <hex>`) em vez de o derivar. O **pedido** de challenge é **assinado pelo próprio aprovador** (AOS-308): `aos-issuer challenge-sign --approver <id> --key-file <ficheiro> --run <id> --request-id <id>` produz o corpo; um pedido sem assinatura, de outra chave, ou em nome de **outro** aprovador é recusado (`403`) e nada é escrito. **INDIVISÍVEL** por construção — o endpoint de emissão, o seu autenticador e a porta do gate são compostos no mesmo bloco; nunca há um sem os outros. Só tem efeito com `AOS_APPROVERS_FILE`. Fail-closed: valor não-booleano **ABORTA** (`ErrBadChallengeIssuance`). Material **público**. |
| `AOS_CHALLENGE_TTL` | `5m` | Validade de um *challenge* EMITIDO (AOS-266) — o tempo de **UMA cerimónia** de aprovação, **não** de uma sessão (não se guarda através de *suspend/resume*). Duração Go **positiva** (ex.: `90s`, `10m`); só tem efeito com `AOS_CHALLENGE_ISSUANCE=1`. Fail-closed: valor não-positivo/lixo **ABORTA** (`ErrBadChallengeTTL`). |
| `AOS_DEVICE_ENROLLMENT_FILE` | *(vazio ⇒ **AOS_APPROVERS_FILE**; sem dispositivos ⇒ **atribuição DORMENTE**)* | Ficheiro montado (mesmo esquema de `AOS_APPROVERS_FILE`, campo `devices` por aprovador) que alimenta a **ATRIBUIÇÃO dispositivo↔aprovador** (AOS-266, achado F10): o `device_id` **opaco** (base64, obtido do `/verify` do componente de attestation) atestado em cada perna tem de estar **REGISTADO** para o aprovador que a assinou, senão a perna é **RECUSADA** (`ErrDeviceNotEnrolled`, *default-deny*). É o que faz a attestation provar **de quem** é o autenticador, não só o modelo. **EXIGE** `AOS_ATTESTATION_VERIFIER_URL` (sem `deviceID` não há o que confrontar): dispositivos sem a URL **ABORTAM** o boot (`ErrEnrollmentWithoutAttestationURL`). Fail-loud (molde AOS-193): ficheiro ilegível, `device_id` não-base64, ou ficheiro **explicitamente apontado** sem dispositivos **ABORTAM** (`ErrBadDeviceEnrollmentFile`). Material **público** (digests opacos). |
...
| `AOS_RETENTION_SWEEP_INTERVAL` | `1h` | **Cadência do scheduler INTERNO de retenção** (AOS-267): de quanto em quanto tempo a expiração por TTL corre **SOZINHA**, no loop de serviço, **sem cron externo**. ⚠️ **Conformidade:** até AOS-267 a expiração só corria sob `POST /dsar/expire` — com a política definida e sem agendador externo autenticado, **nada expirava** e o nó anunciava uma retenção que ninguém aplicava (*storage limitation*, RGPD Art. 5.º-1-e, silenciosamente não cumprido). O scheduler só arranca com a **conjunção** política armada + legal hold composto + WORM composto + `ExpirationJob` composto; falha qualquer uma e fica **DORMENTE com a razão declarada no banner** (nunca em silêncio) — em particular, **sem legal hold composto NÃO arranca**, porque um varredor automático sem barreira de preservação poderia apagar material sob *hold* sem ninguém no caminho. Cada passagem sela **dois** registos na cadeia WORM `governance.retention`, sob a identidade **em nome próprio** `nhi:aos-node/retention-scheduler` (nunca a credencial de um operador): `retention.sweep.started` **ANTES** (fail-closed — sem o selo de atribuição a passagem **não corre**) e `retention.sweep.completed` **DEPOIS**, com as contagens (varridos/expirados/**retidos por legal hold**/saltados) e **sem PII**. Partilha com `POST /dsar/expire` o guard que impede passagens concorrentes. Duração Go **> 0**; ilegível ou `≤0` ⇒ **ABORTA** (`ErrBadRetentionSweepInterval`) — **nenhum valor desliga** o scheduler (`0` não desliga: com a política definida, desligá-lo é exactamente a violação que este ticket fecha). Para não expirar nada, deixe `AOS_RETENTION_VERSION`/`AOS_RETENTION_PERIODS` por definir. Material **público**. |
| `AOS_SHUTDOWN_BUDGET` | `8s` | **Orçamento TOTAL do encerramento gracioso**, repartido por **dois drenos com prazos SEPARADOS**: `2/5` para o ingresso HTTP e a **maioria** para o loop de serviço (é ele que drena runs em curso, liberta *leases* e sela desfechos terminais). ⚠️ **Correcção de 2026-08-23:** os dois drenos partilhavam **um só** `context` — e o `http.Server.Shutdown` **espera** pelos pedidos activos sem cancelar os contextos dos *handlers*, enquanto o SSE de trajectória limpa o *write-deadline* entre escritas de propósito. Um **único** stream aberto e ocioso consumia o prazo inteiro e o dreno do serviço recebia um contexto **já expirado**: em vez de drenar, **cancelava** — sob uma linha de log que prometia «drena runs em curso, liberta leases». Quando o dreno HTTP esgota a sua fatia, o ingresso é **fechado à força** (`Close`) para que a fatia do serviço fique intacta. ⚠️ **Tem de caber no `stop_grace_period` do contentor**: o default do Docker é `10s` entre SIGTERM e SIGKILL, e por isso o default aqui é `8s` — deixa **2s** de margem para tratar o sinal e esvaziar o log. Subir esta variável **sem** subir o `stop_grace_period` traz o SIGKILL **a meio** do dreno do serviço, que é pior do que o cancelamento ordenado. O `docker-compose.prod.yml` declara as duas **juntas** (`stop_grace_period: 35s` + `30s`) para a relação ficar num sítio só. Duração Go **> 0**; ilegível ou `≤0` ⇒ o encerramento **prossegue** com o default e a razão sai no log (`ErrBadShutdownBudget`) — uma env inválida não pode impedir um nó de encerrar. Material **público**. |
| `AOS_DSAR_VAULT_ADDR` | *(vazio ⇒ **vault in-memory demo-grade**)* | URL do **HashiCorp Vault** para a custódia EXTERNA da KEK por-titular (AOS-215/AOS-216) — ex.: `https://vault:8200` (cert via `SSL_CERT_FILE`), ou `http://127.0.0.1:8200` em **loopback** para desenvolvimento. Presente ⇒ `Config.DSARVault` liga o adaptador **key-never-leaves** (motor **Transit**): a KEK vive no Vault, o embrulho/desembrulho da DEK corre lá, e o `/dsar/erase`/expiração fazem **crypto-shred** destruindo a chave Transit do titular. Vazio ⇒ o vault de referência in-memory (KEK em memória, perde-se no restart). **Exige** `AOS_DSAR_VAULT_TOKEN_PATH` (senão `ErrBadVaultDSAR`, **ABORTA** — não degrada para o in-memory). ⚠️ **Segurança (AOS-249):** o **esquema é validado fail-closed** com o mesmo critério do verificador de attestation remoto — **exige `https`, e `http` SÓ em loopback** (`127.0.0.0/8`, `::1`, `localhost`); `http://vault:8200` e afins **ABORTAM** o arranque (`ErrInsecureVaultDSARAddr`) ⚠️ **Segurança (AOS-333):** o URL **não pode trazer credenciais embutidas** (`user-info`, `https://user:pass@host`) — o boot **ABORTA**. É uma **QUEBRA DELIBERADA**: o `net/http` converte `user-info` em `Authorization: Basic`, pelo que esta forma funcionava; a recusa mantém-se porque uma credencial num URL de ambiente aparece na tabela de processos, no `inspect` do contentor e em qualquer erro que ecoe o endereço. **Migração:** use o ficheiro de token — o Vault autentica por `X-Vault-Token` e nunca usou a basic-auth do URL, pelo que nenhum deployment de Vault perde nada com esta recusa., porque cada pedido leva o token que **destrói chaves de titulares** e o material de custódia, e em claro pela rede um intermediário obtém *over-erasure* irreversível. Material **público** (uma URL). |
| `AOS_DSAR_VAULT_TOKEN_PATH` | *(vazio ⇒ **sem custódia externa**)* | Caminho do **ficheiro montado** com o token do Vault. ⚠️ **Material PRIVADO por FICHEIRO montado** (nunca por variável de ambiente), no padrão de `AOS_ISSUER_KEY_PATH`/`AOS_TLS_KEY_PATH`; monte-o read-only e fora da imagem. Ilegível/vazio quando `AOS_DSAR_VAULT_ADDR` está definido ⇒ **ABORTA** (`ErrBadVaultDSAR`). Em produção prefira um token de curta duração (AppRole/Kubernetes-auth): desde **AOS-249** o nó **RE-LÊ este ficheiro periodicamente** (adopta sem reiniciar um token rodado por um agente externo) e **renova** a lease (`renew-self`) antes de a margem esgotar. |
...
| `AOS_BROKER_VAULT_PATH_PREFIX` | *(vazio ⇒ **sem prefixo**)* | Prefixo de path **opcional** para dar namespace às credenciais do broker dentro do KV v2 (ex.: `aos-broker`). O layout de leitura é `[prefix/]provider/region/capability` (cada segmento sanitizado para caracteres Vault-seguros), com o valor no campo `value`. Material **público**. |
| `AOS_MODEL_ENDPOINT` | *(vazio ⇒ **modelo de REFERÊNCIA**)* | Raiz de uma API **OpenAI-compatível** (ex.: OmniRoute/OpenRouter/LiteLLM) para o **Model Gateway** do nó (EPIC-06). Presente ⇒ o nó compõe o gateway REAL (`packages/platform/model-gateway`, `NewProduction`) — allowlist regional **assinada** (embebida), keypool, routing de failover, metering/pricing e endurecimento SSRF — e liga-o a `Config.Model`; ausente ⇒ o `referenceModel` (turno único fixo). **Exige** `AOS_MODEL_NAME` (senão `ErrBadModelConfig`). Fora de produção aceita `http://` (seam de egress delegado); **sob `AOS_MODE=production` EXIGE `https://`** e um host na allowlist de egress (AOS-366) — `http` ou host fora da allowlist **ABORTA** o arranque (`ErrInsecureBaseURL`/`ErrHostNotAllowed`). Material **público** (uma URL). |
| `AOS_MODEL_NAME` | *(vazio ⇒ **sem gateway**)* | Id do modelo a pedir ao gateway (o `model` do `/v1/chat/completions`, ex.: o modelo configurado no OmniRoute). Obrigatório quando `AOS_MODEL_ENDPOINT` está definido; senão `ErrBadModelConfig` (**ABORTA**, não degrada para o modelo de referência). É também o `manifest.model.model_id` de cada `turn.recorded` e o `gen_ai.request.model` dos spans (AOS-396): o nó escreve-o no goal de cada run que hospeda, incluindo as retomas. Material **público**. |
| `AOS_MODEL_EGRESS_HOSTS` | *(vazio ⇒ **deriva do host de `AOS_MODEL_ENDPOINT`**)* | **Só produção** (AOS-366). Allowlist de egress do gateway (CSV de `host` ou `host:porta`) contra a qual o `BaseURL` — e **cada salto de redirect** — é validado no caminho SSRF fail-closed (AOS-223). Vazio ⇒ a allowlist é o host do próprio endpoint (o destino já configurado, sem uma segunda variável a manter em sincronia). Definir para **restringir** ou **alargar** o conjunto de hosts. **Impacto de SEGURANÇA:** uma entrada a mais reabre egress para esse host; fora de produção é ignorada (o seam de dev delega a validação). Material **público**. |
| `AOS_MODEL_EGRESS_TIMEOUT` | *(vazio ⇒ **30 s** sob produção; **60 s** no seam de dev)* | Tempo máximo de **cada pedido HTTP** do Model Gateway ao modelo (duração Go entre `1s` e `30m`, ex.: `120s`). Sob `AOS_MODE=production` é o `Timeout` do transporte **endurecido** (AOS-366) — TLS 1.2, allowlist e re-validação de cada redirect **não mudam**; fora de produção substitui o do seam de dev. Existe porque um tecto fixo de 30 s cortava a meio os turnos de um provider **lento mas são** (um modelo de raciocínio com contexto de tool), com `Client.Timeout exceeded while awaiting headers`. Fora dos limites ⇒ **ABORTA** (`ErrBadModelEgressTimeout`), em vez de seguir com o default em silêncio. Material **público**. |
| `AOS_MODEL_API_KEY_PATH` | *(vazio ⇒ **sem bearer upstream**)* | Caminho do **ficheiro montado** com a API key do gateway upstream (torna-se o segredo de infra que o `CredentialProvider` do gateway serve). ⚠️ **Material PRIVADO por FICHEIRO montado** (nunca por variável de ambiente), no padrão de `AOS_ISSUER_KEY_PATH`. Ilegível/vazio ⇒ **ABORTA** (`ErrBadModelConfig`). Ausente **com o gateway ligado**: em `AOS_MODE=production` ⇒ **ABORTA** (`ErrProductionNeedsModelCredential`, AOS-247 — a produção nunca apresenta ao upstream o bearer de dev embebido no binário); fora de produção ⇒ o nó usa esse bearer de dev não-vazio (o upstream ignora-o se não o exigir) e **declara-o** numa linha de banner em cada arranque. |
...
| `AOS_MODEL_TOOLS_REGISTER` | *(vazio ⇒ **não registado**; revalidação nega)* | Booleano (`1`/`true`/`on`…). Presente e verdadeiro ⇒ as tools de `AOS_MODEL_TOOLS` são **registadas como um catálogo ASSINADO e congelável**, para a **revalidação de registry** do Reference Monitor (AOS-051, o 2.º hook) as **ADMITIR**. **Efeito de segurança — move o gate:** sem isto, uma tool oferecida ao modelo mas sem contrato assinado registado é **negada pela revalidação** (`denied_by=revalidation`, trust store vazio) **antes** do PDP; com isto a revalidação passa e a decisão flui ao **gate seguinte, o PDP (3.º hook)**. O PDP tem, ANTES do Cedar, o gate de **allowlist de capabilities por `agent_class`** (AOS-007, `capabilities/allowlist.json` assinado): a capability tem de constar da allowlist da classe (as committadas são `agent-worker`=cap:http.post+cap:fs.read, `agent-reader`=cap:fs.read, `agent-break-glass`=`*`). Passando esse gate, o Cedar avalia sob o taint da **autorização**, que é o rótulo do **contexto** do turno cunhado pelo runtime (ADR-034): uma call pedida depois de um `plan_input`, resultado de tool ou memória tem `taint=untrusted` e `cap:http.post` é negada (`allow_http_post` exige `context.taint != "untrusted"`, AOS-069/P4); uma call pedida sobre contexto só com o objectivo é `trusted` e passa essa cláusula — o `web_post` committado morre então na **região** (`allow_http_post` exige `resource.region == "eu"`, a tool declara `eu-west`). `cap:fs.read` (regra sem cláusula de taint) passa o PDP em qualquer caso. O `demo-pdp-taint-gate.sh` foi escrito sob a premissa antiga (ver o cabeçalho dele). ⚠️ **DEV-GRADE (auto-assinado):** o nó gera uma chave ed25519 **efémera** ao arranque, assina o catálogo e **confia nela** (trust store in-process) — análogo, no eixo do registry, à identidade demo-only self-minted; em **produção** o catálogo é assinado **out-of-band** por um publicador confiável e a pubkey é forçada por config, **nunca** a própria chave do nó. Ligado mas com `AOS_MODEL_TOOLS` ausente/mal formado, ou `egress` inválido num spec ⇒ **ABORTA** (`ErrBadModelToolsRegister`). Material **público**. |
| `AOS_MODEL_TOOLS` | *(vazio ⇒ **modelo SEM tools**)* | Caminho de um **ficheiro JSON montado e TRUSTED** com o **registry de tools** OFERECIDAS ao modelo (análogo mínimo do EPIC-05). Cada entrada declara a **face do modelo** (`name`/`description`/`parameters` — o `tools` do request OpenAI, sem o qual o modelo **não emite** `tool_calls`) **e** o **binding de governança** (`capability` + `resource_type`/`resource_value`/`resource_region`) que o **Reference Monitor** avalia. Presente ⇒ cada tool call é **MEDIADA** pelo RM/PDP no ponto único (ADR-002): o binding vem do **registry** (config trusted) e o modelo só escolhe **qual** tool pelo nome. O taint da **autorização** não vem do modelo nem do registry: é o rótulo do **contexto** do turno, cunhado pelo runtime (ADR-034) — trusted enquanto o contexto só tem o objectivo e correcções do humano, untrusted a partir do primeiro `plan_input`, resultado de tool ou memória —, logo uma capability privilegiada pedida depois de conteúdo não-confiável é **NEGADA** pelo taint-gate (AOS-069, "untrusted não comanda"). Tool fora do registry ⇒ capability vazia ⇒ **default-deny**. Ficheiro ilegível / JSON inválido / entrada sem `name`+`capability` ⇒ **ABORTA** (`ErrBadModelTools`). 🔒 **Com `AOS_BOARD_REGIONS` definida (AOS-407):** cada tool tem de declarar uma `resource_region` **dentro** das regiões dos boards, senão o arranque **ABORTA** (`ErrToolRegionForaDosBoards`) — com a soberania por board ligada, uma tool fora do mapa seria negada em **todas** as chamadas pela obrigação `region`, e servir tools mortas em silêncio é pior do que recusar. Alinha-se a tool ao board (as tools committadas declaram `eu-west`, o board de produção), **nunca** o board à tool: a residência dos runs já selados depende dele. Ausente ⇒ o modelo não recebe tools (comportamento inalterado). Material **público** (schema + binding). |
| `AOS_MODEL_AUDIT_PATH` | *(vazio ⇒ **audit de governação do GW IN-MEMORY, volátil**)* | Caminho do **WAL de um WORM durável** (`audit.OpenFileStore`, hash-chain tamper-evident, mesmo motor do WORM do nó) onde o **Model Gateway** sela a **governação de soberania**: o changelog de **activação da allowlist** regional (assinada/verificada) no arranque **e** cada decisão **allow/deny por chamada** (incl. o deny de failover cross-border). Presente ⇒ essa governação **sobrevive ao restart** (replay crash-safe). Vazio ⇒ `audit.NewMemStore` (volátil, comportamento até AOS-265) — o **banner** declara o modo. Caminho definido que **não abre** (inacessível / hash-chain adulterada que o replay recusa) ⇒ **ABORTA** (`ErrBadModelAudit`), em vez de degradar em silêncio. ⚠️ **Cadeia DEDICADA do gateway:** a partilha do **WORM único do nó** fica **deferida** — o cliente de modelo compõe-se ANTES de `Bootstrap` abrir o WORM do nó (a re-ordenação da composição do modelo é o eixo). 🔒 **Um escritor por caminho (AOS-399):** o nó pede a **posse exclusiva** do caminho ao SO (`<path>.lock`, o árbitro do AOS-285) **antes** de o abrir; se outro processo (outro nó, ou um `aos-orq` com a mesma variável) já o detém, o nó **recusa arrancar** (`ErrEventStoreJaDetido`, a mesma recusa do Event Store e do WORM detidos) sem tocar no WAL. Cada processo precisa do **seu** caminho, distinto também do `AOS_WORM_PATH`. Material **público** (um caminho). |
| `AOS_MODEL_PRICING_PATH` | *(vazio ⇒ **tabela de preços EMBEBIDA** — só os pares de referência do repositório)* | Caminho de um **documento de preços** do operador (formato de `pricing_table.json`: `version` + `entries` com `(model, region)` e os **quatro rates** — input / output / cache-read / cache-write — em **micro-USD inteiro por 1M tokens**). É a **FONTE do número do custo** (AOS-259): o Model Gateway deriva o custo de cada turno destes rates × os tokens que o provider ecoou, e esse valor viaja num **único canal** — `port.Usage.CostMicroUSD` → `ModelResponse.CostMicroUSD` → span `chat` (`aos.cost.micro_usd`) → campo `cost_micro_usd` do evento durável `turn.recorded`, que é a **fonte do burn-down** (AOS-261). A contabilidade só é **ARMADA** se a tabela em vigor tiver preço para o par (`AOS_MODEL_NAME`, `AOS_MODEL_REGION`) deste nó; **sem preço para o par não há custo derivado**: desde o AOS-406 cada turno sai marcado — span `chat` com `aos.cost.undefined=true` (sem `aos.cost.micro_usd`), `turn.recorded` com `custo_nao_derivado: true` — e o SLI `cost_per_trajectory` não conta esses traces (fica sem amostras, nunca verde com zeros); o **banner** declara-o, e nenhum preço é inventado. É a postura certa para um modelo pago por **subscrição**. ⚠️ Com um alias no LiteLLM, a entrada leva o **nome do alias** (é o par que o nó consulta) e as **taxas do modelo que serve**, não as do nome pedido. ⚠️ A tabela **embebida** cobre `claude-sonnet`/`claude-haiku`/`gpt-4o` em `eu-west`/`us-east`; o **default** `AOS_MODEL_REGION=eu` **não** casa com ela, pelo que um nó real precisa desta variável para ter custo em dólares. O burn-down **não** fica cego sem ela: a dimensão que decide é **tokens** (`AOS_BUDGET_MAX_TOKENS`), que continua fail-closed. Caminho definido que **não carrega** (ilegível, JSON inválido, versão vazia, rate negativo, par duplicado) ⇒ **ABORTA** (`ErrBadModelPricing`), em vez de facturar com os preços que o operador julga ter substituído. Material **público** (preços não são segredo). |
| `AOS_APPROVERS_FILE` *(exigência nova em produção)* | *(vazio ⇒ four-eyes DESLIGADO, `/approve` responde 501)* | Ficheiro montado com os aprovadores do **four-eyes** (AOS-162). ⚠️ **Sob `AOS_MODE=production` passa a EXIGIR `AOS_DURABLE_EXECUTION=1`** (`ErrProductionNeedsDurableApproval`, AOS-021): o bridge negação→aprovação→reexecução depende da execução durável para (a) **reproduzir o turno escalado com fidelidade** — o log durável **não** guarda os inputs das tool calls, só a captura de replay os tem, e sem eles a acção aprovada nunca voltaria a ser apresentada de forma idêntica (a amarra da preview nunca bateria) — e (b) **impedir a dupla execução** das activities já aplicadas do mesmo turno (step-ledger). Sem ela ficaria um four-eyes que verifica assinaturas e **não destrava nada**: pior do que desligado, porque cria a expectativa de aprovação humana onde só há negações. Fora de produção a exigência não se aplica. Material **público** (principals + pubkeys + autoridade). |
| `AOS_AUTONOMY_LEVELS` | *(vazio ⇒ **oráculo NÃO ligado**; nada escala)* | Níveis de autonomia por par **agente:domínio**, formato `agt-1:http=L4,agt-1:fs=L5`. Aceita também **regras de CLASSE**: `class:agent-worker:fs=L4` aplica-se a QUALQUER NHI dessa classe. A resolução é em **cascata** — instância → classe → piso (`AOS_AUTONOMY_DEFAULT`) → `L0` — e a instância GANHA sempre à classe, pelo que uma regra de classe nunca eleva um agente que alguém pôs deliberadamente mais baixo. A classe é a unidade **estável**: os `agent_id` deste sistema são cunhados por run, e registar instâncias é registar identidades que ainda não existem. É o que torna o veredicto **`escalate`** ALCANÇÁVEL — e com ele todo o bridge de aprovação humana (AOS-021). ⚠️ **Cedar NÃO exprime escalate**: a decisão Cedar é permit/deny; é a composição **nível × classe de risco** que rebaixa um permit para escalate quando o oversight exige um humano. Níveis: `L0` sugestão (**tudo** escala) · `L1` confirma cada acção · `L2` lote (danger confirma) · `L3` tiering SA-ROC · `L4` corre e **só danger confirma** · `L5` corre, danger em amostragem post-hoc. **Opt-in de propósito:** o registo é fail-closed (par não registado ⇒ `L0`), pelo que ligar com registo vazio faria **cada** tool call exigir aprovação individual. ⚠️ **Não chega sozinha: exige também `AOS_POLICY_BUNDLE_DIR`** — o oráculo entra pela opção do PDP carregado, e sem bundle esta variável é **ignorada em silêncio** (nem chega a ser lida, pelo que nem uma config malformada aborta). Com bundle, config malformada ⇒ **ABORTA** (`ErrBadAutonomyLevels`). Cada nível provisionado é **SELADO** no WORM como `autonomy.level_changed` (partição `autonomy`, com motivo e actor `config:node`), e uma selagem recusada **ABORTA** o arranque (`ErrAutonomyProvisioning`, AOS-248): a autonomia com que o nó corre não muda sem rasto. O estado (ligado/não ligado) é declarado no **banner** de arranque. Para MUDAR um nível em RUNTIME use `POST /autonomy`, cujo corpo canónico é produzido por **`aos-issuer autonomy-sign --emitter <id> --key-file <ficheiro> --agent <id> --domain <d> --level <L0..L5> --reason <texto> [--co-emitter <id2> --co-key-file <ficheiro2>]`** — a chave privada assina FORA do nó, como em todo o plano de controlo, e reimplementar o tuplo à mão é a divergência que essa ferramenta existe para evitar. ⚠️ O emissor tem de constar de **`AOS_AUTONOMY_SETTERS`** e `L4`/`L5` exigem a **segunda assinatura** (AOS-305). Um nível posto por `POST /autonomy` **sobrevive ao reinício** (AOS-307): o arranque **reidrata** o registo da partição `autonomy` do WORM e, para cada par, o último selo de **operador** prevalece sobre esta variável — o ambiente só sobrepõe pares cujo último selo foi do próprio provisionamento (`config:node`); os pares preservados são **declarados no banner**. Uma selagem falhada em `POST /autonomy` responde **`503`** e **não** aplica o nível (AOS-306). Material **público**. |
| `AOS_AUTONOMY_SETTERS` | *(vazio ⇒ **`POST /autonomy` recusa toda a mudança**, 403)* | Lista `emitterID,emitterID2` dos operadores de `AOS_OPERATORS` que detêm a capability **`autonomy:set`** (AOS-305) — os únicos que podem mudar níveis por `POST /autonomy`. Assinar bem **não chega**: um operador de `AOS_OPERATORS` fora desta lista é recusado **mesmo para `L1`**. ⚠️ **Mudar PARA `L4` ou `L5`** — o limiar em que `danger` deixa de esperar por um humano — **exige DUAS assinaturas de emissores DISTINTOS** desta lista, sobre o mesmo corpo (`aos-issuer autonomy-sign … --co-emitter <id2> --co-key-file <ficheiro2>`); as restantes transições, incluindo qualquer descida, exigem uma. A distinção de pubkeys é a de `AOS_OPERATORS` (pubkey partilhada aborta o arranque), pelo que dois ids são duas chaves. Um id ausente de `AOS_OPERATORS` ⇒ **ABORTA** (`ErrBadAutonomySetters`). O selo `autonomy.level_changed` nomeia **os dois** signatários e transporta as **provas** (as assinaturas do pedido), que o arranque **reverifica** ao reidratar (AOS-307). ⚠️ **Migração:** um registo de operador **anterior** a este mecanismo — ou um registo que não verifique — é **SALTADO** no arranque (nunca aplicado; o par fica no nível de `AOS_AUTONOMY_LEVELS`) e **declarado no banner** com o `audit_seq`; para o repor, **reassine** a alteração por `POST /autonomy`. O arranque **não aborta** por um registo mau, de propósito: abortar entregaria um modo de tijolo a quem tenha escrita no ficheiro do WORM. Declarado no **banner**. Material **público** (ids). |
| `AOS_AUTONOMY_PROOFS` | *(vazio ⇒ **sem provas**; uma subida a `L4`/`L5` pelo ficheiro é recusada ao nível)* | **Provas assinadas que AUTORIZAM cada SUBIDA a `L4`/`L5` declarada em `AOS_AUTONOMY_LEVELS`** (AOS-377). Fecha o buraco em que editar o ficheiro e reiniciar aplicava **qualquer** nível **sem assinatura**, contornando o dual-control que `POST /autonomy` impõe. Formato **objecto JSON** `{ "agente:dominio=Ln": [ {"emitter_id":…,"signature_b64":…,"nonce_b64":…,"issued_at":…}, … ] }` — a **chave** é o par **exactamente** como em `AOS_AUTONOMY_LEVELS` com o **nível de destino** (`agt-1:fs=L5`, `class:agent-worker:fs=L4`), e cada valor é a lista de provas. ⚠️ **Só as SUBIDAS que atravessam o limiar do gate humano são bloqueadas:** destino `>= L4` **E** acima do nível corrente do par. **Descer qualquer nível e subir até `L3` NUNCA exigem prova** (AC2 — a de-escalada é a alavanca de incidente). Subir a `L4`/`L5` exige **DUAS provas de emissores DISTINTOS** de `AOS_AUTONOMY_SETTERS`, verificadas contra `AOS_OPERATORS` e o direito `autonomy:set`, sobre o **payload canónico** `(agente, domínio, nível, motivo="provisionamento por AOS_AUTONOMY_LEVELS")` — a **mesma** verificação que a rota e a rehidratação (AOS-305/307). Gere-as com **`aos-issuer autonomy-sign --emitter <id> --key-file <f> --agent <id> --domain <d> --level <L4\|L5> --reason "provisionamento por AOS_AUTONOMY_LEVELS" --co-emitter <id2> --co-key-file <f2>`** e cole as duas provas seladas no array. **Sem prova válida a subida é RECUSADA AO NÍVEL** — o par fica no nível **anterior** (reidratado ou piso), o arranque **NÃO aborta** (fail-closed sem modo de tijolo) e a recusa é **declarada no banner**. ⚠️ Um **deployment INALTERADO** cujo `L4`/`L5` já está selado no WORM **não** volta a pedir prova (o provisionamento idempotente salta o gate — retro-compatível). ⚠️ **Não chega sozinha: exige `AOS_POLICY_BUNDLE_DIR`** (como `AOS_AUTONOMY_LEVELS`); JSON malformado, chave sem `=Ln` ou prova sem `emitter_id` ⇒ **ABORTA** (`ErrBadAutonomyProofs`). Material **público** (só assinaturas, nonces e carimbos — nenhum segredo). |
| `AOS_DSAR_ERASERS` | *(vazio ⇒ **prova de autoridade DSAR DESLIGADA** — as rotas ficam por leitura)* | Lista `emitterID,emitterID2` dos operadores de `AOS_OPERATORS` que detêm a capability **`dsar:erase`** (AOS-367) — os únicos autorizados a **ASSINAR** as quatro rotas de destruição de dados: `POST /dsar/erase` (crypto-shred), `/dsar/hold`, `/dsar/release` e `/dsar/expire`. ⚠️ **Segurança:** vazio ⇒ a prova está **desligada** e as rotas mantêm a autenticação por **LEITURA** (`readGov.authorize`) que sempre tiveram — o mesmo par issuer/audience OIDC que serve `GET /runs/{id}`; é retro-compatível (dev, testes por headers) mas significa que **um token emitido para LER autoriza o crypto-shred IRREVERSÍVEL**. Definida (≥1 id) ⇒ a prova é **EXIGIDA**: cada uma das quatro rotas passa a exigir, além da identificação OIDC, a assinatura ed25519 de um eraser sobre o payload canónico (com a **acção** amarrada — uma assinatura de `hold` não se reapresenta como `erase`), com nonce durável de uso único e frescura. A chave privada assina **FORA** do nó, como em todo o plano de controlo. O `/dsar/release` ganha ainda a **barreira de região** (um chamador de outra região não levanta o hold de um titular cuja residência a fronteira protege) e o `/dsar/expire` exige **DUAS assinaturas de erasers DISTINTOS** (dual-control — é um varrimento global sem alvo único). Um id ausente de `AOS_OPERATORS`, vazio ou duplicado ⇒ **ABORTA** (`ErrBadDSARErasers`). ⚠️ **Sob `AOS_MODE=production` a lista NÃO pode ser vazia** (`ErrProductionNeedsDSARErasers`): a destruição irreversível não pode ficar autorizada por um token de leitura. Material **público** (ids). |
| `AOS_AUTONOMY_DEFAULT` | *(vazio ⇒ **`L0`** — o mais supervisionado)* | **PISO** dos pares **sem nível registado**. Sem ela nada muda: um par desconhecido continua a resolver para `L0`, fail-closed. Existe para o piso ser uma **DECLARAÇÃO** e não uma herança do valor-zero — hoje um par ausente cai em `L0` **em silêncio**, e é isso que faz "ligar a autonomia" significar, sem mais nada, **"todo o agente novo bloqueia"** (os `agent_id` deste sistema são cunhados por run, pelo que nenhum está registado à partida). A diferença não é de comportamento: é de **quem responde pela decisão**. ⚠️ **Um nível explicitamente registado GANHA sempre ao piso** — declarar `L4` aqui não eleva pares que alguém pôs deliberadamente em `L0`. Valor fora de `L0..L5` ⇒ **ABORTA** o arranque (`ErrBadAutonomyDefault`), em vez de cair no valor-zero e ignorar em silêncio o que o operador escreveu: um typo que produz a postura *mais restritiva* é o pior tipo de typo, porque nada parece errado e ninguém o procura. O piso em vigor é devolvido por **`GET /autonomy`** em `unregistered_resolves_to`. Material **público**. |
| `AOS_AUTONOMY_CONTROL` | *(vazio ⇒ **política de referência do blueprint** — erro ≤ 2%, override ≤ 40%, janela 30 dias, salto de demoção 2, piso `L1`)* | **Policy-as-code dos limiares do controlador de autonomia** (AOS-090/ADR-025, CA3). JSON com `version` (SemVer), `error_rate_max`, `override_rate_max`, `window` (ex.: `"720h"`), `demotion_drop`, `demotion_floor` (`L0..L5`) e `promotion_ceil`. Governa a **PROMOÇÃO** (só promove se erro **e** override ficarem sob os limiares SUSTENTADOS na janela) e a agressividade da **DEMOÇÃO** (quantos níveis baixa, até que piso). ⚠️ O tecto de promoção **AUTOMÁTICA** é sempre **travado abaixo de `L4`** independentemente do `promotion_ceil` declarado: o controlador nunca auto-promove para `L4`/`L5` — isso exige a cerimónia assinada (`AOS_AUTONOMY_SETTERS`, dual-control). ⚠️ **Segurança/Conformidade:** a promoção automática **NÃO sobrevive a reinício** (o par reverte ao último nível assinado); a demoção automática (por TRIP do disjuntor) sim, por só descer. JSON/duração/nível inválido ⇒ **ABORTA** (`ErrInvalidControlConfig`). Só tem efeito com o oráculo composto (`AOS_AUTONOMY_LEVELS` + `AOS_POLICY_BUNDLE_DIR`). Material **público**. |
| `AOS_AUTONOMY_EVAL_INTERVAL` | *(vazio ⇒ **`1h`**)* | Cadência do **Evaluate periódico** da promoção automática de autonomia (AOS-090/ADR-025). Duração (ex.: `30m`, `2h`). Sobre uma janela de dias, avaliar de hora a hora é folgado. Não afecta a DEMOÇÃO, que é reactiva ao TRIP do disjuntor (imediata, não periódica). Material **público**. |
| `AOS_BREAKER_MAX_STALE_ITERATIONS` | `3` | Iterações consecutivas **sem progresso** antes de o disjuntor do agente vivo abrir (AOS-080/081). O default vem de evidência directa: um run observado repetiu a MESMA tool call negada 16× até esgotar `MaxTurns`; 3 é cedo para poupar ~13 turnos e tarde para tolerar um retry legítimo. `0` desliga o sinal. |
...
| `AOS_CRASH_RESUME_INTERVAL` | `2m` (= TTL do lease) | Período da **re-varredura de órfãos** (AOS-253). A varredura de crash-resume corria **só no arranque**, e num nó **ÚNICO** isso deixa um buraco real: matando o nó a meio de um run, o arranque seguinte **encontra** o órfão e **salta-o** — o lease da encarnação **anterior do mesmo processo** ainda não expirou (TTL de 2 min) e a varredura recusa-se, correctamente, a roubar partição. Como nada re-varria, o run ficava em `running` **para sempre**, com trabalho durável feito, e a API respondia `404` (indistinguível de "nunca existiu") até alguém **reiniciar outra vez**. Em multi-réplica não há buraco (outra réplica reclama). Com esta variável o nó volta a tentar depois de o lease expirar — **sem nunca reclamar leases mais cedo**: reclamar por reconhecer a própria encarnação anterior produziria **dupla execução** se o processo antigo ainda estivesse vivo. Silenciosa quando não há órfãos. `0` **desliga** e deixa só a varredura de arranque (opt-out explícito, declarado no banner). Valor ilegível ⇒ **ABORTA** o arranque. Material **público**. |
...
| `AOS_SANDBOX_GVISOR_URL` | *(vazio ⇒ **skeleton**)* | URL do **componente gVisor** (host-side externo, ex.: `http://gvisor:9101/exec`) que conduz o `runsc` REAL. Só se aplica com `AOS_SANDBOX_DRIVER=gvisor`: presente ⇒ o nó INJECTA um executor remoto (HTTP stdlib) no driver e uma tool call permitida EXECUTA num sandbox gVisor dedicado — bundle OCI efémero, rootfs read-only, raiz semeada em bind read-only, zero capabilities, `noNewPrivileges`, uid não-root e sem rede. ⚠️ **A fronteira é DIFERENTE da do Firecracker e deve ser lida como tal:** o gVisor **interpõe syscalls em user-space** (plataforma `systrap`), enquanto a microVM tem uma fronteira de **virtualização de hardware** (ADR-004) — mais forte. Em troca, o gVisor **NÃO exige `/dev/kvm`**, e é por isso a única fronteira ao nível do kernel disponível num host que seja ele próprio um convidado sem virtualização aninhada (o caso de um VPS típico, onde `firecracker` está estruturalmente fora de alcance). Ausente ⇒ o driver gvisor é o **skeleton** e o exec devolve `ErrDriverUnavailable` (o gap honesto: o executor não está provisionado) — nunca uma execução fora do sandbox. O nó fica **zero-dep**: o `runsc` corre no componente, não no nó, no padrão do `attestation` e do orchestrator Firecracker. Material **público** (a URL interna). |
...
| `AOS_CONTROL_MTLS_CA_PATH` | *(vazio ⇒ **mTLS de controlo DESLIGADO**)* | Caminho do **bundle PEM da CA de cliente** que autentica o plano de controlo por **certificado** (DEF-012, EIXO 1) — ver [mTLS do plano de controlo](#mtls-do-plano-de-controlo--autenticação-forte-otlp-def-012). Presente ⇒ as **11 rotas do plano de CONTROLO** (`/steer`,`/pause`,`/approve`,`/challenge`,`/resume`,`/exhaustion`,`/promote` as tres de `/autonomy` e `/nhi/revoke`) exigem um **certificado de cliente verificado**, **ALÉM** da assinatura ed25519 do corpo (AOS-160) — é **ADITIVO, nunca um bypass**. Exige **terminação TLS no nó** (senão `ErrControlMTLSNeedsNodeTLS`); bundle inválido ⇒ `ErrBadControlMTLSCA`. Material **público** (a CA). |
...
| `AOS_OTLP_SERVICE_NAME` | `aos` | **`service.name` do recurso OTLP** (AOS-368) — a **IDENTIDADE do produtor** que o backend de traces usa para atribuir a trajectória. Vazio ⇒ default determinista **`aos`**; o nó **nunca** exporta um documento sem identidade, pelo que o backend **nunca** vê o trace como `unknown_service`. Só tem efeito com `AOS_OTLP_ENDPOINT` definido (senão o tracer é o `NoopTracer` e não há documento). Material **público** (um rótulo de serviço). |
| `AOS_SLO_EVAL_INTERVAL` | `1m` | **Cadência do avaliador de SLOs/alertas em runtime** (AOS-274): de quanto em quanto tempo o nó agrega a sua própria telemetria, avalia os **4 SLIs da mediação** (AOS-085/086) e os **7 SLIs canónicos operacionais** (AOS-104/105) na **mesma passagem**, e produz alertas com **janela sustentada** (um pico transitório não alerta). ⚠️ Até AOS-274 os avaliadores só corriam em **testes**: os SLOs existiam como documento e os **runbooks de AOS-106 mapeavam alertas que ninguém produzia**. **Onde se vê o resultado:** `GET /metrics` (`aos_slo_sli`, `aos_slo_target`, `aos_slo_samples`, `aos_slo_breached`, `aos_alert_firing` — **com o runbook como *label*** —, e, sem SLO, a escrita do selo de mediação por decisão em `aos_mediation_audit_write_samples` / `aos_mediation_audit_write_latency_ns`, AOS-402, e a janela da política partida por hook em `aos_mediation_hook_samples` / `aos_mediation_hook_latency_ns`, AOS-405) e uma linha de **log estruturado** por alerta disparado, já com o **título, o doc e o ADR** do runbook resolvido. **Fontes REAIS, nunca sintéticas:** os spans que o próprio nó emite (via a torneira que espelha o que sai para o colector — exige `AOS_OTLP_ENDPOINT`, senão os SLIs derivados de spans ficam **sem amostras**, nunca «cumpridos»), a **sonda de prontidão** do plano de controlo (a mesma do `/readyz`) e a **verificação da hash-chain** do WORM. Sem produtor no nó (headroom do *scheduler*, fidelidade de *replay*) o SLI fica **sem amostras** e **não** dispara — nada é injectado para «preencher o painel». 🔓 **FAIL-OPEN DECLARADO — a excepção do nó:** ao contrário de tudo o resto (fail-closed), **a observabilidade NUNCA derruba o nó** — pânico contido, sem propagação ao caminho de execução, sondas com prazo próprio, e o laço **não pára** nem ao detectar adulteração. A **configuração**, essa, é fail-closed: duração Go **>= 0**; ilegível ou negativa ⇒ **ABORTA** (`ErrBadSLOEvalInterval`). **`0` DESLIGA** explicitamente o avaliador (e o *banner* di-lo). Material **público**. |
| `AOS_SLO_WINDOW` | `5m` | **Janela de dados** que cada passagem do avaliador (AOS-274) agrega: só entram os spans que **fecharam** dentro dela. Existe porque o anel de retenção é limitado por **contagem**, não por tempo — sem janela, um nó que ficou ocioso continuaria a publicar (e a alertar sobre) os percentis do último *burst*, para sempre. Duração Go **> 0**; ilegível ou `≤0` ⇒ **ABORTA** (`ErrBadSLOWindow`) — `0` não é aceite porque uma janela vazia deixaria todos os SLIs derivados de spans permanentemente sem amostras, o que é **indistinguível de um nó calmo**. Material **público**. |
| `AOS_BACKUP_EXPORT_INTERVAL` | *(vazio ⇒ default do módulo `platform/backup`, **30s**)* | **Periodicidade do ciclo de exportação do Event Store para backup imutável** (AOS-101) — e a **BASE DO RPO**: a janela máxima de perda é limitada por esta cadência. Até agora o `platform/backup` **não estava ligado a binário nenhum** (`Export()` era UM ciclo, sem laço; o `go list -deps` do nó dava **zero** ocorrências do módulo), pelo que «backup contínuo» descrevia uma *capacidade*, não um comportamento. Agora um **laço no loop de serviço** (mesmo molde dos outros cinco: *ticker*, mesmo `sweepStop` fechado pelo `Shutdown`) corre o **mesmo** `Exporter.Export`: incremental (só o que passou do *head* anterior), **cifrado em repouso** (AES-256-GCM, DEK fresca por segmento; a KEK do backup vive na custódia de `AOS_BACKUP_VAULT_TRANSIT_MOUNT` — **envelope**: a DEK é embrulhada **dentro** do Vault Transit, num mount **próprio**, e a KEK nunca entra no processo, AOS-453), encadeado num **manifesto hash-chain** com **checkpoint ed25519**. **Onde se vê:** o *banner* de arranque do serviço e `GET /metrics` (`aos_backup_scheduler_armed`, `aos_backup_export_cycles_total`, `aos_backup_export_failures_total`, `aos_backup_scheduler_stopped`, `aos_backup_export_periodicity_seconds{region}` e `aos_backup_rpo_window_seconds` — a **janela efectiva de RPO**, agora medida em runtime e não só em teste). 🔓 **FAIL-OPEN:** um ciclo falhado é contado e re-tentado, um pânico é **contido** — o backup nunca derruba o nó. **Cinco paragens permanentes**, distinguíveis no `/metrics` de «a correr»: violação de **soberania** (re-tentar seria insistir numa cópia *cross-border* negada, ADR-011), **cadeia com outro dono** (`ErrChainOwned` — um destino por exportador), **registo de ciclo que não verifica** (`ErrCycleRecordInvalid` — adulteração, lixo ou outra chave), **colisão de conteúdo** na ref do segmento (`ErrSegmentRefCollision`) e **log atrás do cursor** da cadeia (`ErrSourceBehindBackup` — rebobinado, ou de outro nó). ⚠️ **ISTO NÃO LIGA O BACKUP.** O interruptor é o **DESTINO** — `AOS_BACKUP_DEST` (linha abaixo; porta `backup.ImmutableStore`, AOS-453 F2), **vazio por omissão**. Um destino durável já é **utilizável** (AOS-101): o exportador **retoma** a cadeia que lá esteja, verificada fail-closed no arranque — uma cadeia de outra chave, de outra região, ilegível, com um buraco, ou cujo último segmento não abre com a KEK deste nó (KEK destruída, de outro mount, ou o vault de referência em memória, que morre com o processo), e um destino que não é write-once condicional, **ABORTAM o arranque**; um Event Store atrás do cursor (outro log, PITR, DR) **não** aborta: o nó sobe e o laço de backup pára no primeiro ciclo —, e `Restorer.LoadManifest` reconstrói a cadeia para restauro. Os dois banners (composição e serviço) dizem se a cadeia é **NOVA** ou **RETOMADA do ciclo N**. Sem destino, o exportador **nem é composto** e nada muda: o backup imutável em produção continua a ser o `deploy/server/backup.sh` (cópia de **volume**, cron diário, **RPO de 24h**). Com destino, a composição é **fail-closed** em cada perna (Event Store sem `BackupSource`, `Config.BackupSigningKey` em falta — *trust domain* próprio, ADR-017 §5: o nó **não** auto-gera a chave que sela *checkpoints*, porque uma chave nova por arranque tornaria inverificáveis os anteriores — ou soberania violada ⇒ **ABORTA o arranque**). Duração Go **> 0**; ilegível ou `≤0` ⇒ **ABORTA** (`ErrBadBackupExportInterval`) — **nenhum valor desliga** o agendador. Material **público**. |
| `AOS_BACKUP_DEST` | *(vazio ⇒ backup imutável **DESLIGADO** — nada muda, também em produção)* | **O interruptor do backup imutável do Event Store** (AOS-453 F2 → AOS-101). `file:///caminho/absoluto` ⇒ destino **em disco local, write-once** (`backup.FileImmutableStore`: temporário + `fsync` + `link` — a escrita numa referência existente é recusada atomicamente, o equivalente local do `If-None-Match: *`). O directório **tem de existir** e ser gravável pelo uid `65532` — o nó **não o cria** (um caminho mal escrito criaria em silêncio um destino vazio e o banner diria «cadeia NOVA»). `s3://…` ⇒ **ABORTA** (`ErrBackupDestNotImplemented`: o S3 com Object Lock é a fase **F4**, não implementada); outro esquema ⇒ **ABORTA** (`ErrBadBackupDest`). ⚠️ **O disco local NÃO protege da perda do host nem de root** — é write-once perante o exportador, não perante o sistema operativo. Com destino, **todas** as quatro variáveis abaixo são obrigatórias e fail-closed. **Épocas:** a retenção é finita; rodar de época é apontar este caminho para um **subdirectório novo** (génese nova) antes de a época anterior expirar. Material **público**. |
| `AOS_BACKUP_DEST_REGION` | *(nenhum — obrigatória com `AOS_BACKUP_DEST`)* | **Região de soberania do destino** (ADR-011). Tem de ser uma das regiões de `AOS_BOARD_REGIONS` (em produção, sempre confrontada); fora dela, ou em falta ⇒ **ABORTA** (`ErrBadBackupDestRegion`). O exportador revalida-a contra a do Event Store na construção e a cada ciclo. Material **público**. |
| `AOS_BACKUP_SIGNING_KEY_PATH` | *(nenhum — obrigatória com `AOS_BACKUP_DEST`)* | **Ficheiro com a seed ed25519 (64 caracteres hex) que sela os checkpoints do backup** — *trust domain* **próprio** (ADR-017 §5), gerada **OFFLINE** pelo dono. O nó **LÊ-A e NUNCA a cria** (sem `LoadOrCreate`): uma chave nova por arranque tornaria inverificáveis os checkpoints anteriores e a retoma recusaria a cadeia. Ausente, ilegível ou mal formada ⇒ **ABORTA** (`ErrBadBackupSigningKey`). 🔒 Material **PRIVADO** — só por ficheiro montado `:ro`, legível pelo uid `65532`. |
| `AOS_BACKUP_RETENTION` | *(nenhum — obrigatória com `AOS_BACKUP_DEST`)* | **Object-lock de cada objecto do backup** — FINITO por decisão do dono (épocas). Duração Go `> 0` (ex.: `2160h` = 90 dias); ausente, ilegível ou `≤ 0` ⇒ **ABORTA** (`ErrBadBackupRetention`). Um prefixo expirado **recusa** a retoma (uma cadeia incremental sem génese não se restaura): roda-se de época **antes** de a anterior expirar. Material **público**. |
| `AOS_BACKUP_VAULT_TRANSIT_MOUNT` | *(vazio; **obrigatória** em `AOS_MODE=production` com `AOS_BACKUP_DEST`)* | **Custódia da KEK do backup** (AOS-453): mount Transit **PRÓPRIO** (ex.: `transit-backup`) sobre o **mesmo** Vault e o **mesmo token** de `AOS_DSAR_VAULT_ADDR` (a credencial é uma só, mantida pelo renovador da DSAR). Os segmentos selam-se por **envelope**: a DEK é embrulhada no Vault e a KEK nunca entra no processo. Tem de ser **diferente** do mount do DSAR — o do DSAR está atrás do portão da reconciliação de apagamentos (AOS-436) e ao alcance do `delete` da política do nó —, e a política do nó sobre ele **não tem `delete`** (`deploy/server/provision-identity.sh`). Sem `AOS_DSAR_VAULT_ADDR`, igual ao mount do DSAR, ou em falta em produção ⇒ **ABORTA** (`ErrBadBackupVault`); com a custódia do nó em Vault e sem este mount, a composição recusa (`ErrBackupVaultMountMissing`). Uma custódia que não responde é `ErrKEKCustodyUnavailable` — nunca lida como «KEK errada». Material **público**. |
...
| `AOS_ORQ_PLAN_BUDGET_MAX_TOKENS` | *(vazio ⇒ `1073741824` — 1<<30, declarado no banner como POR OMISSÃO)* | **Só o `aos-orq`** (AOS-434). Tecto em TOKENS da raiz da árvore de orçamento de um plano. **NÃO protege do custo do modelo** — o que limita é a soma das ESTIMATIVAS DECLARADAS (a do planeador e o `budget_estimate` de cada nó do documento); o planeador reserva a estimativa e nunca a reconcilia com o consumo real, e o trabalho dos nós corre como runs do nó e debita `AOS_BUDGET_MAX_TOKENS`, que é o travão de custo REAL. Um plano cuja soma não caiba é **RECUSADO** na materialização (não há adiamento nesta árvore). Valor ilegível ou `<= 0` **ABORTA** o arranque (`ErrTectoDePlanoInvalido`) — o zero não desligaria o tecto, negaria todos os planos. Material **público** (um número). |
| `AOS_ORQ_PLAN_BUDGET_MAX_COST_MICRO_USD` | *(vazio ⇒ `1073741824` micro-USD)* | **Só o `aos-orq`** (AOS-434). A segunda dimensão do mesmo tecto. Uma reserva só cabe se couber nas **duas** dimensões. Mesma postura fail-closed e as mesmas ressalvas de alcance da linha anterior. Material **público** (um número). |
...
**Integridade do registo.** Cada linha leva um MAC **encadeado** (cobre a linha anterior): uma linha
forjada, re-datada, removida, inserida ou trocada a meio deixa de autenticar e nunca destrói. Um
importado a quem falte um apagamento que o bundle restaurado já conhece é **mais antigo** do que ele e
é recusado — e, recusado o importado, **nada** se escreve no registo próprio. A chave **nunca** se
recria por cima de um registo com entradas, nem quando há uma importação pedida (tem de vir do
bundle); nasce atómica (temporário + `link`). Aceite, o importado é fundido e **deixa de ser lido**.
**Indisponível não é apagado:** portão fechado ou Vault sem resposta sai do cifrador como
`durable.ErrConteudoIndisponivel` — o step-ledger falha fechado na retoma em vez de esquecer um passo
já aplicado, e o replay soberano responde 503, não o 410 de «apagado». Os erros e logs não nomeiam a
KEK pelo nome do Vault (invertível): usam o prefixo do `id` do registo, ou nada.
...
intervalo entre a criação e a destruição faz uma KEK ressuscitada parecer nova; um nó adiantado (dentro
da folga de 5 min) pode fazer uma KEK nova parecer ressuscitada. (4) Só a custódia Vault implementa a
porta. (5) Uma KEK conhecida **só** pelo registo cujo titular a cadeia não nomeia não chega ao legal
hold. (6) Expirações por TTL anteriores a AOS-436 não estão em registo nenhum. (7) Um `key_destroyed`
selado antes do AOS-249 sobre uma chave que nunca morreu: o primeiro arranque destrói-a, com os dados
escritos depois do pedido de apagamento. (8) **O corte do fim** de um registo — ou importar um registo
mais recente do que o bundle mas mais antigo do que o último — não se distingue por conteúdo; confere-se
pela contagem do `pull.log`. (9) **Portão global:** uma linha rejeitada numa fonte fecha o conteúdo de
**todos** os titulares até o operador corrigir a fonte. (10) **(R6) Quem lê e escreve o volume de dados
destrói a KEK viva de qualquer titular:** a chave do registo vive no volume, ao lado dele, e o nome de
uma KEK deriva de um keyRef público — quem tiver as duas coisas escreve uma linha válida. Antes do
AOS-436 o acesso ao volume não bastava para isso; agora basta. (11) **(R8) Vault restaurado com o nó a
correr:** até ao tick seguinte (≤ 1 min) a KEK ressuscitada decifra e aceita escritas, que a
re-destruição depois torna ilegíveis. (12) Um só processo por volume: a escrita do registo não tem lock
entre processos.
...
| Variável | Default | Efeito |
|---|---|---|
| `AOS_EVENTSTORE_NATS` | *(vazio)* | Endereço(s) `host:porta` — **vários separados por vírgula, e é o recomendado** — de um cluster **NATS JetStream** que serve de Event Store **replicado por quórum** (AOS-100, ADR-007). **PRECEDE** `AOS_EVENTSTORE_PATH`. **Impacto de conformidade — o mais importante desta tabela:** este substrato **ARBITRA ENTRE PROCESSOS** (o `expected_seq` é imposto pelo servidor, medido), pelo que correr **N réplicas do nó** sobre ele passa a ser a configuração PRETENDIDA e o guard de arranque de AOS-285 deixa de se aplicar ao Event Store. O guard do **WORM mantém-se** — o WORM continua a ser um ficheiro local e dois escritores forkam a hash-chain (AOS-284). **Disponibilidade:** o cliente **RECONECTA-SE sozinho** e as escritas retomam — medido: **1 s** depois de morrer o nó da ligação, sem reiniciar o processo. **Com um só endereço** só há cura quando esse nó voltar; daí a lista. **Soberania:** ver `AOS_EVENTSTORE_NATS_REGION`. **Backup:** o log deixa de estar no volume `aos-data` — o `deploy/server/backup.sh`, que copia esse volume, **recusa-se** a produzir um artefacto verde e sem log (ver `deploy/server/README.md` §Backup). |
| `AOS_EVENTSTORE_NATS_STREAM` | *(vazio ⇒ `AOS_EVENTS`)* | Nome do stream JetStream. Só é lido com `AOS_EVENTSTORE_NATS` definido. Um cluster serve mais do que um board; nomes distintos separam-nos. |
| `AOS_EVENTSTORE_NATS_REPLICAS` | *(vazio ⇒ 3)* | Factor de replicação do stream (**3** ou **5**; `1` é só dev). Só é lido com `AOS_EVENTSTORE_NATS` definido. **Presente e não-inteiro-positivo ⇒ o nó RECUSA arrancar** (`ErrBadEventStoreReplicas`): degradar em silêncio para R1 daria um nó a anunciar substrato replicado sobre um stream sem réplicas. |
| `AOS_EVENTSTORE_PATH` *(exigência nova em produção)* | *(vazio)* | **Vazio ⇒ Event Store IN-MEMORY** (volátil: perde tudo quando o processo/contentor morre). Definido ⇒ Event Store **durável** (WAL append-only + `fsync` + replay crash-safe no arranque) no caminho dado. **Tem de apontar para DENTRO do mount gravável** (`-v aos-data:/var/lib/aos`, ex.: `/var/lib/aos/events.wal`). ⚠️ **Sob `AOS_MODE=production` é OBRIGATÓRIO** — este ou `AOS_EVENTSTORE_NATS` — senão `ErrProductionNeedsDurableSubstrate` (AOS-300). Ao contrário das outras duas guardas de durabilidade, que são condicionais a uma opção (`AOS_DURABLE_EXECUTION` e `AOS_APPROVERS_FILE`), esta é **incondicional**: a **revogação de NHI** é composta sempre desde AOS-288, e sobre um store volátil um token revogado volta a ser **aceite** ao primeiro restart. |
...
### Orçamento / tecto de custo — o alcance declarado (AOS-008 / AOS-255 / AOS-260)
...
Leia-a **antes** de ligar um orçamento, não depois.

> ⚠️ **A declaração MUDOU em AOS-260.** Até aqui dizia «cobre tool calls em TOKENS; o gasto de
> inferência é travado por tempo (wall-clock), não por tecto» — e era verdade: `rt.model.Call`
> era invocado **fora** da cadeia do Reference Monitor e nenhuma reserva o admitia. A decisão do
> dono (D1 = opção B) mandou fechar esse eixo e ele está fechado: há uma **porta de admissão**
> no *runtime* que reserva *headroom* **imediatamente antes** da inferência e o salda com o
> consumo **medido** da resposta. Se leu a frase antiga noutro documento, é a antiga.

| Eixo | O que o orçamento cobre | O que o trava |
|---|---|---|
| **Tool calls** | ✅ **Sim, em TOKENS** — o *hook* de orçamento é um `Hook` do Reference Monitor e a cadeia é atravessada **por tool call**; a estimativa incide sobre a **call materializada** (ver abaixo). | Negação por falta de *headroom* (fail-closed, auditada no WORM). |
| **Turno de modelo (inferência)** | ✅ **Sim, desde AOS-260** — a porta `agentruntime.ModelAdmission` **reserva** com o *prompt* já materializado, imediatamente antes de `rt.model.Call`, e **salda** logo a seguir com `usage` + `cost_micro_usd` **reais**. É a **maior** fatia do gasto de um *run* e passou a ter tecto. | Esgotamento ⇒ **degradação declarada** (ver abaixo): o *loop* **não retenta**. |
| **Resultado da tool** | ❌ Não como linha própria — volta à transcrição e é **pago** quando entra no *prompt* do turno seguinte (aí sim, contado pela estimativa do turno), mas não é admitido separadamente. | O tecto do turno seguinte. |

**O que acontece quando esgota — degradação DECLARADA, nunca *deny-loop*.** O *loop* **não**
volta a tentar o turno negado (retentar queimaria *wall-clock* sem progresso e o *run* morreria
pelo disjuntor, com a causa **errada** no log). Há dois desfechos, e qual deles ocorre depende do
que o nó tem composto:

| Maquinaria HITL (`AOS_APPROVERS_FILE` + operador pinado) | O que acontece ao esgotar |
|---|---|
| **Composta** (prompt de exaustão ARMADO, AOS-263) | O *run* **suspende** em `waiting_on_human` com um **pendente durável** — o mesmo mecanismo do aviso de burn-down. Visível em `GET /runs/{id}`; decide-se em `POST /runs/{id}/exhaustion` (`continue` ou `abort`, assinado). O *run* fica **retomável**. |
| **Não composta** | O *run* **pára com razão própria** e o log durável sela **`timed_out`** com o rótulo **`budget_exhausted`** — ao lado de `max_turns_exhausted` e do *wall-clock*, que são os outros tectos defensivos. **Nunca `failed`**: um tecto atingido não é uma falha recuperável por compensação, e mandá-lo para `failed` accionaria a saga de *rollback* (AOS-254) sobre efeitos legítimos. |

**Retomar um *run* não o cobra duas vezes.** Na retoma, os turnos já dados são **reproduzidos**
da captura (o *model client* devolve a resposta registada e nenhum *provider* é chamado). A
admissão reconhece-os e **não reserva nada** — dedup por `run_id:step_id`, o mesmo par de
idempotência do Event Store e do *step-ledger*. A regra é a simetria: **sem chamada, sem
cobrança**.

Consequências práticas, ditas sem rodeios:

- **A dimensão que decide por omissão é TOKENS.** A de **dólares** é debitada e legível em cada
  *run* (o canal de custo está ligado ponta a ponta desde AOS-259), mas só **nega** se definir
  `AOS_BUDGET_MAX_COST_MICRO_USD`. A projecção de custo do turno seguinte sai da **tarifa medida
  do próprio *run*** — nada é inventado; no **primeiro turno de cada incarnação** ainda não há
  medição e a decisão é só por *tokens*.
...
- ⚠️ **O tecto é também POR-INCARNAÇÃO — não é cumulativo entre elas.** A árvore de orçamento
  ...
  escalada/retoma pode consumir até **N × `AOS_BUDGET_MAX_TOKENS`** (em *tool calls* **e** em
  turnos de modelo).
...
**O que a estimativa da TOOL CALL conta — e o que não conta (AOS-258).** O estimador em vigor é
...
| Não conta | Porquê |
|---|---|
| O **turno de modelo** | tem **estimador próprio** desde AOS-260 (ver abaixo) — não é este que o cobre |
| O **resultado da tool** | só é mensurável **depois** do efeito; entra no orçamento quando entra no *prompt* do turno seguinte, contado aí pela estimativa desse turno |
| **Dólares** | o custo de um efeito não é conhecível antes dele; a dimensão `CostMicroUSD` da estimativa da tool call é **zero** de propósito. Quem debita dólares é o turno de modelo, com o número **medido** |

**O que a estimativa do TURNO DE MODELO conta (AOS-260).** O **prompt materializado** do turno —
os mesmos bytes que vão para o *provider*, contados pela **mesma** aproximação por átomos da
tool call — **mais uma provisão de output fixa**, porque o tamanho da resposta é desconhecido
antes da chamada. A provisão é uma **reserva**, não uma cobrança: assim que a resposta chega, o
**consumo medido** substitui-a e o excedente volta ao *headroom* **no mesmo turno**.
Sobre-provisionar custa admissão momentânea, nunca orçamento.

> ⚠️ **Um turno cujo consumo real exceda o que restava do tecto** é cobrado até ao topo (o *run*
> fica a 100%) e o *run* **não recebe mais turnos de modelo**. O número exacto do gasto **não se
> perde**: fica no ledger durável de turnos, que é a fonte do burn-down.

**Como se liga, e o que muda (AOS-256/AOS-257/AOS-260).** O orçamento é **opt-in por uma
variável**, `AOS_BUDGET_MAX_TOKENS`, com uma segunda **opcional** para o eixo `$`,
`AOS_BUDGET_MAX_COST_MICRO_USD` (ver o índice de configuração):

- **Por definir (o default):** não há orçamento **nenhum** — o ponto de injecção `budget` da
  cadeia fica com o **stub neutro**, a porta de admissão do turno de modelo **não é ligada**,
  nenhuma decisão consulta custo, e o banner di-lo em cada arranque (`orcamento / tecto de custo
  (AOS-008): NAO COMPOSTO`).
- **Definida (inteiro `> 0`):** o `BudgetCheck` real entra na cadeia **e** a porta de admissão do
  turno de modelo é composta; **cada *run* recebe o seu próprio nó de orçamento** com esse tecto,
  registado antes do primeiro turno e **libertado no fim** (também em erro e em *panic*). Cada
  tool call **reserva** a sua estimativa antes de executar e a reserva é **confirmada** em
  `permit` ou **devolvida** em `deny`/`escalate`/erro — incluindo uma negação **a jusante** (o
  `egress` corre depois do `budget`), que de outro modo roubaria *headroom* ao *run* sem nada ter
  gasto. Cada turno de modelo **reserva** a sua provisão antes da inferência e **salda** pelo
  consumo medido. O banner passa a `COMPOSTO e a cobrir as DUAS coisas que gastam` — **com a
  mesma frase acima**, palavra por palavra.
- **`AOS_BUDGET_MAX_COST_MICRO_USD` definida (inteiro `> 0`):** a dimensão **dólares** passa a
  **decidir** a par dos *tokens* — uma reserva só cabe se couber nas **duas** dimensões, pelo que
  o *run* é negado pela primeira que esgotar. Sem ela, dólares são **medidos e debitados** mas
  nunca negam.
- **Definida com valor ilegível, negativo ou `0`:** o nó **ABORTA o arranque** (`ErrBadBudget` /
  `ErrBadBudgetCost`). `0` não é «desligado»: nenhuma estimativa cabe em zero, logo seria **tudo
  negado**. `AOS_BUDGET_MAX_COST_MICRO_USD` **sem** `AOS_BUDGET_MAX_TOKENS` também aborta — não
  há orçamento onde pendurar o tecto em `$`, e escrevê-lo na config faria o operador ler uma
...
> **Se precisa de limitar o gasto de inferência hoje** (reescrito em AOS-260 — a frase anterior
> dizia que o único travão era temporal, e deixou de ser verdade): defina
> **`AOS_BUDGET_MAX_TOKENS`**. O tecto por *run* passou a cobrir **também o turno de modelo**, por
> **reserva prévia** — a porta `agentruntime.ModelAdmission` reserva com o *prompt* já
> materializado imediatamente antes de `rt.model.Call` e salda com o **consumo medido** da
> resposta. Opcionalmente, **`AOS_BUDGET_MAX_COST_MICRO_USD`** acrescenta o tecto em **dólares**,
> que decide a par dos *tokens* — e que o nó **recusa** aceitar sem fonte de preço para o par
> (modelo, região), em vez de o servir inerte (`ErrBudgetCostNoPriceSource`). O
> **`AOS_BREAKER_MAX_WALL_CLOCK`** continua a ser um travão útil, mas agora é **complemento**, não
> substituto: cobre o run pendurado que gasta pouco, enquanto o orçamento cobre o run que gasta
> muito depressa.
...
#### Os quatro PLANOS, e porque a barreira deixou de estar no corpo do handler

O ambito do mTLS nao e uma lista escrita na documentacao: e a **classificacao de cada rota** na
tabela de `packages/cmd/aos/planos.go`, e e dela que as barreiras derivam.

| Plano | Rotas | Barreiras aplicadas no REGISTO |
|---|---|---|
| `planoAberto` | `/healthz`, `/readyz`, `/metrics` | **nenhuma** — sondas e scrape nao assinam e nao podem consumir o balde (causaria falso-unready) |
| `planoDados` | `POST /runs`, `GET /runs/{id}`, `/trajectory`, `/reconstruct` | nenhuma no involucro: o balde de ingresso e da **submissao** e o gate soberano devolve a identidade do leitor ao corpo |
| `planoGovernacao` | `/dsar/erase`, `/dsar/hold`, `/dsar/release`, `/dsar/expire` | admissao (token-bucket) + credencial **forte** do gate soberano, verificada no corpo |
| `planoControlo` | `/steer`, `/pause`, `/approve`, `/challenge`, `/resume`, `/exhaustion`, `/promote`, `POST /autonomy`, `GET /autonomy`, `/autonomy/simular` | admissao **+ mTLS de cliente**, ambos ANTES do handler |

O **valor-zero** de `plano` e invalido: uma rota registada sem plano declarado **aborta o arranque**
(`ErrRotaPorClassificar`). Antes, cada handler chamava as barreiras no seu proprio corpo — e tres
rotas nasceram sem elas enquanto o comentario da propria rota afirmava o contrario. O que tornava
isso invisivel: **sem `AOS_CONTROL_MTLS_CA_PATH` a barreira devolve sempre `true`**, pelo que um
handler que a esquecesse era indistinguivel de um que a chamasse, em dev, em CI e em producao — ate
ao dia em que alguem ligasse o mTLS e descobrisse que uma das portas nunca esteve fechada.

> **Decisao em aberto — o DSAR nao exige certificado de cliente.** As quatro rotas de
> `planoGovernacao` autenticam-se pela assercao OIDC verificada (claim de *board*), que identifica o
...
apresentado) e as **11 rotas de `planoControlo`** (tabela acima) **RECUSAM** (`403`) um pedido sem um
...
| `POST /runs/{id}/resume` | **Retoma** um run SUSPENSO à espera de aval humano (AOS-021). Corpo: `{"credential":"<NHI fresco>"}`. A credencial original **não é persistida** (é um bearer token e teria expirado na janela de 15 min) — quem retoma **re-autentica-se**. Os turnos já dados são **reproduzidos da captura** (o modelo NÃO é reinterrogado, senão emitiria outra tool call e a aprovação — amarrada à preview — nunca se aplicaria); a acção escalada é re-mediada pela cadeia **completa** e encontra agora o grant. Run não suspenso ⇒ **404 uniforme** (não enumera runs alheios); sem four-eyes ⇒ 501. |
...
### Ratificação de auto-modificação — `POST /promote` (AOS-275)

O *promotion controller* (AOS-159/AOS-206) é composto **sempre**, mas até AOS-275 só era
alcançável **in-process**: com ratificadores em `AOS_RATIFIERS`, o nó anunciava uma capacidade que
**nenhum operador conseguia exercer**. A rota fecha-o — **não** afrouxa nada:

| Rota | O que faz | Colaborador |
|---|---|---|
| `POST /promote` | Submete uma **ratificação assinada** de um artefacto de auto-modificação (*skill*/memória procedural). `200` ⇒ promovido; `403` ⇒ recusado; `400` ⇒ corpo inválido (nada chega ao *gate*). | `hitl.RatificationGate` de **produção** (*freshness* + *nonce-store* **durável** forçados) |

**Autenticação: a MESMA do `/approve`.** *Token-bucket* do plano de controlo, mTLS de controlo
quando composto (`AOS_CONTROL_MTLS_CA_PATH`) e — a barreira que **decide** — a **assinatura
ed25519** do ratificador verificada contra a **pubkey pinada** em `AOS_RATIFIERS`. Não há
credencial ambiente e o nó continua **non-signing**: a chave privada do humano nunca entra no
processo. Sem ratificadores pinados, todo o `POST /promote` é `403` (`ratifier_unknown`).

**Anti-replay.** A **mesma** ratificação re-submetida pela rota é recusada com
`ratification_replayed` (*nonce* consumido no *Event Store*) — a resposta HTTP é `403` **uniforme**
e o motivo estável fica **selado no WORM**, na partição `ratification:<artifact-id>`, com o
**principal** (o ratificador) e a assinatura, para re-verificação independente. Uma tentativa cuja
assinatura **não** verifica não é atribuível e vai para a partição de quarentena
`ratification-unratified`.

**Produzir o corpo (fora do nó).** A ratificação assina um canónico com separador de domínio e o
`request_id` tem de ser o `RatificationID` do artefacto — não se calcula à mão:

```bash
aos-issuer ratify-sign \
  --artifact-id skill.summarize --kind skill --version 1.4.0 \
  --content-hash "$(openssl dgst -sha256 -binary skill.md | base64)" \
  --canary-passed \
  --eval-suite golden.summarize --eval-id eval-abc-123 \
  --eval-dataset golden --eval-verdict pass --eval-score 0.95 \
  --ratifier human:alice --key-file /etc/aos/ratifier.seed > promote.json
curl -sS -X POST --data @promote.json https://<no>/promote
```

> ⚠️ `--key-file` é a **seed ed25519 do ratificador** — material **PRIVADO por ficheiro montado**,
...
docker run --read-only --tmpfs /tmp -v aos-data:/var/lib/aos -p 8443:8443 \
  ...
  -e AOS_BOARD_REGIONS="board:prod=eu-west" `# a regiao das tools TEM de casar - ver AOS_MODEL_TOOLS` \
