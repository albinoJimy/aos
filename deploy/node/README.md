...
| `AOS_PRIVILEGED_CAPS` | *(vazio/ausente ⇒ TaintGate **INERTE**, retro-compat)* | Lista de **capabilities privilegiadas** (separadas por vírgula, ex. `cap:fs.read,cap:fs.write,cap:net.connect`) que torna a **barreira control/data-plane** (o `TaintGate`, ADR-005/AOS-069) **EFICAZ**: uma tool call privilegiada cuja autorização foi promovida sobre dados **não-confiáveis** (`taint=untrusted`) é **barrada**. Presente com ≥1 capability ⇒ o ápice compõe a via **ENDURECIDA** (`NewProductionHardenedTaint`). **Ausente, vazia, ou só vírgulas/espaços** ⇒ conjunto vazio, `TaintGate` **presente-mas-inerte** (o comportamento de **todo** deployment até AOS-363, preservado): a barreira estrutural fica desligada. Uma variável **definida-mas-vazia** conta como ausente **de propósito** — é como o idioma de deployment desliga uma opção (`${AOS_PRIVILEGED_CAPS:-}`), e tratá-la como erro quebraria esses deployments. ⚠️ **Consequência a saber:** com o `TaintGate` inerte, a única aplicação de taint que resta é a cláusula `context.taint != "untrusted"` que cada regra Cedar do bundle traz — e no bundle de referência a regra `allow_fs_read` **ainda não a tem** (eixo AOS-363 critério 6, por fechar): um `cap:fs.read` sobre dados não-confiáveis passa. Ligar a barreira estrutural — incluindo **`cap:fs.read`** na lista — é o que fecha esse buraco. **Opt-in de propósito:** o estado (activo/inerte) é **declarado no banner** de arranque. Material **público** (nomes de capabilities). |
