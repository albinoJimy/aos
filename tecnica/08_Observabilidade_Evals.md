...
**Implementação (AOS-077).** A propagação EM-`context.Context` do `SpanContext` (secção 3.1) só liga spans dentro do MESMO `ctx`; NÃO atravessa a fronteira de serialização de uma delegação (RT-pai → `Orquestrador.Spawn` → RT-filho), onde o sub-agente corre com o seu `ctx`-raiz novo. O veículo cross-fronteira é um **carrier W3C `traceparent`** (`FormatTraceParent`/`ParseTraceParent` em `substrate/otel-genai/traceparent.go`, `fail-closed`, zero-dep): no `Spawn`, o `Delegator` abre o span `invoke_agent`-**âncora** do filho COMO FILHO do `SpanContext` do pai (vindo do `ctx` no mesmo processo, ou de `SpawnRequest.ParentTraceParent` na fronteira) e devolve o `traceparent` da âncora em `SpawnHandle.ChildSeedTraceParent` (e no evento `subagent.spawned`, atributo não-secreto — só ids de trace/span). O RT-filho semeia o seu `ctx`-raiz com esse `traceparent` (`Goal.ParentTraceParent`) antes de abrir o SEU `invoke_agent`, que assim herda o `trace_id` do pai e aponta `parent_span_id` ao `span_id` da âncora — ligação NATIVA OTel, com os atributos NHI (`aos.node.parent_nhi`) como reforço. A escolha do ancoradouro: a âncora do `Spawn` é a **decomposição** (custo 0) e o `invoke_agent` do RT-filho **continua a mesma árvore** um nível abaixo (custo real da execução), coerente com `startNodeSpan` de AOS-025. O canal de **resumo-ao-pai** é o artefacto distinto `agentruntime.TrajectorySummary` (limitado a ~1–2k tokens por um proxy de tamanho — NÃO contabilidade fina de custo/token por span, que é AOS-078), devolvido por `Runtime.RunDelegated` em paralelo com a árvore completa que o `tracer` exporta ao backend. A recursão (map-reduce) usa o mesmo mecanismo em cada nível — neto semeia a partir do handle do filho — produzindo uma árvore de N níveis **acíclica e completamente ligada** (um só `trace_id`, cada não-raiz com exactamente um pai presente), verificada por teste de propriedade a profundidade arbitrária.

### 4.1 Redação de PII no backend (AOS-188)

O motor de redação de PII (`packages/substrate/redaction`, AOS-091) é um módulo folha do substrate que aplica **minimização** antes da persistência. O seu `Ingestor` oferece três vias — `UserInput`, `ToolResult` e `Memory` — todas delegando no mesmo `Engine.Redact`, o que prova por construção que o mesmo classificador/política protege eventos, spans e audit records. Hoje o motor é consumido pelos módulos de governação do plano de controlo (trajectory-surface, approval-card, dsar, confidence-calibration) e cablado nos composition-roots; a ligação directa ao Event Store, `platform/memory`, `substrate/otel-genai` e `platform/audit` ainda não está completa na v1. Quando essa ligação ocorrer, o contrato é: o payload serializado que entra em qualquer destino de backend é varrido pelo `Engine.Scan` e não pode conter PII em claro — o teste `TestNoPIIPersistsInEventSpanAuditSerialization` em `packages/substrate/redaction/aos091_test.go` simula e prova este contrato.
...
2–8,6 ms; a diferença atribuiu-se, por inferência, à escrita durável do selo. O **AOS-401** emendou o ADR-026: o SLO governa só a política. A medida directa do **AOS-402** (2026-09-17, v0.1.19) desmentiu a inferência: a escrita custa 6,5–8,3 ms e política + escrita ≈ 12–13 ms, pelo que a diferença para os ~31 ms da v0.1.15 não é da escrita. O **AOS-404** explicou-a: os ~31 ms da v0.1.15 foram o p95 de poucas amostras dominado por duas tool calls lentas (AOS-404): a 1.ª do run, com a escrita a frio (WORM 7,9 ms + Event Store ~15 ms), e a 6.ª, cuja própria política levou 17,06 ms — e a política, afinal, nem sempre coube em 2–8,6 ms (4 de 27 calls de produção passaram os 15 ms).
