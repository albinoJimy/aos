...
| Despacho governado (AOS-390, ADR-024) | O `plandispatch.Dispatcher` (elegibilidade + poda `branch_not_taken`) **não tinha chamador de produção**; a materialização dava efeito eager, ignorando `conditional_on` (fail-open, §2.1) | O efeito de despacho **move-se da materialização para o despacho** (ADR-024): a materialização ADMITE (nós pendentes no DAG, sem efeito) e o `DispatchSink`, sob a posse, produz o efeito por-nó só quando ele é elegível (gate + `depends_on` + condicionais + cartão + headroom). Composto no `aos-orq serve`; provado sob N processos por AOS-392 (o efeito nasce uma só vez, no dono) |
...
**O que ainda não é verdade, e não se deve ler como se fosse.** **Não há hoje uma implementação DURÁVEL da porta `backup.ImmutableStore` neste repositório (só a de referência, em memória), e o nó não inventa um destino.** O que antes tornava um destino durável *inutilizável* está fechado (AOS-101, retoma do manifesto — `packages/platform/backup/resume.go`): `NewExporter` **retoma** a cadeia do destino a partir de um registo de ciclo imutável (`<região>/cycle-%08d`), verificado fail-closed no arranque (assinatura, índice, região e elo recomputado do último registo, buraco na cadeia, e o segmento desse elo a abrir com a KEK deste processo — `ErrResumeUnverifiable`; destino não condicional — `ErrDestinationNotConditional`); um log atrás do cursor não impede o arranque e pára o laço no primeiro ciclo (`ErrSourceBehindBackup`), a ref do segmento passou a ser endereçada por conteúdo (um ciclo interrompido já não bloqueia o destino), e `Restorer.LoadManifest` reconstrói a cadeia para restauro sem manifesto guardado à parte. Medido em `reinicio_test.go`, `retoma_test.go` e `retoma_falha_fechada_test.go` e, sobre o nó, em `packages/cmd/aos/aos101_retoma_no_test.go`. Uma implementação durável tem de cumprir o contrato da porta que a retoma pressupõe (`Put` condicional que devolve `ErrImmutable` numa ref existente — em S3, `If-None-Match: *`, porque o Object Lock sozinho cria uma versão nova —, e `ErrNotFound` só para «não existe»; README do módulo). Até haver essa implementação, um destino e uma chave de assinatura compostos em produção, o backup imutável em produção continua a ser o `backup.sh` (cópia de volume, cron diário, **RPO de 24h**), e o `<= 1 min` é o RPO do exportador **para o destino que lhe for injectado**. O PITR tem alvo em `seq`, não em instante. O ensaio de restauro **não está agendado**. E as portas de produção (object storage WORM, KMS) continuam a ser portas — a KEK do backup passou a ser a **do nó** (`audit.KeyVault`), mas isso **não** a põe em custódia externa: a custódia Vault Transit do nó é *key-never-leaves* e com ela todos os ciclos falham (`crypto/aes: invalid key size 0`); só o vault de referência em memória funciona, e morre com o processo. Ligar o exportador em produção está **bloqueado pelo AOS-453** (DEK embrulhada pelo Vault Transit).
...
**PROC-DESPACHO-MULTIPROC — Topologia N-réplicas do despacho governado (AOS-392, v1.1 distribuído).** Procedimento operacional (não ligado a alerta) para correr `N× aos-orq serve --nats --nats-replicas 3`: atribuição de runs por-lease, arranque/paragem graciosa, recuperação da morte de uma réplica (takeover por expiração de TTL + re-hidratação), e o que observar (spans do laço de despacho e do sink; `vencedores=1` por run). Ancorado em ADR-023 (o lease arbitra) e ADR-024 (o efeito nasce no despacho, uma só vez, no dono). Doc em `docs/runbooks/PROC-DESPACHO-MULTIPROC.md`; prova em `packages/cmd/aos-orq/aos392_despacho_multiproc_test.go` (cluster-gated, como AOS-100).
