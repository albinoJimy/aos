...
**O que ainda não é verdade, e não se deve ler como se fosse.** O `platform/backup` **não está ligado a binário nenhum**: `Export()` é UM ciclo, sem laço nem agendador. O «contínuo» acima descreve a primitiva, não um comportamento em produção — o que corre no servidor é o `backup.sh` (cópia de volume, cron diário, RPO de 24h), e o RPO `<= 1 min` é uma propriedade da periodicidade configurada, não do que hoje corre. O PITR tem alvo em `seq`, não em instante. O ensaio de restauro **não está agendado**. E as portas de produção (object storage WORM, KMS) continuam a ser portas, com implementações de referência em memória.
