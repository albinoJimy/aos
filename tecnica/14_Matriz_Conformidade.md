...
| **Backup de volumes do servidor** (`deploy/server/backup.sh`, cron diário) — copia o `events.wal`/`worm.wal` **e** o volume do Vault (as KEKs por-titular) num bundle cifrado | GDPR Art. 17 | **Composto no servidor; reconciliado no arranque do nó (AOS-436)** | **Até AOS-436, um restauro DESFAZIA o crypto-shred:** o bundle traz as KEKs vivas no instante do backup, e restaurar um anterior a um apagamento repunha a KEK e o conteúdo voltava a decifrar — com o WORM actual o nó não re-verificava, e com o WORM também antigo nem sabia. **Desde AOS-436:** no arranque o nó une a cadeia DSAR, o seu registo de apagamentos (`AOS_DSAR_ERASURE_REGISTER`, só nomes `aos-kek-<sha256>`, copiado **em claro** para fora do bundle e recolhido para a máquina do operador) e um registo **importado** no restauro (`AOS_DSAR_ERASURE_REGISTER_IMPORT`), e destrói de novo — selando `dsar.key_reshredded` — cada KEK que o Vault restaurado tenha e que nasceu antes da destruição registada; por provar ⇒ `/readyz` 503. **Residual declarado:** (1) apagamentos entre a última recolha do registo e a perda do servidor voltam com o restauro; (2) a KEK re-provisionada no mesmo segundo da destruição é tratada como a destruída; (3) expirações por TTL anteriores a AOS-436 não estão em registo nenhum; (4) só a custódia Vault implementa a reconciliação. A refutação desta acusação em `docs/reports/varredura-adversarial-2026-08-21.md` §2 apoiou-se na linha anterior, que é sobre outro mecanismo — a correcção está registada no AOS-436 e o relatório datado fica como está. |
