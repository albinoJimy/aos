...
**Eixo declarado.** A cifra por titular do substrato (Event Store, memórias de longo prazo e backups) e a composição do `IngestPipeline` no nó ficam deferidas para os eixos de Model Gateway/Governação/Topologia (`specs/EPIC-06`, `specs/EPIC-09`, `specs/EPIC-10`), pelo mesmo tratamento dado à autoridade de identidade. A **superfície de administração de legal hold e retenção** (achado CON-02) e a **superfície de carregamento de bundle de política no binário** ficam deferidas para `specs/EPIC-09`. Enquanto não existirem, as linhas correspondentes desta matriz permanecem «Parcial», ou «Coberto com perna inerte» no caso do Art. 25 — não sobem por promessa de ticket.
