...
|---|---|---|---|---|
| **Art. 9** — Gestão de risco | Sistema de gestão de risco contínuo ao longo do ciclo de vida | Gates de risco SA-ROC (safe/gray/danger) e circuit breaker multi-sinal **compostos**; a taxonomia de autonomia L0–L5 (ADR-014, materializado em AOS-380) impõe o overlay de oversight `nível × classe` no PDP e a cerimónia de dual-control L4/L5 (AOS-305/AOS-377) — mas a **demoção automática em anomalia NÃO está composta**: o `autonomy.Controller` tem zero chamadores (`DEF-908`, aberto), pelo que um par promovido a L5 fica a L5 até intervenção humana. `DEF-908` é **pré-condição** desta perna do controlo, ainda por cumprir (ADR-013/014) | AOS-089, AOS-090, AOS-080, AOS-095 | Parcial |
...
|---|---|---|---|---|
| **Art. 5** — Minimização e limitação | Minimização, limitação da finalidade e da conservação | O Reference Monitor **no nó** impõe *fail-closed* a *obligation* `redact_pii` sobre os campos nomeados **quando o PDP a emite**; o nó arranca com PDP **sem bundle** (`pdp.NewUnloaded`, deny fail-closed) **por omissão**, mas o binário **EXPÕE** a via de carregamento: `AOS_POLICY_BUNDLE_DIR` + `AOS_POLICY_TRUST_ANCHOR` (`nodeConfigFromEnv` → `loadPolicyBundleFromEnv`, AOS-220, entregue em 2026-07-31; teste `aos220_pdp_bundle_surface_test.go`), pelo que esta redação entra em vigor **pelo binário entregue** assim que o operador as define. **Nota de precisão:** as obrigações `redact_pii`/`audit` são derivadas por pós-avaliação em Go (`engine_cedar.go`, `obligationsFor`) e **não** viajam no bundle — carregar *qualquer* bundle assinado acende esta perna, e alterá-las exige libertar binário, não trocar política. A limitação da conservação (TTL por classe) é hoje **composta no nó** (`audit.ExpirationJob`, conduzida por `POST /dsar/expire`, AOS-213; política de retenção vazia por omissão ⇒ nada expira, *fail-closed*). A redação/tokenização na ingestão continua a existir como **componente** mas **não está no caminho do nó** (ver §5.2) | AOS-091, AOS-092, AOS-213 | **Parcial** |
...
| **Art. 25** — Proteção de dados desde a conceção | *Data protection by design and by default* | PDP/PEP com allowlist *capability-scoped* **default-deny, activo no binário entregue** (sem bundle o PDP nega tudo — é a perna *by default*). **Ressalva de precisão (reconciliada por AOS-220):** as *obrigações* de redação/região impostas antes do efeito são impostas *fail-closed* pelo Reference Monitor **quando emitidas**; quem as emite é o motor de política, e o binário **EXPÕE** a via de carregamento do bundle (`AOS_POLICY_BUNDLE_DIR` + `AOS_POLICY_TRUST_ANCHOR`, `nodeConfigFromEnv` → `loadPolicyBundleFromEnv`, AOS-220) — pelo que a perna *by design* **cumpre o critério (c)**, exactamente como diz a ressalva 3 do §4 e a linha do Art. 5 (§4). Inerte por omissão é só a *emissão* de obrigações enquanto nenhum bundle for carregado, não a via de accionamento (ver §5.2). O registo de soberania board→região só é composto sob `Config.BoardRegions` / `AOS_BOARD_REGIONS`; vazio ⇒ read-path legado, sem authz por-chamador nem selo (ADR-011) | AOS-087, AOS-094 | Coberto (com perna inerte) |
...
3. A perna das **obrigações** (redação/região impostas antes do efeito) está composta mas **inerte no binário entregue**: o Reference Monitor impõe-as *fail-closed* quando as recebe, mas a superfície de configuração do binário **carrega** um bundle (`AOS_POLICY_BUNDLE_DIR` + `AOS_POLICY_TRUST_ANCHOR`, AOS-220). **Cumpre o critério (c).** Mantém-se «Coberto» por a perna *by default* passar os três critérios e o Art. 25 se satisfazer materialmente por ela; a perna inerte fica nomeada na célula e na §5.2 em vez de ser dissolvida no estado.
...
**Re-medição de 2026-09-06 (commit `2ca2d5c`, AOS-356-c).** O mesmo comando, repetido: `go list -deps ./...` a partir de `packages/cmd/aos` devolve agora **73 pacotes `github.com/aos-ref/*`** — vinte a mais do que a linha de base de Julho. A reverificação apanhou **uma célula errada**, e errada na direcção menos habitual: a do isolamento de execução declarava «Ausente do grafo» quando `substrate/sandbox`, `substrate/sandbox/seccomp`, `substrate/sandbox/network` e `platform/broker` **estão** no grafo — só `platform/attestation` não está. Uma célula que **subdeclara** o que o binário executa é tão nociva como uma que sobredeclara: esta escondia uma escolha de postura por omissão (ver a linha corrigida abaixo e `tecnica/17` §5.2-f).

**Limite desta re-medição, declarado e não escondido.** AOS-356-c corrigiu **uma** célula — a do isolamento/broker. As restantes **não** foram reavaliadas nesta emissão, e a medição do grafo contradiz pelo menos duas delas: `substrate/redaction` e `platform/backup` **aparecem hoje** nos 73 pacotes, contra o «Ausente do grafo» que as suas células declaram. Estar no grafo não basta para mudar a classificação — falta medir se o objecto é **construído** por código de produção, que é o critério (b) da §3 —, pelo que as células ficam como estão e a discrepância fica **registada como pendente de reavaliação**, sem ticket atribuído nesta emissão. É a mesma disciplina do resto do documento: declarar o que não foi medido em vez de o deixar passar por medido.

**Re-medição de 2026-09-08 (commit `167d0a4`, AOS-375).** Reconciliadas duas células que se contradiziam com o resto desta matriz sobre o carregamento de bundle: a linha «Carregamento de bundle de política no PDP» da tabela abaixo e a célula do Art. 25 (§4). Ambas afirmavam o carregamento de bundle inerte, negando que o binário exponha via de ambiente para o carregar e alegando que o nó executa incondicionalmente o PDP não-carregado — falso desde AOS-220 (entregue 2026-07-31, o dia seguinte ao último commit que tocou a alegação; ver o achado E-02 no fim desta secção). Evidência no binário, verificável sobre o commit acima: `packages/cmd/aos/main.go` chama `loadPolicyBundleFromEnv()` de dentro de `nodeConfigFromEnv` e preenche `cfg.PDP` (chamada e atribuição em `main.go:804`/`:808`, construtor em `main.go:1407`, fail-closed `ErrPolicyBundleNeedsTrustAnchor` em `main.go:101`); `packages/cmd/aos/bootstrap.go` regista no banner «PDP com BUNDLE CARREGADO (AOS_POLICY_BUNDLE_DIR)» (`bootstrap.go:2444`); teste de aceitação `packages/cmd/aos/aos220_pdp_bundle_surface_test.go`. *(A ficha do ticket AOS-375 aponta estas âncoras como `main.go:746`/`:750`/`:1288`/`:98` e `bootstrap.go:2378-2379`; os números deslizaram com a árvore — os símbolos e as linhas acima são os deste commit.)* A via de carregamento é accionável pelo binário entregue e **cumpre o critério (c)**; o que permanece inerte por omissão é apenas a **emissão** de obrigações enquanto nenhum bundle for carregado. É a mesma leitura que a §5.2 já dava nas notas «ENTREGUE por AOS-220» e as ressalvas 2–3 do §4, agora propagada às duas células que a contradiziam.
...
| Carregamento de bundle de política no PDP (`pdp.Open`, motor Cedar que **emite** `redact_pii`) | GDPR Art. 5, 25; AI Act Art. 10 | **Accionável (AOS-220); só a *emissão* é inerte por omissão** (o pacote está no grafo e é composto; `Config.PDP` é injectável em processo **e** o binário EXPÕE a via de carregamento pelo ambiente — `AOS_POLICY_BUNDLE_DIR` + `AOS_POLICY_TRUST_ANCHOR`, `nodeConfigFromEnv` → `loadPolicyBundleFromEnv`) | Por omissão (ambiente não definido) o nó compõe `pdp.NewUnloaded()` e, sem bundle, nenhuma `redact_pii` é emitida — a perna *default-deny* é real e activa, mas a **emissão** de *obligations* fica inerte até o operador carregar um bundle assinado. O que é inerte por omissão é a EMISSÃO, **não** a via de carregamento: esta existe no binário entregue (AOS-220, ver a nota «ENTREGUE por AOS-220» abaixo e as ressalvas 2–3 da §4) e o operador — não apenas um integrador que use o AOS como biblioteca — acciona-a definindo as duas variáveis. Dizer que a redação está «em vigor no nó» sem bundle seria incorrecto; o que está sempre em vigor é a **imposição** *fail-closed*, e a via de carregamento **cumpre o critério (c)**. |
...
| Cifra por titular do conteúdo dos runs no Event Store | GDPR Art. 17, 32 | **Composto (AOS-093/AOS-245), condicionado a `AOS_DURABLE_EXECUTION=1`** — um só cifrador por-titular (`newContentSealer` em `packages/cmd/aos/bootstrap.go`, sobre a MESMA custódia de KEK que o `/dsar/erase` destrói) partilhado pelo capturer de replay, pelo step-ledger e pelo registo de retoma | O que estes três persistem no Event Store — a resposta do modelo e os resultados de tools (`replay.captured`), o output de cada passo (`step.ledger.applied`, com `WithRequireTitular`: um passo sem titular é recusado em vez de escrever em claro) e o objectivo da retoma — é **cifrado sob a KEK do titular** antes de tocar o WAL, e o crypto-shredding torna-o ilegível sem mutar a hash-chain; desde AOS-429 também o objectivo da fila de planos. Sem execução durável o capturer não é composto e esse conteúdo **não é persistido**; o banner declara as duas posturas. **Correcção (AOS-436):** esta célula afirmava «Ausente» e «texto-claro», contradizendo o cifrador composto desde AOS-093 e o banner do nó. A correcção cobre as três vias nomeadas; **não** re-verificou a totalidade dos tipos de evento que o nó emite. |
| Job de expiração / retenção (`audit.ExpirationJob`) e legal hold como superfície administrável | GDPR Art. 5 (limitação da conservação), Art. 17 | **Composto? Sim (AOS-213)** (`Node.ExpirationJob` composto no nó, conduzido por `POST /dsar/expire`; legal hold administrável por `POST /dsar/hold`//`/dsar/release`) | O `ExpirationJob` varre os registos classificados do Event Store e expira os que cruzaram o TTL da sua classe (respeitando o legal hold) por **crypto-shred da KEK por-titular** de AOS-093 — apagamento real, provado ao nível do nó (`OpenContent`→`ErrDecrypt`+hash-chain valida). O operador coloca/levanta holds em runtime pelas rotas, autenticadas pela mesma credencial forte do `/dsar/erase` e seladas no WORM sem PII. **Granularidade:** por-titular (a KEK embrulha todas as DEKs do titular), não por-registo — residual nomeado (eixo AOS-093/envelope) em `DEF-903`. **TTL:** a política de retenção (`Config.Retention`, vazia por omissão ⇒ nada expira, *fail-closed*) é a superfície a preencher pelo operador. |
| **Fila de pedidos de plano** (`aos-internal/plan-requests`) — o **objectivo escrito pelo utilizador** | GDPR Art. 5, 17 | **Composto? Sim (AOS-429)**, com duas ressalvas de postura declaradas abaixo | O objectivo submetido por `POST /plans` é **texto livre de uma pessoa**. Até AOS-429 era persistido inline e **em claro** no facto `planrequest.submitted`, e dali ia para o WAL e para os backups, fora do alcance do crypto-shredding. Desde AOS-429 é **cifrado sob a KEK do titular** — o `principal` do submissor, verificado pelo gate soberano — pelo mesmo `audit.SealContent` que sela o conteúdo de captura/ledger, e o titular é ligado à partição no índice DSAR, o que o torna alcançável pelo legal hold e pelo `/dsar/erase`. **Ressalva 1 — sem gate soberano composto não há titular** e o objectivo fica em claro, como antes; é a postura do nó de referência, e em produção não ocorre (`AOS_MODE=production` exige o gate). **Ressalva 2 — o TTL é o do titular, não um mais curto**, e isso é deliberado: a granularidade do mecanismo é por-titular (uma KEK embrulha as DEKs todas), pelo que um TTL próprio para a fila destruiria a MESMA chave e com ela o conteúdo dos runs desse titular — um pedido de dez minutos apagaria runs de dez meses. **O evento em si NÃO é removido do log**, e não há operação que o remova: o contrato do Event Store não tem `Delete`/`Truncate`/`Purge`/`Compact`, por desenho — um log encadeado por hash de que se removessem entradas deixaria de ser tamper-evident. O apagamento efectivo é por ilegibilidade, não por remoção. |
...
| Isolamento microVM e credential broker JIT (`substrate/sandbox`, `platform/broker`, `platform/attestation`) | GDPR Art. 32; AI Act Art. 15 | **Composto? Não** para o broker (`platform/broker` **está** no grafo — `broker.NewVaultKVv2` tem chamador de produção — mas `broker.New` não, logo nenhum `*broker.Broker` é construído); **Composto? Sim, condicionalmente** para o sandbox (`substrate/sandbox`, `sandbox/seccomp` e `sandbox/network` estão no grafo e `NewLauncher`/`NewMediatedLauncher` têm chamador de produção, activado quando o catálogo `AOS_MODEL_TOOLS` traz uma tool com bloco `sandbox`); **Ausente do grafo** só para `platform/attestation` | O nó **compõe** o isolamento de execução — o que não compõe é a **intermediação JIT de segredos**: as credenciais downstream entram por ficheiro montado, são lidas uma vez no arranque e ficam em memória do processo, sem troca token-scoped, TTL curto nem revogação por lease (o banner de arranque declara-o; eixo `DEF-218`). **Esta correcção AGRAVA o quadro, e é essa a razão de estar aqui.** A célula errada («Ausente do grafo») sustentava a consequência confortável de que «o nó não aplica o isolamento por microVM» — se não o aplica, não há postura a discutir. Corrigida, a consequência passa a ser: o nó aplica **o isolamento que o operador escolher**, e até **AOS-344** a escolha por omissão era o driver de referência in-process, o único dos três que falha **aberto** (ver `tecnica/17` §5.2-f). A linha errada estava a esconder o achado. AOS-344 fechou o default em produção; a fronteira ao nível do kernel continua a depender de um executor host-side provisionado (**AOS-103**), e `platform/attestation` continua fora do grafo (**AOS-177**, `DEF-106`). |
...
| Cifra por titular de **backups** (`platform/backup`, que invoca `KeyVault.EnsureKey`) | GDPR Art. 17, 32 | **Ausente do grafo** | Backups produzidos fora do nó ficam fora do alcance do crypto-shredding: a destruição da KEK não os torna ilegíveis. É uma perna clássica do Art. 17 e não estava nomeada em versão anterior desta matriz. **Esta linha é sobre o exportador `platform/backup` (desligado por omissão), NÃO sobre o backup que corre em produção** — esse é a linha seguinte (AOS-436). |
| **Backup de volumes do servidor** (`deploy/server/backup.sh`, cron diário) — copia o `events.wal`/`worm.wal` **e** o volume do Vault (as KEKs por-titular) num bundle cifrado | GDPR Art. 17 | **Composto no servidor; reconciliado pelo nó no arranque e a cada minuto (AOS-436)** | **Até AOS-436, um restauro DESFAZIA o crypto-shred:** o bundle traz as KEKs vivas no instante do backup, e restaurar um anterior a um apagamento repunha a KEK e o conteúdo voltava a decifrar. **Desde AOS-436:** o nó une a cadeia DSAR (`dsar.key_destroyed`, a única autoridade), o seu registo de apagamentos (linhas autenticadas por HMAC sob uma chave que só viaja no bundle cifrado; copiado em claro para fora dele e recolhido para a máquina do operador) e um registo **importado** no restauro, e destrói de novo — selando `dsar.key_reshredded` — cada KEK que o Vault tenha e que nasceu antes da destruição registada, **salvo sob legal hold** (fica fechada). Enquanto a reconciliação estiver por provar, a custódia **recusa embrulhar e desembrulhar DEKs**: nenhum conteúdo por-titular se lê nem se escreve. **O que não cobre:** a cópia fora do bundle é do mesmo instante que ele, pelo que só acrescenta ao **restaurar um bundle anterior ao último**; os apagamentos posteriores ao último backup voltam com qualquer restauro. **Residual declarado:** (1) essa janela; (2) KEK re-provisionada no mesmo segundo da destruição tratada como a destruída; (3) relógio do Vault adiantado pode fazer uma KEK ressuscitada parecer nova; (4) só a custódia Vault implementa a reconciliação; (5) expirações por TTL anteriores a AOS-436 fora do registo; (6) o corte do fim de um registo não se distingue de um registo mais antigo; (7) quem lê e escreve o volume de dados destrói a KEK viva de qualquer titular (a chave do registo vive lá); (8) um Vault restaurado com o nó a correr deixa a KEK ressuscitada servir até ao tick seguinte (≤ 1 min). A refutação desta acusação em `docs/reports/varredura-adversarial-2026-08-21.md` §2 apoiou-se na linha anterior, que é sobre outro mecanismo — a correcção está registada no AOS-436 e o relatório datado fica como está. |
...
**Excepção introduzida por AOS-429, e é a primeira.** O parágrafo seguinte foi escrito quando
nenhuma superfície do nó cifrava PII sob a KEK do titular. Deixou de ser inteiramente verdade: a
**fila de pedidos de plano** passou a fazê-lo (linha própria na tabela acima), e sobre ela o
`dsar.key_destroyed` tem efeito verificável — há teste que destrói a KEK e exige que o objectivo
deixe de abrir. A leitura abaixo continua válida para tudo o resto, e a excepção é nomeada aqui
em vez de se deixar o parágrafo a afirmar o que já não afirma.
...
**Legal hold e retenção (CON-02) — ENTREGUE por AOS-213** (`DEF-903` fechado como `FECHADO-RESIDUAL` no registo de deferimentos). O gatilho da Opção C do dono (o apagamento tornar-se real) cumpriu-se com AOS-093, pelo que a superfície de administração foi construída: rotas `POST /dsar/hold`//`/dsar/release` (legal hold administrável) e `POST /dsar/expire` (`audit.ExpirationJob` composto no nó, expiração por crypto-shred da KEK por-titular respeitando o hold). Residual nomeado com eixo (granularidade por-titular vs por-registo) em `DEF-903`.
**Superfície de carregamento de bundle de política — ENTREGUE por AOS-220** (2026-07-31). Deixou de haver eixo por atribuir: o binário expõe `AOS_POLICY_BUNDLE_DIR` + `AOS_POLICY_TRUST_ANCHOR` (`nodeConfigFromEnv` → `loadPolicyBundleFromEnv`), com o trust anchor forçado out-of-band e recusa fail-closed de bundle ausente, adulterado ou assinado por outra chave; provado por `packages/cmd/aos/aos220_pdp_bundle_surface_test.go`.

Esta matriz afirmou o contrário durante **453 commits** — que o binário «não expõe variável de ambiente que carregue uma política» —, tendo o controlo entrado **no dia seguinte** ao último commit que lhe tocou. A deriva era CONSERVADORA (subdeclarava cobertura, não a inflacionava), e é por isso que atravessou revisões: um documento de conformidade que se engana **contra si próprio** não dispara alarme nenhum. Quebra na mesma a promessa de reverificação a cada emissão que a §7 faz. Achado E-02 de `analises/10`.
...
| Mecanismo composto mas inerte | Um mecanismo está no binário e é composto, mas nenhuma via de configuração o acciona — passa uma verificação de alcançabilidade e continua sem produzir efeito. *(Exemplo já reconciliado por AOS-220: a via de carregamento do bundle PDP é hoje accionável; o que resta inerte por omissão é só a **emissão** de obrigações enquanto nenhum bundle for carregado.)* | Critério (c) da §3 (inspeção de `nodeConfigFromEnv` em `packages/cmd/aos/main.go`); estado da linha «Carregamento de bundle» na tabela da §5.2 (accionável, emissão inerte por omissão); ressalva de precisão na célula do Art. 25 |
