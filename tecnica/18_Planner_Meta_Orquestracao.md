...
|---|---|
| qualquer nó lê qualquer coisa | um nó só obtém o que **declarou** consumir; não existe operação que devolva «tudo o que há» — o conjunto legível é o contrato, fixado na admissão. *(A projecção desse contrato para o `PlanCard` do gate ainda **não existe** — ver o limite declarado no fim desta secção.)* |
...
**Versão de schema.** `outputs`/`consumes` são **opcionais e aditivos**: um documento sem eles decodifica e comporta-se como antes, pelo que a extensão **não** consome um MAJOR (§3.6). O bump de MINOR e a migração que agregam as três extensões de ADR-022 são AOS-273.

**LIMITE DECLARADO — o gate ainda não vê as extensões.** O invariante §2.4(5) do ADR-022 («o humano no gate vê o organigrama **com** as condições e os verificadores declarados») **não tem implementação**: `planapproval.PlanNode`/`PlanCard` (`packages/control-plane/governance/plan-approval`) não projectam `role`, `conditional_on`, `outputs` nem `consumes` — um plano com verificador, ramo de qualidade e arestas de dados chega ao revisor como uma lista de nós e arestas de precedência. Enquanto essa projecção não existir, o cartão **descreve menos** do que a execução, e as três extensões apoiam-se apenas nas defesas de admissão (§3.3.1–§3.3.3) e no clamp de materialização. Eixo: **AOS-238** (o mesmo *wiring* a jusante); registado em `docs/governance/REGISTO-Deferimentos.md` (**DEF-274**).
