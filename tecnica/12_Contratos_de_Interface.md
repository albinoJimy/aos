...
**Estado medido (auditoria multiagente v4, achado DAT-09) e reconciliação (AOS-382).** Os dois contratos que o código rastreia explicitamente a este documento — C1 e C2 — estão fiéis: os seis códigos de porta de §4 e §5 existem como constantes em `packages/control-plane/pdp/errors.go` e `packages/substrate/eventstore/errors.go`. Os três **sem** rastreio — C3, C4, C5 — tinham divergido integralmente (o registry usava um espaço de nomes próprio `E_REG_*`/`E_SIG_*`/`E_TOFU_*`/`E_DIGEST_*`, e o broker/gateway usavam erros sentinela sem código de porta). Essa divergência foi **reconciliada** por AOS-382, e não perdoada: C5 passou a citar os nomes **reais** do código (`E_DIGEST_MISMATCH`, `E_SIG_INVALID`, `E_TOFU_SCHEMA_DRIFT`, `E_REG_UNPINNED_RESOLUTION` + `E_TOFU_UNPINNED_VERSION`), coerentes com `tecnica/05`; C3 (broker) e C4 (gateway) ganharam, em `port_codes.go`, as constantes de código de porta com os nomes documentados **e** uma tradução opt-in (`PortErrorCode`, via `errors.Is`/`errors.As`) que mapeia os erros sentinela para esses códigos — o chamador do contrato passa a ter um código estável por onde distinguir as condições, sem que a semântica de nenhum erro existente mude (a recusa continua a devolver o sentinela; a tradução é do lado de quem consome a porta); e a condição de rate-limit saiu da porta C4 por ser do scheduler (ver §7). As dez entradas de `scripts/ci/baseline/contract-codes.txt` ficaram assim **obsoletas** e foram removidas — a baseline continua a só encolher. O gate 4 imprime, por contrato, quantos códigos extraiu deste documento — 3/3/3/2/5 para C1…C5 — e quantos estão presentes no código, para que uma quebra de parse (ou um verde por não olhar) fique visível no log em vez de reduzir o gate em silêncio.
...
**Semântica de erro.** `E_REGION_DENIED` se o modelo pedido não estiver na allowlist regional (soberania — failover proibido de cruzar fronteira, ADR-011); `E_MODEL_UNAVAILABLE` quando não há adaptador/endpoint conhecido para o provider, com sugestão de fallback de classe. O `model_id` concreto é sempre devolvido para replay fiel.

**Admissão global (rate-limiting).** O `retry_after` por recusa de headroom é decidido pelo **admission control global**, que vive no *scheduler* do control-plane (ADR-008) e NÃO nesta porta: o gateway não devolve um erro de porta C4 de rate-limit. Por isso a condição de rate-limit não é um código desta porta e não é exigida ao pacote do model-gateway — é responsabilidade do scheduler.
...
**Semântica de erro.** `E_REG_UNPINNED_RESOLUTION` (resolução sem versão pinada, na porta do REG) e `E_TOFU_UNPINNED_VERSION` (observação TOFU sem versão pinada exacta) se a referência não trouxer versão fixa — resolução flutuante proibida; `E_DIGEST_MISMATCH` se o digest não corresponder ao registado (rug-pull detectado — falha fechado); `E_SIG_INVALID` se a assinatura não verificar; `E_TOFU_SCHEMA_DRIFT` (mudança de schema em estado pinado) exige re-aprovação com nova versão SemVer antes de uso. Uma definição não verificada nunca é devolvida como utilizável.
