...
A disciplina de SemVer é imposta por um **gate de validação de bump** determinista (pacote `registry/semver`), não por convenção. Dado o contrato público **antigo** e o **novo** (schema de I/O + scopes de credencial + classe de egress + **digest do manifesto**, este último só nos `mcp_server`), o classificador `ClassifyContract` deduz a mudança **mínima** exigida, ancorada a cada eixo do contrato:

- **MAJOR (quebra)** — schema de *input* com propriedade removida, tornada obrigatória ou de tipo apertado/incompatível; schema de *output* com propriedade removida ou garantia enfraquecida; scopes de credencial **acrescentados**; classe de egress **elevada** (`none` < `internal` < `external`); **`manifest_digest` alterado** (AOS-335 — ver abaixo); ou semântica **declarada** quebrada (sinal não-estrutural que o publicador afirma). É a única classe que exige re-aprovação e justifica um novo estado de confiança TOFU (cruza AOS-049).
...
- **PATCH / none** — contrato byte-idêntico (mesmo schema/scopes/egress **e mesmo `manifest_digest`**).

**Que artefactos passam a exigir MAJOR, e porquê (AOS-335).** Todos os artefactos que **transportam âncora de manifesto**, e só quando ela muda. Isso são os **`mcp_server`** e — o que é a maior parte do custo real — as entradas **`kind=tool` derivadas de um servidor MCP**: o `mcp.Host` grava a **mesma** âncora em cada tool que expõe, pelo que mover o endpoint de um servidor com N tools exige bump MAJOR nas N. As tools e skills genéricas, que não vêm de um servidor MCP, têm o campo vazio dos dois lados e não se movem. *(Correcção: a primeira redacção deste parágrafo dizia «só os `mcp_server`» e omitia o fan-out às tools — a maior parte do custo que ela devia declarar.)* A regra é MAJOR e não uma classe mais fina porque o digest é uma **âncora opaca**: o `mcp.DigestAncorado` funde a superfície declarada do servidor com o endpoint onde ele atende, e o resultado **não é decomponível** — daqui não se distingue «o servidor mudou de porta» de «o servidor ganhou uma tool `exec`». Perante duas leituras possíveis e nenhuma forma de as separar, classifica-se pela pior. É também a leitura certa para o que o campo significa: o digest é o que afirma que o servidor por trás deste contrato é o **mesmo** que foi aprovado; mudou, não é. **Custo declarado:** uma mudança puramente operacional de endpoint, sem alteração de superfície, passa a exigir bump MAJOR. Separar os dois eixos exige decompor a âncora — trabalho no `DigestAncorado`, não no classificador — e fica nomeado em vez de resolvido por uma heurística que adivinhasse qual dos dois mudou. **Antes disto**, um `mcp_server` cujo manifesto trocasse por inteiro classificava `none` e passava como `1.0.0 → 1.0.1`: a omissão não deixava a classificação indefinida, **degradava-a** para «compatível».
