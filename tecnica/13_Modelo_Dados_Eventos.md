...
#### Verificação (enquanto não há gate automático)
...
Três verificações complementares, todas manuais hoje: que nenhum `eventstore.EventInput{Type: "…"}` usa uma literal **nem uma concatenação**; que o primeiro segmento de cada nome consta de uma das duas tabelas acima; e que um nome catalogado em (a) é mesmo apendado ao Event Store (o pacote importa `substrate/eventstore`). A automatização num gate de CI **não é parte desta revisão documental** — é o que falta para satisfazer o CA3 de AOS-201, e fica registada como pendência aberta (§8.1).
...
|---|---|---|
| **Gate de CI do catálogo de tipos — CA3 de AOS-201, ABERTO.** Deve verificar: constante nomeada declarada junto do emissor; identificador contém `Event` e declaração numa linha (§3.3, «Regras»); prefixo conhecido; **zero literais e zero concatenações** em `EventInput.Type` (o padrão a rejeitar está em `control/steer_channel.go`, ramo `default` de `eventTypeFor`); e separação das famílias (a)/(b) pela importação de `substrate/eventstore` | `scripts/`, pipeline de CI | Requer alterar `scripts/**` e a CI; esta revisão só toca documentação. Os comandos reproduzíveis de §3.3 (GNU e PowerShell) são o substituto manual entretanto — **não satisfazem o CA3**, que exige script ou gate. |
...
| Marcar as *checkboxes* de critérios de aceitação de AOS-201 | `specs/EPIC-18_Remediacao_Auditoria_Multiagente_v4.md` | Ficheiro de outro dono nesta revisão. Estado a registar: CA1 e CA2 satisfeitos por §3.1–§3.4; **CA3 por satisfazer** (ver a primeira linha desta tabela). |
