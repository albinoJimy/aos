...
- **Selo de governação por chamada (AOS-058, AOS-265, AOS-394)** — cada decisão fica num registo `audit.AuditRecord` na partição `modelgw-gov:<board>` do WORM de governação do GW (durável com `AOS_MODEL_AUDIT_PATH` no nó). São três os sítios que o escrevem: o estágio `allowlist-regional` (allow e deny), o deny de failover cross-border e a troca de modelo pelo refino. Cada registo leva:
  ...
  **Decomposição do planeador no `aos-orq` (AOS-395).** O mesmo mecanismo cobre as chamadas do `--goal`. O `planner.runAttempt` anexa ao ctx o run e o passo `planstep:decompose:<tentativa>` antes de chamar o decompositor, e o `gatewayDecomposeModel` passa-os ao `port.ChatRequest`. Com `AOS_MODEL_AUDIT_PATH`, os selos vão para um WORM em ficheiro que sobrevive ao fim do processo; sem ela, o arranque declara a postura volátil. O caminho só se abre quando a decomposição vai pelo gateway, e o seu directório tem de existir. O processo toma a posse exclusiva de escrita do caminho ao SO antes de o abrir, e um segundo escritor sai com o código 5. O caminho tem de ser distinto do `model-audit.wal` do nó, que pede a mesma posse desde o AOS-399. A tentativa recomeça em 1 em cada decomposição, pelo que um segundo `--goal` sobre o mesmo run sela outra vez `planstep:decompose:1`: tal como o passo do turno, o par não é chave única.
...
> **Ponto de consumo — assembler/runtime, não o estágio do GW (integração POR FAZER).** A guarda de layout **NÃO está composta em lado nenhum** — o desenho previa que o assembler/runtime compusesse `cache/freeze` (`RunPrefix.Turn`) → `cache/layout` (`Guard.Admit`) por turno, validando byte-a-byte antes de a montagem seguir para o GW. **CORRECÇÃO (AOS-325): essa composição NÃO existe** — medido, `cache/freeze` não tem importador não-teste fora de si próprio e `layout.Guard.Admit` não tem chamador de produção. O CA é cumprido pelo `PromptAssembler` do agent-runtime, que produz um prefixo byte-estável por construção e expõe o seu `PrefixHash`; os três subpacotes de cache do GW existem, estão testados e não são compostos por ninguém. O estágio `cache-layout` da pipeline do Gateway **mantém-se pass-through por desenho**: o `Exchange` da pipeline transporta identidade/modelo/região/custo, **não** a `PromptView`/runID/turno, pelo que a validação byte-a-byte não é (nem precisa de ser) alcançável pela fachada do GW. Não há `WithCacheLayoutStage` — ao contrário de auth/allowlist/roteamento, a validação de layout não é um estágio do data-plane do GW mas uma guarda da montagem a montante. Ligar a guarda ao `Exchange` (transportando a `PromptView`) fica para um ticket futuro se a validação passar a ser exigida também no gate do GW; até lá, o CA "o GW rejeita/sinaliza montagens que reordenem o prefixo" é cumprido pelo **chamador da montagem** (runtime/assembler), não pelo estágio pass-through.
...
- **Agregação por run e por tenant (`Recorder`, `Key{RunID, Tenant}`).** Um agregador concorrente-seguro acumula `(read, prompt)` por chave e expõe o rate agregado como SLI (`RateFor`/`Snapshot`). O **tenant** reutiliza o *board*/humano responsável de AOS-057. Runs/tenants distintos são **isolados** — chaves distintas nunca se contaminam. Uma chamada indefinida (prompt 0) é omitida da agregação (não conta como 0% que envenenaria o sinal). **Retenção (AOS-397):** o agregador retém no máximo `DefaultRetainedKeys` (4096) chaves, com despejo do menos-recentemente-usado (`WithRetention` muda o tecto; não há opção sem tecto). Um run despejado que volte a ser observado recomeça do zero, incluindo o estado anti-flapping, pelo que o alerta pode voltar a disparar para ele.
...
- **Agregação por run e por árvore (`cost`, `Recorder`).** Um agregador concorrente-seguro acumula custo+tokens por `RunKey{RunID, Tenant}` **e** por `TreeKey{TreeID, Tenant}` — o eixo **árvore** é o do burn-down/admission **GLOBAL** (ADR-008, EPIC-03). Runs/árvores distintos são **isolados** (chaves distintas nunca se contaminam). O custo agregado é legível (`CostForRun`/`CostForTree`) e entregue por uma porta `BurndownSink` (impl de referência `MemoryBurndownSink`; o admission de EPIC-03 liga o real). Este ticket produz a **MEDIDA**; o *enforcement* é EPIC-03. **Retenção (AOS-397):** cada eixo retém no máximo `DefaultRetainedKeys` (4096) chaves, com despejo do menos-recentemente-usado (`WithRetention` muda o tecto; não há opção sem tecto). Antes eram mapas sem remoção, e com o run real a chegar a cada chamada (AOS-394) um nó de vida longa guardava uma entrada por run para sempre. Um run activo é tocado a cada chamada e só perde o cumulativo depois de 4096 outros runs serem observados; um run despejado recomeça do zero. Nenhum consumidor composto lê hoje o cumulativo: o nó constrói o recorder sem sinks e sem tracer, e o burn-down de AOS-261 soma o custo **por chamada** do `turn.recorded`. O `MemoryBurndownSink` de referência não tem tecto (guarda todos os incrementos para os testes) e não está composto em produção.
