...
> **Implementação (AOS-031).** O **executor** da degradação graciosa vive em `packages/control-plane/scheduler` (`degradation.go`, `Degrader`). A política de AOS-030 (`policy.go`) **SELECCIONA** a acção quando uma fila satura; o `Degrader` **EXECUTA-a** na ORDEM DE PREFERÊNCIA canónica (ADR-008) **shed → defer → downgrade → reject**. **Compõe** — não reimplementa — a política (AOS-030) nem o admission (AOS-027); o *downgrade* encaminha para um tier mais barato via a **porta** `ModelTierRouter` (impl de referência `StaticModelTierRouter`, à imagem do `QuotaProvider` de AOS-027) — o **Model Gateway (EPIC-06) NÃO é implementado aqui**. Cada degrau tem semântica precisa: **SHED** descarta trabalho opcional/baixa prioridade com **razão** (`work_shed`), mas trabalho **crítico** NUNCA é descartado — um shed de crítico é `ErrCannotShedCritical` **fail-closed** (sem evento, sem efeito); **DEFER** adia trabalho admissível com `retry_after`, PRESERVANDO-o pela porta `DeferSink` (integra AOS-027/030 — não perde o trabalho; erro do sink propaga, fail-closed) (`work_deferred`); **DOWNGRADE** encaminha para o tier mais barato via `ModelTierRouter.Cheaper`, registando o swap como **VARIÂNCIA EXPLÍCITA** (`tier_antigo→tier_novo`, `model_downgraded`) — NUNCA silencioso; a variância entra no log para o **replay ser fiel** (ADR-010/AOS-016) e o downgrade fica **reversível**; **REJECT** recusa como último recurso com erro **claro e accionável** (`ErrWorkRejected`, `work_rejected`), **fail-closed para irreversíveis** (o efeito irreversível não ocorre e `Normalize` nunca ressuscita trabalho rejeitado). A **ordem é configurável** por classe/tenant (`ExecuteChain` com `DefaultPreferenceOrder` ou uma ordem própria; um crítico salta o shed e é adiado); a **escalada por pressão crescente** é conduzida pela política de AOS-030 e executada com `Execute`. A **REVERSIBILIDADE** ao normalizar a carga (`Normalize`, ex.: `backpressure_cleared`) reverte os downgrades reversíveis — restaura o tier (`tier_restored`, `tier_novo→tier_antigo`, por ordem de ID, idempotente; cascata restaura o tier ORIGINAL) — mantendo a variância `model_downgraded` no log (ADR-010). Shed/reject NÃO são reversíveis (documentado). Os **eventos append-only** (`work_shed`/`work_deferred`/`model_downgraded`/`work_rejected`/`tier_restored`), cada um com **gatilho + acção + efeito**, reconstroem-se por `ReplayDegradation` (determinismo: relógio/IDs injectáveis, iteração ordenada, serialização estável). Span OTel `degradation_execute` por acção, via a porta `agentruntime.Tracer` zero-dep. **Fora** deste âmbito: o Model Gateway (EPIC-06 — porta), o scheduling priority-aware (AOS-032), o roteamento least-loaded (AOS-033) e as métricas de saturação (AOS-034).

> **Implementação (AOS-030).** O **backpressure real** — filas limitadas + motor de política declarativa — vive em `packages/control-plane/scheduler` (`queue.go` + `policy.go`). As **filas limitadas por partição** `tenant:priority` (`PartitionedQueues`) têm **limite EXPLÍCITO** de **comprimento** (`MaxLen`, tecto duro que a fila **nunca** ultrapassa) **e/ou idade** (`MaxAge` do item mais antigo): ao atingir o limite aplica-se a **política** em vez de crescer — **sem acumulação ilimitada** (a profundidade nunca excede `MaxLen`, provado sob carga). O enchimento é **detectado e sinalizado a montante**, nunca silenciosamente absorvido. **Watermarks high/low** dão **histerese**: uma partição satura ao cruzar o high e só **sai** de saturado ao descer **até/abaixo** do low — o estado latched entre os dois evita *flapping*. O **motor de política DECLARATIVA** (`PolicyEngine`) carrega um artefacto **JSON** (stdlib, **zero deps** — o ticket dá "YAML/Rego" como exemplo, não obrigação; JSON evita o dep-tree da OPA) que mapeia condições de saturação (`priority` + `min_fill_ratio` + `min_age_ms`) → **acção nominal**; a selecção é **determinística** (primeira regra que casa por ordem de declaração, senão `default_action`) e emite `degradation_policy_selected`. **AQUI só SELECCIONA** — a **execução** shed/defer/downgrade/reject é o AOS-031. O **hot-reload é atómico e versionado** (SemVer, à imagem do bundle versionado do PDP/AOS-004): `Reload` troca o motor de decisão via `atomic.Pointer` — as filas em curso **não** são tocadas, logo **nenhum trabalho se perde**; a nova versão tem de ser **estritamente mais recente** (SemVer monótono) e cada troca regista o **changelog** (versão-antiga→nova) no **audit trail** (evento append-only `policy_reloaded`). A **validação é fail-closed**: JSON malformado, acção desconhecida, SemVer inválido ou versão não-monótona são **rejeitados** e mantém-se a política anterior (nunca se cai numa política vazia/permissiva). O **acoplamento ao admit (AOS-027) é ADITIVO**: `PartitionedQueues` implementa `BackpressureSource`, injectada via `Admission` + **`WithBackpressure(source)`**; sob saturação da fila do tenant, o admit passa a **ADIAR (defer)** mais agressivamente mesmo havendo headroom — o sinal propaga-se a montante. **Sem** a opção, o admit é bit-a-bit o do AOS-027 (os seus testes **não** mudam de comportamento — verificado). Prova concreta: fila saturada ⇒ mais defers no admit. Os **eventos append-only observáveis** (`queue_saturated`/`backpressure_signalled`/`backpressure_cleared`/`degradation_policy_selected` + changelog `policy_reloaded`) reconstroem o estado de fila e a linhagem de versões por replay (`ReplayQueue`/`ReplayVersions`); determinismo: relógio/IDs injectáveis (idade de fila usa relógio injectável), iteração de partições ordenada, serialização estável. Spans OTel `backpressure_enqueue`/`backpressure_policy_select` (profundidade/idade de fila) via a porta `agentruntime.Tracer` zero-dep. **Fora** deste âmbito: a **execução** das acções (AOS-031), a degradação graciosa (AOS-031), o scheduling priority-aware (AOS-032), o roteamento (AOS-033) e as métricas de saturação (AOS-034); **não** reescreve o admission (só acoplamento aditivo).
...
> **Implementação (AOS-032).** O **scheduling priority-aware + aging** vive em `packages/control-plane/scheduler` (`priority.go`, `Dispatcher`). **Compõe** — não reimplementa — as filas particionadas (AOS-030, `PartitionedQueues`: bounding + backpressure) e o admission control global (AOS-027, `Admission` pela porta `AdmissionGate`): o `Dispatcher` **ORDENA** sobre as filas por **prioridade efectiva** e **SÓ despacha trabalho ADMITIDO** — serve a MAIOR prioridade efectiva **admissível**. A **prioridade efectiva = f(base, idade, SLO)** é **MONÓTONA na idade** e calculada em **aritmética inteira** (sem float, sem relógio embutido — replay byte-a-byte): `eff = Base + AgingStep·(idade/AgingInterval) + SLOWeight·(idade/SLO)`. Como o termo de **aging** cresce **sem tecto**, qualquer classe baixa acaba por ultrapassar trabalho **NOVO** de classe alta — a garantia **ZERO starvation** (provada num cenário adversarial de fluxo P0 contínuo, `TestDispatch_NoStarvationAdversarial`). A decisão é **LATENCY-AWARE**: o **SLO** da tarefa entra na prioridade efectiva (fracção do orçamento de latência consumida), pelo que a igual classe e idade a tarefa de SLO mais apertado é servida primeiro — a decisão não depende só da prioridade nominal. A **ordem é determinística e reproduzível em replay**: a selecção ordena um **slice** (NUNCA ordem de mapa Go) com **tie-break estável** (prioridade efectiva desc → timestamp de entrada asc → `task_id` asc, um total order porque o `task_id` é único), o relógio da idade é **injectável** (sem `time.Now` na decisão) e a serialização é estável (structs) — mesmos inputs ⇒ MESMOS bytes de evento na mesma ordem (`TestDispatch_ReplayByteForByte`). Os **parâmetros de aging** são **configuráveis por classe e por (tenant, classe)** — resolução por especificidade (`AgingParams` + `WithClassAging`/`WithTenantClassAging`, nunca constantes). Os **eventos append-only** (`scheduling.task_scheduled` com prioridade efectiva/idade; `scheduling.priority_aged` na promoção por aging) reconstroem a ordem de despacho por `ReplaySchedule`. Span OTel `priority_dispatch` (tempo de espera por classe) via a porta `agentruntime.Tracer` zero-dep. **Fora** deste âmbito: o roteamento least-loaded/token-aware (AOS-033) e as métricas de saturação/headroom (AOS-034); **não** reimplementa as filas (AOS-030) nem o admission (AOS-027).

> **Implementação (AOS-033).** O **roteamento least-loaded/token-aware** vive em `packages/control-plane/scheduler` (`routing.go`, `Router`). Substitui o **round-robin CEGO** por uma escolha do destino (worker/tier) **MENOS carregado** em função de **carga real E consumo de tokens** — nunca rotação por índice. **Compõe** — não reimplementa — a **carga** (porta `LoadSource`: `Destinations`/`Load`, com sinais `QueueDepth`/`TokensInFlight`/`RecentLatencyMs`/`CapacityTokens`; impl de referência determinística `StaticLoadSource`, à imagem do `QuotaProvider` de AOS-027), o **admission control global** (AOS-027, porta `AdmissionGate`) e o **cost-aware tiering** (AOS-031, **a mesma** porta `ModelTierRouter`/escada `StaticModelTierRouter`) — o **Model Gateway (EPIC-06)** e a **frota (EPIC-10) NÃO são implementados aqui** (entram como portas). A **selecção least-loaded** minimiza um **custo INTEIRO** sobre a carga (`DefaultRouteWeights`: `Queue·depth + Token·in_flight + Latency·ms`); a porta OPCIONAL `LoadReporter` **reflecte** o trabalho encaminhado (tokens em voo), pelo que decisões sucessivas **equalizam** a pressão — o que distingue o least-loaded do round-robin. A **prova anti-hotspot** é concreta: sob **carga heterogénea**, o least-loaded tem **menor max-load E menor variância** que um **round-robin de referência** (implementado só no teste) no MESMO workload (`TestRoute_BeatsRoundRobin_HeterogeneousLoad`). O **token-awareness** tem duas camadas coerentes: (1) headroom **LOCAL** por destino (`CapacityTokens − TokensInFlight`) — um destino sem margem para o custo estimado é **PRETERIDO** (`TestRoute_TokenAware_SkipsNoHeadroom`); (2) integração com AOS-027 — o router **reserva** no destino escolhido pelo `Admit` e um destino sem headroom **GLOBAL** é preterido (`TestRoute_AdmissionHeadroom_SkipsGloballySaturated`), reutilizando o `Admit`/`Release` (a reserva é **libertada** se um erro ocorrer após a concessão — sem fuga de headroom). O **cost-aware model tiering** é **coerente com AOS-031**: quando nenhum destino do tier corrente tem margem e o trabalho é **elegível** (`TierEligible`), o router **desce a escada** pela **mesma** `ModelTierRouter.Cheaper` que o downgrade de AOS-031 usa, registando a **variância explícita** de tier (`premium→economy`, `TestRoute_CostAwareTiering_DescendsWhenTierSaturated`) — trabalho **não-elegível** fica no tier e **adia** em vez de descer em silêncio. As **decisões são observáveis e replayáveis**: cada decisão é um **evento append-only** `routing.work_routed` (destino escolhido + **carga no momento** + custo + custo estimado + variância de tier); um adiamento por ausência de destino com margem é `routing.work_route_deferred` (**observável, NUNCA silencioso**, `TestRoute_TokenAware_DefersWhenAllFull`). Determinismo/replay (ADR-001/010): os destinos são iterados por **id** (nunca ordem de mapa Go), o **tie-break é estável** (custo asc → id asc, total order porque o id é único), o relógio (carimbos) e os decision IDs são **injectáveis** (sem `time.Now`/`rand` na decisão) e a serialização é estável (structs) — **mesmos sinais ⇒ mesmo destino e MESMOS bytes de evento** (`TestRoute_DeterministicReplay_SameSignalsSameDecisions`); `ReplayRouting` reconstrói a sequência. Span OTel `routing_decision` (destino/carga/custo) via a porta `agentruntime.Tracer` zero-dep. **Fora** deste âmbito: as **métricas de saturação/reserva de headroom** (AOS-034 — o último), o **Model Gateway** (EPIC-06 — a porta `LoadSource`/`ModelTierRouter`) e a **frota** (EPIC-10 — a topologia); **não** reimplementa o admission (AOS-027), o tiering (AOS-031) nem o Event Store (AOS-002).

> **Nota cruzada (EPIC-06 — Model Gateway).** As portas `LoadSource` (estado de carga por destino: filas, tokens em voo, latência) e `ModelTierRouter` (escada de tiers cost-aware) são o **ponto de sutura** com o Model Gateway: em produção, o GW implementa-as sobre o TPM/RPM e a telemetria reais de cada worker/tier (à imagem de como implementa o `QuotaProvider` de AOS-027), e a impl de referência (`StaticLoadSource`/`StaticModelTierRouter`) fecha o contrato entretanto. O `Router` consome-as **sem** conhecer o GW — a fronteira mantém-se limpa (o roteamento é do plano de controlo; a leitura de carga/tier real é do EPIC-06).

> **Implementação (AOS-034 — o ÚLTIMO do EPIC-03).** As **métricas de saturação e reserva de headroom** vivem em `packages/control-plane/scheduler` (`metrics.go` + `slo.go`) e **fecham o ciclo de controlo** do epic: instrumentam o plano de controlo em OTel e ligam-no ao pilar de métricas do EPIC-08. **PORTA `Meter` zero-dep** — à imagem da `agentruntime.Tracer` (spans OTel GenAI sem SDK), define-se uma porta própria com `Counter`/`Gauge`/`Histogram` (nome + atributos estáveis + valor); `NoopMeter` é o default e `RecordingMeter` capta tudo para teste **e** é o substrato dos wide events. O OTel SDK real é EPIC-08 — os **nomes/atributos são OTel-estáveis** (o adaptador mapeia sem renomear). A instrumentação é **ADITIVA**: as fontes recebem opções `WithAdmissionMeter`/`WithDegradationMeter`/`WithSpawnMeter` (counters event-driven — defer-rate, taxa de degradação por acção, spawns adiados) e a amostragem (`SampleHeadroom`/`SampleQueues`, à imagem dos *observable gauges* OTel) lê o headroom/filas **REAIS** pela **mesma** projecção determinística que o `Admit` decide (`Admission.Headroom`) e pelo `PartitionedQueues.Snapshot` (read-only) — **sem** esta instrumentação, os AOS-027..033 são bit-a-bit os seus (nenhum teste muda). As **métricas de saturação** (`aos.scheduler.queue.depth`/`oldest_age_ms`/`saturated`, `admission.deferred` com `defer_reason`, `degradation.actions` por `action`) e de **headroom** (`headroom.free_tokens`/`free_requests`/`reserved_tokens`/`utilization` por `provider:model:region` e tenant, `spawn.deferred`) têm **custo/dimensões sem segredos** (só provider/model/region/tenant/partition/acção — nunca tokens/chaves/PII, ADR-010). Os **SLIs/SLOs vivem em config VERSIONADA** (`SLOConfig`, SemVer + validação fail-closed via `LoadSLOConfig`, à imagem da política de AOS-030) e os **alertas são DETERMINISTAS** (`SLOEvaluator` com o streak de saturação avançado só por `Evaluate`, sem `time.Now`/`rand`): disparam em **headroom criticamente baixo** (`headroom_critically_low`, fracção livre < limiar) e em **saturação SUSTENTADA** (`saturation_sustained`, N observações consecutivas), mais os avisos `headroom_utilization_high` e `defer_rate_high`. O **dashboard mínimo AGREGADO** (`AggregateDashboard`/`BuildDashboard` → `DashboardSnapshot`) descreve o estado agregado — profundidade total, defer-rate, headroom livre, alertas activos — por agregação **query-time**, o antídoto directo ao *"individualmente ok, agregadamente colapsa"*. O padrão **WIDE EVENTS** é literal: o `RecordingMeter` **nunca** filtra no emit-time (guarda cada medição com TODOS os atributos de alta cardinalidade); a filtragem é query-time (`Query`/`FilterByAttr`) — prova por teste que um atributo não-filtrado no emit continua disponível para query (`TestMetrics_WideEvents_QueryTimePreservesUnfilteredAttr`). Determinismo total: agregação por série ordenada, iteração de partições/chaves ordenada, serialização estável, relógio/IDs injectáveis. **Fora** deste âmbito: o **OTel SDK real** e os dashboards de produção (EPIC-08 — a porta `Meter` é a sutura), o **TPM/RPM real** (EPIC-06 — porta `QuotaProvider`); **não** reimplementa nenhuma fonte de sinal (admission/filas/degradação/spawn) — instrumenta-as. Com o AOS-034, o **EPIC-03 está COMPLETO**.
