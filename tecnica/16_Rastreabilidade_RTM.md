...
Este documento não introduz decisões de arquitectura; **rastreia** as 28 existentes (ADR-001 a ADR-028, `docs/adr/README.md`). Referencia-as sempre por código.
...
Para cada ADR-001…028, os tickets `AOS-NNN` cujo bloco de especificação o cita explicitamente (extracção por correspondência textual sobre `specs/EPIC-*.md`) e o(s) documento(s) técnico(s) que o desenvolvem. A coluna **Nº** é a contagem de tickets implementadores distintos.
...
| **ADR-007** | Event Store replicado | Catálogo, por materializar | 17 | AOS-001, AOS-002, AOS-015, AOS-022, AOS-027, AOS-030, AOS-035, AOS-038, AOS-045, AOS-098, AOS-099, AOS-100, AOS-101, AOS-102, AOS-118, AOS-281, AOS-358 | `tecnica/01`, `tecnica/02`, `tecnica/03`, `tecnica/04`, `tecnica/05`, `tecnica/09`, `tecnica/10`, `tecnica/11` |
...
- **Sub-cobertura (≤3 tickets):** 
  ...
  - **ADR-028** (Um objectivo entra no caminho do plano por um FACTO gravado pelo nó, e o `aos-orq` consome-o sob a posse que já governa o run: o ingresso é rota do nó e NÃO importa o orquestrador (ADR-018 intacto); o consumo-uma-só-vez segue o molde de idempotency-key já existente, sem substrato de fila novo; um `serve` continua a possuir um run e a terminar (ADR-023 intacto); e um pedido sobre um run com posse responde `201 accepted` idempotente — NUNCA o estado do run —, para não reabrir o oráculo de existência que o ADR-016 fecha) — 2 ticket(s): AOS-230, AOS-417.
...
```mermaid
flowchart LR
    RF["RF-01..RF-13 (capacidades)"] --> ADR["ADR-001..028 (decisoes)"]
...
| GAP-08 | **Os catálogos de enunciado estão atrás do catálogo de documentos** — `docs/adr/README.md` enuncia 28 ADRs e é a fonte do canon que os gates lêem (AOS-314), mas `_BRIEF` §3 enuncia 14 (faltam ADR-015, ADR-016, ADR-017, ADR-018, ADR-019, ADR-020, ADR-021, ADR-022, ADR-023, ADR-024, ADR-025, ADR-026, ADR-027, ADR-028) e `specs/00` §11 enuncia 23 (faltam ADR-024, ADR-025, ADR-026, ADR-027, ADR-028). O próprio README declara os dois «a referência de enunciado para todos os ADRs», pelo que a divergência é, pela sua própria regra, um defeito e não uma actualização | `_BRIEF` §3, `specs/00` §11, `docs/adr/README.md` | Completar os dois catálogos de enunciado com os ADRs em falta, ou emendar o README para deixar de os declarar referência de enunciado |

Nenhum ADR do canon gated e nenhum NFR está **sem** cobertura mínima: 28/28 ADRs (ADR-001…028) e 12/12 NFRs (NFR-01…NFR-12) têm pelo menos um ticket associado. As lacunas acima são de **profundidade e verificação** — excepto GAP-08, que é de **coerência entre catálogos**.
...
- **GAP-07** — registava que ADR-020…023 estavam fora do canon lido pelos gates e deixava a decisão por tomar. **Decidido em AOS-314: o canon passa a ADR-001…028.** ADR-020 tinha zero tickets e teria posto o `ref-lint` vermelho; passou a ser citado pelos cinco tickets que o próprio ADR nomeia (§5 e §6), em `specs/EPIC-19` — a lacuna era da citação, não da cobertura.
...
- **ADR (Architecture Decision Record):** registo de decisão de arquitectura. O canon que os gates lêem é **ADR-001…028**, alinhado com o catálogo em `docs/adr/README.md` por AOS-314 (e ADR-024, AOS-390; ADR-025, AOS-090; ADR-026, AOS-398; ADR-027, AOS-413; ADR-028, AOS-417). Os catálogos de *enunciado* (`_BRIEF` §3, `specs/00` §11) estão atrás dele — divergência registada em **GAP-08**, não uma gama a corrigir por edição desta linha.
