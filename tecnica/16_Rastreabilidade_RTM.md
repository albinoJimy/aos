...
A rastreabilidade cobre os 19 ADRs canónicos (`_BRIEF` §3), as 11 capacidades funcionais (`specs/00` §4), os 10 *drivers* não-funcionais (`specs/00` §7) e os **227 tickets** `AOS-001`–`AOS-227` distribuídos por 18 epics. Os dados das matrizes ADR×ticket e NFR×ticket foram extraídos por análise textual dos ficheiros `specs/EPIC-*.md` (correspondência dos códigos `ADR-0NN` e `AOS-NNN` por bloco de ticket), não por atribuição editorial *a posteriori*.
...
|---|---|---|
| `tecnica/00_Arquitectura_Solucao.md` | Todos (transversal) | AOS-001 – AOS-227 |
...
| `tecnica/09_Governacao_Conformidade.md` | EPIC-09, EPIC-18 | AOS-087 – AOS-097 (+ AOS-190 – AOS-227) |
| `tecnica/10_Topologia_Implantacao_Operacao.md` | EPIC-10, EPIC-11 | AOS-098 – AOS-108 (+ AOS-118) |
| `tecnica/11_Convencoes_Engenharia_Evolucao.md` | EPIC-11 (+ EPIC-05 auto-mod), EPIC-18 | AOS-109 – AOS-118 (+ AOS-045–054, + AOS-190 – AOS-227) |
...
| `tecnica/16_Rastreabilidade_RTM.md` | Todos (transversal — meta-rastreabilidade) | AOS-001 – AOS-227 |
...
flowchart LR
    ...
    EPIC --> TICK["AOS-001..AOS-227 (tickets)"]
