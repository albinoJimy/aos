...
| **I** | Fuga de contexto sensível entre turnos ou principals. | Contexto ≠ registo (projecção); microVM efémera por execução sem estado residual (ADR-004). | AOS-036, AOS-064, AOS-066 | **deferido-com-eixo** (AOS-036 entregue; os drivers de microVM injectam executores REAIS quando as URLs host-side estão definidas e recusam a execução sem elas — falta a infra provisionada, AOS-103. Parêntesis corrigido a 2026-09-06 pelo mesmo motivo que §4.6, AOS-344/AOS-356-a) |
...
| **E** | Reutilização de credencial fora de contexto (confused deputy downstream). | TTL curto + escopo + revogação central; credencial destruída após uso (ADR-006; `tecnica/07` §3). **CORRECÇÃO (AOS-324, 2026-09-05): «escopo» eram TRÊS eixos e só um estava imposto.** A autorização da troca decide sobre `Capability`, `Region` e `Provider`. A capability é imposta pelo `ScopeGate` do broker (`utilizador ∩ classe`); a região pela `ObligationRegion` do RM, que compara `Resource.Region` — o mesmo valor que forma a chave do Vault. **O provedor não tinha imposição nenhuma**, e como a capability da troca é uma CONSTANTE (`cap:http.post`, AOS-264) o provedor era o único discriminante: um principal autorizado a trocar obtinha material de QUALQUER provedor presente no Vault — a confusão de deputado que esta linha dava por mitigada. O AOS-324 fecha-o por política declarada (`WithClassProviders`; tecto da classe ∩ grants `prov:` do token, que só estreitam), com defesa server-side no despacho antes de a chave do Vault existir. **Estado por omissão `unset`:** sem política declarada o eixo não é imposto por conjunto, e a postura é selada em `provider_policy` de cada `credential.exchange.issued` — auditável, nunca silenciosa. O wiring (DEF-218) tem de declarar a política e assertar `enforced`. | AOS-070, AOS-071, AOS-324 | **entregue por configuração** (era: entregue) |
...
| Cat. | Ameaça concreta | Controlo AOS (componente/mecanismo + ADR) | Ticket | Estado |
|---|---|---|---|---|
| **S** | Execução assume identidade de outro principal. | Identidade efémera por execução; microVM destruída no fim (ADR-004). | AOS-064, AOS-344 | **deferido-com-eixo** (executor remoto REAL injectado por `AOS_SANDBOX_{FIRECRACKER,GVISOR}_URL`, com bundle/microVM novo por execução; sem URL o exec é recusado com `ErrDriverUnavailable`. Falta o pool provisionado — AOS-103. A produção deixou de poder cair no driver in-process, AOS-344) |
| **T** | Persistência de payload malicioso entre corridas. | FS read-only + overlay efémero descartado; sem estado residual (ADR-004). | AOS-066, AOS-344 | **deferido-com-eixo** (o bundle OCI é novo e efémero por execução no componente gVisor, rootfs read-only e `/seed` em bind read-only; falta o provisionamento — AOS-103) |
...
| **I** | **Exfiltração via canal permitido** (padrão CamoLeak, CVSS 9.6). | Egress default-deny + allowlist por identidade + filtragem DNS + content-security (ADR-004; `tecnica/07` §5). | AOS-067, AOS-068 | entregue no módulo; **por-fazer** no nó (o nó monta o substrato, mas o caminho de egress do guest não é o que esta linha descreve — §5.2-f) |
| **D** | Bomba de recursos esgota o host. | Limites de CPU/mem/IO por microVM; pool com quota; cold-start < 125 ms (ADR-004). | AOS-064, AOS-065 | **deferido-com-eixo** (é a linha MENOS afectada pela reavaliação: quota de pool e orçamento de cold-start são propriedades do pool, e o pool é precisamente o que falta — AOS-103) |
| **E** | **Evasão do sandbox para o host** (sandbox escape). | microVM/gVisor (fronteira de virtualização), seccomp mínimo, sem socket do host; jails só defesa secundária (ADR-004; `tecnica/07` §4). | AOS-064, AOS-066, AOS-344 | **deferido-com-eixo** (a fronteira ao nível do kernel é fornecida pelo código e provada no componente; o que resta é topologia — AOS-103. O caminho que faria esta linha ser lida como falsa — produção a correr o driver in-process — está fechado desde AOS-344; §5.2-f) |

> **Reavaliação de 2026-09-06 (commit `2ca2d5c`) — AOS-344 e AOS-356-a.** As quatro linhas S/T/D/E sustentavam-se num mitigante que não existia («o catálogo de tools do nó está vazio») e descreviam os drivers como *skeletons* que não fornecem a fronteira. Nenhuma das duas coisas é verdade: o catálogo sancionado declara uma tool com bloco `sandbox` (`deploy/server/model-tools/tools.json:18`) e o compose de produção monta-o (`:381`), e `buildSandboxDriver` (`packages/cmd/aos/firecrackerexecutor.go:104-119`) injecta executores REAIS quando as URLs dos componentes host-side estão definidas. O estado declarado das quatro mantém-se **deferido-com-eixo** — o adiamento continua deliberado e registado, e a contagem de §5.4 não muda —, mas o que está adiado mudou de natureza: **é provisionamento de topologia, não construção de mecanismo** (AOS-103). O que era falso era o parêntesis, e é o parêntesis que muda. Ver §5.2-f.
...
- **(f) Substrato de sandbox: a fronteira é fornecida, falta topologia.** *(Reavaliado a 2026-09-06 sobre o commit `2ca2d5c`, no método de `tecnica/14` §5.2 — AOS-344 e AOS-356-a. As duas afirmações da versão anterior deixaram de ser verdade, e em direcções opostas: uma **subdeclarava** a postura, a outra **inventava** um mitigante.)*
  - **Os drivers já não são skeletons.** `packages/cmd/aos/firecrackerexecutor.go:104-119` (`buildSandboxDriver`) injecta executores remotos REAIS quando `AOS_SANDBOX_FIRECRACKER_URL`/`AOS_SANDBOX_GVISOR_URL` estão definidas — microVM sobre KVM num caso, `runsc` com interposição de syscalls em user-space no outro —, e os componentes host-side existem e estão escritos (`deploy/node/dev-hardened/firecracker/orchestrator/`, `deploy/server/gvisor/component/`). Sem URL o driver fica sem executor provisionado e o exec devolve `ErrDriverUnavailable`: fail-closed, o gap honesto. **A fronteira ao nível do kernel é fornecida pelo código; o que falta é provisioná-la na topologia** — que é o que **AOS-103** possui. Foi esta subdeclaração que manteve uma lente inteira de `analises/12` longe do caminho onde o enforcement vive.
  - **O mitigante invocado era falso.** «O catálogo de tools do nó está vazio» não descreve o deployment sancionado: `deploy/server/model-tools/tools.json:18` declara `"sandbox": { "command": "read", "path_arg": "doc_id" }` para a tool `doc_read`, e `deploy/server/docker-compose.prod.yml:381` monta o ficheiro no contentor do nó. O catálogo é *entregue e montado*; fica *activo* quando o operador aponta `AOS_MODEL_TOOLS` (default vazio no mesmo compose, `:350`).
  - **O que o mitigante escondia, e que AOS-344 fechou.** O mesmo compose definia `AOS_SANDBOX_DRIVER` vazio (`:364` à data do achado; hoje `:373`, e já com `gvisor` por omissão), e o vazio elegia o driver de referência in-process. O dano não é escape de sandbox — esse driver não corre processos, é um VFS in-process que impõe as invariantes de isolamento fail-closed. O dano é de **postura**: é o único dos três que falha **aberto**. Os outros dois recusam a execução sem executor; este sucede em silêncio, e o resultado — que nenhuma fronteira ao nível do kernel produziu — é selado na hash-chain WORM como se fosse um efeito real. Desde **AOS-344**, sob `AOS_MODE=production` e com pelo menos uma tool com bloco `sandbox` no catálogo, o nó **recusa arrancar** com esse driver (`ErrProductionNeedsSandboxDriver`), por omissão ou por escolha explícita; fora de produção nada muda.
  - **Mitigantes actuais** (os que o estado verificado sustenta, agora que o antigo caiu): a produção só compõe drivers com fronteira ao nível do kernel; sem executor provisionado a execução é recusada em vez de fabricada; e as tools oferecidas ao modelo sem executor são NOMEADAS no banner de arranque (AOS-203/AOS-248), pelo que «oferecida» nunca se confunde com «executável». Eixos: AOS-064/AOS-066 (contrato), **AOS-103** (pool provisionado em produção), **AOS-344** (porta de produção do driver).
...
**Aviso de sequência (não é recomendação):** carregar o bundle PDP real no nó (§4.3-E, AOS-181) **não deve** preceder a activação do TaintGate (§5.2-e, AOS-183). Hoje o *deny-all* do PDP mascara §5.2-e; ligar a política sem ligar a barreira de taint torna o nó permissivo com a defesa control/data-plane inerte. *(Correcção de 2026-09-06, commit `2ca2d5c`, AOS-344: a versão anterior invocava aqui, como segundo mascarador, «o catálogo de tools vazio» — o catálogo sancionado não está vazio, e §5.2-f já não depende dele.)*
