...
| **Anti-replay por `jti`** — um ID-token não vale duas vezes | Mesma leitura com o mesmo token: `200` depois `404`. **Adversário novo desde 2026-08-20:** o memo do leitor (PR #100) guarda a credencial verificada no contexto **do pedido**, para que uma rota que autorize N vezes não produza um falso replay contra si própria. Criá-lo fora do closure por pedido faria uma verificação servir **todos** os pedidos — bypass de autenticação completo, invisível em qualquer teste de caminho feliz. **Controlo em produção (`-Fronteira`):** o **mesmo** token apresentado duas vezes em pedidos separados dá `200` e depois `403`. É o único gesto que distingue «a fronteira está inteira» de «o memo vazou», e nenhum outro modo do script o consegue fazer, porque todos vão buscar um token novo por chamada | ✅ |
...
| **O apagamento sobrevive ao restauro** (AOS-436) — um bundle anterior ao apagamento já não traz a KEK de volta em silêncio | Nó real sobre um Vault falso que sabe a **idade** de cada chave, lista as que tem e recusa o `DELETE` sem `deletion_allowed`: KEK ressuscitada é destruída de novo (pela cadeia e pelo registo importado, autenticado por HMAC) e `dsar.key_reshredded` selado. **Controlos:** a KEK de um titular que **voltou** fica intacta; registo forjado, linha no futuro e selo relido não destroem; KEK sob legal hold não é destruída; e **enquanto por provar o conteúdo não abre nem se sela** (medido pelo `contentOpener` do replay soberano, não pelo `/readyz`). Mutação «desligar a reconciliação» avermelha 20 testes, e cada achado das duas revisões adversariais tem a sua. **Não exercido em produção nem no `restore-drill.sh`**; não cobre apagamentos posteriores ao último backup | 🧪 |
...
| **Rota de autonomia assinada e selada** (AOS-087) — mudar níveis deixou de ser editar um ficheiro no servidor | `POST /autonomy` com assinatura ed25519 sobre o payload canónico (agente‖domínio‖nível‖motivo), *nonce* de uso único durável, motivo **obrigatório**. O `actor` selado vem do **emissor verificado**, nunca do corpo. **Controlos:** emissor não registado → `403`; mesmo *nonce* duas vezes → `403`; assinatura de `L1` reapresentada como `L5` → `403`; o selo nomeia quem assinou e guarda o motivo; `GET` reflecte o `POST`. **EXERCIDA EM PRODUÇÃO a 2026-08-21** com um agente FICTÍCIO (`agt-prova-autonomia/fs`), escolhido para não mudar decisão nenhuma — a cascata é instância → classe → piso, e nenhum run usa esse nome. Mudança assinada: `200`, `(nao registado)` → `L2`. **Controlos, os dois a morder:** o MESMO pedido reapresentado → `403` (nonce de uso único durável); assinatura de uma chave impostora → `403`. **Verificado pelo WORM e não pela resposta da rota** — o `worm.wal` vivo trazido por SSH mostra `seq=21 cap=autonomy:set_level actor=ops:prod recurso=agt-prova-autonomia/fs` com a obrigação `autonomy.level_changed=L0→L2`. O motivo selado diz que é uma prova com agente fictício, para quem auditar a cadeia daqui a um ano não ler uma mudança inexplicada | ✅ |
...
| **Ensaio antes de virar o interruptor** — `POST /autonomy/simular` | Relê os selos de mediação do WORM e re-classifica com o **mesmo** classificador do nó, num registo efémero **sem sink**. **Controlos:** a simulação e o classificador real **concordam** para os mesmos factos; usa o **parser do arranque** (recusa o que o nó recusaria); **não sela** — o ensaio não contamina o trilho. Limite declarado na resposta: avalia **só** o overlay de autonomia. **Exercido no nó real (2026-08-20)** com credencial OIDC humana de uso único: `avaliados: 3`. **Controlo:** a mesma rota, no mesmo nó e no mesmo código, devolveu `avaliados: 0` **antes** de existir um run com tool calls, e `3` depois — o par 0→3 é o que torna a prova não-vacuosa, porque o defeito que o #96 corrigiu (re-verificação da credencial **por run**, que o anti-replay por `jti` recusava) daria `0` nas **duas** leituras | ✅ |
