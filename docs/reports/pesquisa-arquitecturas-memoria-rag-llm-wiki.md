...
| `log.md` append-only (V6) | AOS-038, ADR-007 | **Implementado e superior** — o histórico episódico vive no Event Store replicado (`adapters/eventstore_adapter.go`), não num ficheiro; `KnowledgeStreamID` com estado reconstruído por replay (à data deste relatório valia `"memory.semantic.knowledge"`; renomeado para `"aos-internal/memory/semantic-knowledge"` pelo AOS-424, porque o ponto não é representável num subject NATS). |
