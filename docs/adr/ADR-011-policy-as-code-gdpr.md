...
**Nota posterior (2026-09-17, AOS-407): a soberania por board está LIGADA no caminho de efeito.** O §5 deixou de ser forma: a claim `board` vai assinada no token NHI (afirmada pelo IdP no mint, herdada pelos filhos), o hook de identidade propaga-a ao principal do Reference Monitor, e o nó liga ao PDP a autoridade board→região — a MESMA do read-path — como resolvedor vivo. Cada decisão de base permit exige um board resolvível e leva a obrigação `region`, que o PEP impõe contra a região do recurso; board vazio ou desconhecido é negado fail-closed. Residual declarado: a região do recurso é declarada pelo operador (validada no arranque contra o mapa de boards, não assinada — DEF-812).
