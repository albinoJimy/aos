...
**O que a produção mediu.** A v0.1.15, com a decisão original do §1, entrou em produção a 2026-09-16. O run `run-delegado-1789569005` (tool `doc_read` em gVisor) deu ao SLI **30,8 ms com 2 amostras e 32,7 ms com 7**, contra 15 ms, com o streak dos dois `critical` a subir até 2 de 3. A correcção do AOS-398 tirou a execução do sandbox (1,21 s → ~31 ms) mas deixou o SLO violado em cada run normal — o mesmo alerta falso, com uma causa diferente. A política sempre coube em 2–8,6 ms (o `latency_ns` dos selos); a diferença atribui-se à escrita durável do selo no Event Store e no WORM — **por inferência**: nesse run não foi possível separar as duas metades, e é precisamente essa medida directa que esta Emenda passa a publicar. **Nota posterior (2026-09-17, AOS-402):** a medida directa em produção a 2026-09-17 (v0.1.19, AOS-402): escrita p50 7,38 ms, p95 8,17 ms e máximo 8,26 ms em dois permits contraria a inferência — política + escrita ≈ 12–13 ms por tool call, pelo que a escrita não explica os ~31 ms da v0.1.15. A decisão emendada mantém-se: o argumento é o do RB-04 (um sink lento não é uma falha do PDP), não o tamanho da escrita. **Nota posterior (2026-09-17, AOS-404):** decompostos por tool call a partir dos ficheiros de produção, os ~31 ms da v0.1.15 foram o p95 de poucas amostras dominado por duas tool calls lentas (AOS-404): a 1.ª do run, com a escrita a frio (WORM 7,9 ms + Event Store ~15 ms), e a 6.ª, cuja própria política levou 17,06 ms. Os 30,8 e 32,7 ms registados são consistentes com essa decomposição: o p95 interpolado sobre 2 e 7 amostras dá esses valores com appends do Event Store dentro dos limites medidos. A frase «a política sempre coube em 2–8,6 ms» é falsa: em 27 calls de produção, 4 passaram os 15 ms (até 103,63 ms). O Facto 2 mantém-se — a execução em gVisor continua duas ordens de grandeza acima —, mas o SLO da política, com a janela de 5 minutos e sem mínimo de amostras, é violado por um run com uma call lenta; a escolha entre corrigir esse caminho e recalibrar o SLO fica para ticket próprio.
