...
**O que a produção mediu.** A v0.1.15, com a decisão original do §1, entrou em produção a 2026-09-16. O run `run-delegado-1789569005` (tool `doc_read` em gVisor) deu ao SLI **30,8 ms com 2 amostras e 32,7 ms com 7**, contra 15 ms, com o streak dos dois `critical` a subir até 2 de 3. A correcção do AOS-398 tirou a execução do sandbox (1,21 s → ~31 ms) mas deixou o SLO violado em cada run normal — o mesmo alerta falso, com uma causa diferente. A política sempre coube em 2–8,6 ms (o `latency_ns` dos selos); a diferença atribui-se à escrita durável do selo no Event Store e no WORM — **por inferência**: nesse run não foi possível separar as duas metades, e é precisamente essa medida directa que esta Emenda passa a publicar. **Nota posterior (2026-09-17, AOS-402):** a medida directa em produção a 2026-09-17 (v0.1.19, AOS-402): escrita p50 7,38 ms, p95 8,17 ms e máximo 8,26 ms em dois permits contraria a inferência — política + escrita ≈ 12–13 ms por tool call, pelo que a escrita não explica os ~31 ms da v0.1.15. A decisão emendada mantém-se: o argumento é o do RB-04 (um sink lento não é uma falha do PDP), não o tamanho da escrita. **Nota posterior (2026-09-17, AOS-404):** decompostos por tool call a partir dos ficheiros de produção, os ~31 ms da v0.1.15 foram o p95 de poucas amostras dominado por duas tool calls em que TODOS os troços ficaram lentos ao mesmo tempo (AOS-404): a janela da política — que inclui o selo durável da revalidação no WORM — e as duas escritas do selo de mediação, no WORM e no Event Store. Isso aponta para episódios transitórios de I/O ou do nó, não para o PDP; o custo do Event Store dessas duas calls é inferido do próprio SLI, dentro de limites medidos. A frase «a política sempre coube em 2–8,6 ms» é falsa: em 27 permits de produção, 4 passaram os 15 ms (até 103,63 ms). E a janela da política não é só decisão: inclui o selo durável da revalidação por chamada (AOS-381), escrito no WORM por um hook da cadeia — pelo que o argumento desta Emenda, «um sink lento não é uma falha do PDP», também atinge o SLO que ela deixou. O Facto 2 mantém-se (a execução em gVisor continua duas ordens de grandeza acima). Com a janela de 5 minutos e sem mínimo de amostras, um run curto com uma call lenta viola o SLO; medir por hook é o AOS-405 (Decisão emendada §5), e decidir entre corrigir e recalibrar espera pelos dados que ele trouxer.
...
5. **A política também se publica partida por hook** (AOS-405, 2026-09-17), num atributo por hook (`aos.mediation.hook_latency_ns.<hook>`) e no `/metrics` do nó, sem SLO. Não muda o SLO nem a janela: dá ao operador — e à decisão que o AOS-404 deixou em aberto — o hook que pesa, incluindo o selo durável que o hook de revalidação escreve dentro da janela.
...
- Tickets: **AOS-398** (a decisão original), **AOS-401** (a Emenda), **AOS-402** (a escrita do selo no `/metrics`), **AOS-404** (os ~31 ms da v0.1.15 explicados) e **AOS-405** (a política partida por hook), todos em `specs/EPIC-08_Observabilidade_Evals.md`.
