...
| ADR-005 | Separação control/data-plane + taint — a semântica de enforcement com que o nó a cumpre hoje (opção C, autorização derivada do contexto) está materializada no **ADR-034**; a forma forte (dual-LLM/CaMeL, opção A) fica com gatilho | Catálogo, por materializar | catálogo em `specs/00_System_Spec.md` §11 |
