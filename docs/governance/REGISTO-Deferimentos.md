...
| DEF-282 | DEFERIDO | packages/cmd/aos-orq/main.go | **O EVENT STORE DE REFERÊNCIA NÃO ARBITRA ENTRE PROCESSOS — medido, não suposto (AOS-281).** A posse por lease depende inteiramente de o `expected_seq` do stream `lease:<run_id>` ser atómico ENTRE ESCRITORES. O `LeaseManager` está correcto, mas a sua correcção é CONDICIONAL a essa propriedade. As réplicas de AOS-100 são cópias IN-PROCESS do log e o índice de dedup vive em memória: um `Open` faz replay do WAL no arranque e, a partir daí, cada processo tem a SUA cabeça. Medido deterministicamente a 2026-08-30 — dois `eventstore.Open` sobre o mesmo ficheiro, dois `Claim` do mesmo run: AMBOS passam e AMBOS mintam o token 1. CONSEQUÊNCIA: o AC1 de AOS-281 («dois processos a disputar o mesmo run: só um ganha, o outro vê o `expected_seq` falhar») NÃO é satisfazível com o Event Store de referência, e correr dois `aos-orq` em paralelo sobre um WAL partilhado NÃO é seguro. O que fica provado com processos reais é a posse SEQUENCIAL (handoff por anúncio e re-hidratação a atravessarem a fronteira do processo); a contenção concorrente fica provada in-process, onde o log é efectivamente partilhado. Tem SENSOR: `TestLimite_EventStoreDeReferenciaNaoArbitraEntreProcessos` falha no dia em que o substrato ganhar a propriedade, obrigando a retirar esta linha e a declaração de ADR-023 §4 . MITIGAÇÃO EM CURSO (AOS-285, criado 2026-08-30): enquanto o substrato não arbitrar, a configuração insegura passa a ser IMPEDIDA por um guard de arranque — o nó recusa arrancar sobre um Event Store já detido. O guard NÃO fecha este deferimento (o substrato continua sem arbitrar); fecha a via pela qual alguém chegaria ao defeito sem saber. Nota de desenho registada no ticket: o guard não pode assentar num lease do Event Store, que seria circular e vacuoso pela mesma razão medida aqui | AOS-100, AOS-285 | Arquitecto de Plataforma | Existir um Event Store genuinamente partilhado/replicado entre processos (NATS JetStream, tabela de stack) em que o `expected_seq` seja atómico entre escritores | ABERTO |
