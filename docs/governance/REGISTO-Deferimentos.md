...
| DEF-268 | DEFERIDO | packages/cmd/aos/bootstrap.go | **SELAGEM PERIÓDICA de checkpoints do WORM — DEFERIDA; a VERIFICAÇÃO ancorada está ENTREGUE (AOS-268).** O nó CONSOME uma âncora assinada (`audit.VerifyFromCheckpointAtHead` na etapa 2a-bis do `Bootstrap`, com o trust-anchor + checkpoint + piso de frescura de `AOS_WORM_TRUST_ANCHOR`/`AOS_WORM_CHECKPOINT_FILE`/`AOS_WORM_EXPECTED_HEAD`) e fecha, fail-closed, a truncatura do tail e a reescrita da génese que a re-verificação de hash-chain de AOS-221 não apanha. O que fica deferido é o lado da PRODUÇÃO da âncora: SELAR novos checkpoints exige a chave PRIVADA do selador, que — pela regra do nó — NÃO vive no runtime (`audit.Signer`/`Seal` correm OUT-OF-PROCESS, molde AOS-156/custódia de chave D4). Sem selagem periódica, o piso de frescura só prova até ao último audit_seq selado: registos legítimos apendidos depois do último checkpoint e ainda não selados podem ser truncados sem esta verificação os apanhar — encolher essa janela é o trabalho da selagem periódica out-of-process. Eixo do checkpoint como âncora de frescura: AOS-072 (na prosa; o eixo executável é AOS-268 + custódia de chave AOS-156/D4) | AOS-268; custódia de chave AOS-156 (D4) | Responsável de Segurança | Custódia de chave real (D4/AOS-156) que permita selar checkpoints periodicamente out-of-process | ABERTO |
| DEF-269 | DEFERIDO | packages/cmd/aos/posture_banner.go | Idem DEF-268 na linha de banner `wormAnchorPostureBanner`: a dobra ANCORADA declara a verificação entregue e o LIMITE honesto (o piso só prova até ao audit_seq selado); a dobra NÃO ANCORADA nomeia as três envs que a ligam e declara que a selagem periódica de novos checkpoints fica DEFERIDA (chave privada out-of-process, molde AOS-156). A postura anunciada = postura ligada (AOS-203/AOS-248) | AOS-268; custódia de chave AOS-156 (D4) | Responsável de Segurança | Idem DEF-268 | ABERTO |
...
| DEF-271 | DEFERIDO | packages/platform/model-gateway/routing/router/router.go | **COMPOSIÇÃO do estágio de roteamento no pipeline do GW — DEFERIDA; a MÁQUINA de scoring está ENTREGUE (AOS-269).** O ADR-021 está implementado por inteiro sobre o `router.Router`: portas de factores (health, headroom, custo, latência, task-fit, estabilidade) em aritmética INTEIRA, tabela de pesos EMBEBIDA e ASSINADA (ed25519 + trust anchor pinado, molde de `policy/allowlist`) com carregamento fail-closed, guard-test AST que proíbe float/rand/relógio no caminho de decisão, e o cenário de soberania que prova que NENHUM peso elege candidato cross-border. O que fica deferido é o **wiring**: o `router.Router` (AOS-059) **nunca esteve composto em produção** — `NewProduction` compõe `failover.NewStage` como estágio de roteamento e `router.New` não tem chamador fora de testes em todo o repositório. Logo o scoring, construído SOBRE esse router, **não tem efeito em produção** enquanto o estágio não for composto. A dívida é **PRÉ-EXISTENTE a AOS-269** (o router já estava por ligar antes deste ticket) e fechá-la é substituir/complementar o `failover` no pipeline — mudança arquitectural do caminho quente de TODAS as chamadas de modelo, com verificação própria. Alcance declarado na **emenda 1.1 do ADR-021** (§5-bis, 2026-08-13): na v1 o scoring é composto por opção e a regra 3 aplica-se quando está composto | AOS-269 (máquina); wiring POR ATRIBUIR — ver emenda 1.1 do ADR-021 | Arquitecto de Plataforma | Ticket próprio que componha `routingstage`+`router` (com o scoring armado) no pipeline do GW, hoje servido pelo `failover`, com prova pela cadeia real | ABERTO |
| DEF-272 | DOCUMENTAL | specs/EPIC-20_Prontidao_Agentica_Remediacao.md | **EMISSÃO do veredicto do verificador — SEM CHAMADOR DE PRODUÇÃO (AOS-271).** O contrato tipado (`plannerevents.NewVerdictRecorded`: pass/fail + códigos + inteiros, sujeitos amarrados às arestas de entrada do verificador no documento aprovado), a atribuição na admissão (`planvalidate/verifier.go`) e a projecção para o observável do ramo (`plandispatch.ResultFromVerdict`) estão fechados; o que NÃO existe é quem chame `Recorder.RecordVerdict` por um verificador em execução — é wiring do ciclo-de-vida do run. O AC «veredicto registado como evento» foi DESMARCADO no EPIC-20 por esta razão. EIXO DE SEGURANÇA (o que a auditoria adversarial da wave falsificou na afirmação anterior de que não havia nenhum): com a admissão a aceitar ramos sobre `verdict` e nenhum emissor ligado, um ramo de qualidade admitido era DECIDIDO como `branch_not_taken` e o facto ficava REGISTADO — poda silenciosa de metade do organigrama aprovado. MITIGADO nesta passagem do lado do despacho: a ausência de um observável no resultado registado é agora INDECIDA (o nó espera, visível em `waiting_condition`), nunca falsa, pelo que nenhuma decisão de ramo é registada sobre um observável que ninguém produz | AOS-238 | Arquitecto de Plataforma | Existir wiring do ciclo-de-vida do run que ligue um nó verificador ao emissor (AOS-238) | MITIGADO |
| DEF-273 | DOCUMENTAL | specs/EPIC-20_Prontidao_Agentica_Remediacao.md | **TRANSPORTE de payload tipado — SEM CHAMADOR DE PRODUÇÃO (AOS-272).** Não existe implementação de `plandispatch.PayloadView` nem chamador de `Recorder.RecordPayloadPublished` fora de testes: nenhum consumidor recebe referência nenhuma num run real (`ErrPayloadNotPublished` em qualquer passagem). O AC «o consumidor recebe referência/resumo» foi DESMARCADO no EPIC-20 por esta razão. EIXO DE SEGURANÇA (a afirmação anterior de que não havia nenhum era FALSA): sem `planmaterialize.WithEffectOracle` ligado pelo mesmo wiring, o materializador usa o `DefaultEffectOracle` — tudo conta como efeito — e emite verificadores com `Authority[]` VAZIA e sem tool call, isto é, o AC1 de AOS-271 cumpre-se por o verificador não conseguir fazer nada | AOS-238 | Arquitecto de Plataforma | Existir wiring do ciclo-de-vida do run que implemente a `PayloadView` e ligue o oráculo de efeito real (AOS-238) | ABERTO |
| DEF-274 | DOCUMENTAL | tecnica/18_Planner_Meta_Orquestracao.md | **O GATE NÃO VÊ AS EXTENSÕES DE ADR-022 (invariante §2.4(5)).** `planapproval.PlanNode`/`PlanCard` não projectam `role`, `conditional_on`, `outputs` nem `consumes`: um plano com verificador, ramo de qualidade e arestas de dados chega ao revisor humano como uma lista de nós e arestas de precedência. Enquanto a projecção não existir, o approval-card DESCREVE MENOS do que a execução — que é a razão declarada para as três extensões rejeitarem na admissão em vez de só clampar. A afirmação de visibilidade foi RETIRADA de tecnica/18 §3.3.3 e do EPIC-20 nesta passagem, em vez de ficar por cumprir | AOS-238 | Arquitecto de Plataforma | Projecção de role/conditional_on/outputs/consumes para o PlanCard no wiring PlanDocument→gate (AOS-238) | ABERTO |
...
| packages/cmd/aos/posture_banner.go | DEFERIDO | 3 |
...
| packages/platform/model-gateway/routing/router/router.go | DEFERIDO | 1 |
