...
| DEF-912 | DOCUMENTAL | analises/10_Auditoria_ORQ_SCH_PDP_Adversarial.md | **Promoção automática de autonomia desarmada, e dois dos três sinais de anomalia sem fonte.** Sucessora de DEF-908, que fechou a metade de SEGURANÇA (o `autonomy.Controller` passou a ter chamador; um TRIP do disjuntor despromove os pares que o run mediou, e o selo sobrevive ao reinício). O que fica é medição, não wiring: **(a) PROMOÇÃO** — `autonomy.ReliabilitySource` pede taxa de erro sustentada e o nó não a consegue medir, porque o selo de mediação é escrito ANTES do despacho (`reference-monitor/monitor.go`: `RecordMediation` na linha 348, `dispatch` na 361) e o `toolErr` nunca volta ao WORM — um agente com 100% das tools a falhar tem taxa de erro ZERO em qualquer fonte derivável do trilho; o controlador corre com `src == nil`, que é o fail-safe «nunca promove». **(b) OVERRIDE-RATE** — calculado em `hitl.Channel.finish`, e o `Channel` não está composto no nó (`integration/secured.go` declara que não há `risk.Gate` composto), pelo que `RecordOverrideRate` nunca produz um valor. **(c) DRIFT** — o `revalidation.Alert` traz `ToolID`/`Version`/`Digest` e NENHUM identificador de agente: derivou o artefacto, não o agente, e despromovê-lo puniria o agente pelo defeito do registry | AOS-090 | Arquitecto de Plataforma | Selar um `tool.result` PÓS-despacho no WORM com NHIID + capability + sucesso/erro — é a única coisa que torna a taxa de erro honesta, e nada no repositório a produz hoje. Sem ela, ligar a promoção é decidir uma ELEVAÇÃO de privilégio sobre uma taxa de negação de política a fingir de fiabilidade | ABERTO |
