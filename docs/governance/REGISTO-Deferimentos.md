...
| DEF-282 | DEFERIDO | packages/cmd/aos-orq/main.go | **O EVENT STORE DE REFERÊNCIA NÃO ARBITRA ENTRE PROCESSOS — medido, não suposto (AOS-281).** A posse por lease depende inteiramente de o `expected_seq` do stream `lease:<run_id>` ser atómico ENTRE ESCRITORES. O `LeaseManager` está correcto, mas a sua correcção é CONDICIONAL a essa propriedade. As réplicas de AOS-100 são cópias IN-PROCESS do log e o índice de dedup vive em memória: um `Open` faz replay do WAL no arranque e, a partir daí, cada processo tem a SUA cabeça. Medido deterministicamente a 2026-08-30 — dois `eventstore.Open` sobre o mesmo ficheiro, dois `Claim` do mesmo run: AMBOS passam e AMBOS mintam o token 1. CONSEQUÊNCIA: o AC1 de AOS-281 («dois processos a disputar o mesmo run: só um ganha, o outro vê o `expected_seq` falhar») NÃO é satisfazível com o Event Store de referência, e correr dois `aos-orq` em paralelo sobre um WAL partilhado NÃO é seguro. O que fica provado com processos reais é a posse SEQUENCIAL (handoff por anúncio e re-hidratação a atravessarem a fronteira do processo); a contenção concorrente fica provada in-process, onde o log é efectivamente partilhado. Tem SENSOR: `TestLimite_EventStoreDeReferenciaNaoArbitraEntreProcessos` falha no dia em que o substrato ganhar a propriedade, obrigando a retirar esta linha e a declaração de ADR-023 §4 . MITIGAÇÃO **ENTREGUE** (AOS-285, 2026-08-30): a configuração insegura passa a ser IMPEDIDA — o nó recusa arrancar sobre um Event Store já detido (`ErrEventStoreJaDetido`), com a posse arbitrada pelo SISTEMA OPERATIVO sobre um ficheiro irmão `<wal>.lock` (`eventstore.LockWAL`), não pelo log. Provado com dois PROCESSOS reais, incluindo que a morte abrupta do detentor liberta a posse (sem TTL, sem posse órfã) e que as vias de leitura (`wal-inspect`/`wal-summary`) continuam a abrir o mesmo WAL. O guard NÃO fecha este deferimento (o substrato continua sem arbitrar); fecha a via pela qual alguém chegaria ao defeito sem saber. Nota de desenho registada no ticket: o guard não pode assentar num lease do Event Store, que seria circular e vacuoso pela mesma razão medida aqui. **ACTUALIZAÇÃO 2026-08-31 (AOS-100): o gatilho de fecho está SATISFEITO NUM SUBSTRATO, e NÃO no nó.** `packages/substrate/eventstore/jetstream` implementa `eventstore.EventStore` sobre JetStream R3 e PASSA o instrumento de conformidade (`conformance.RunArbitration`) nas quatro sondas — o MESMO instrumento que reporta as quatro AUSENTES contra o store de referência. Mas este deferimento fecha pelo `aos-orq`, e o nó NÃO PODE hoje usar esse substrato: `Config.EventStore` é do tipo CONCRETO `*eventstore.Store`, não da interface (medido). Enquanto isso não mudar, correr dois `aos-orq` continua a NÃO ser seguro e o sensor `TestLimite_EventStoreDeReferenciaNaoArbitraEntreProcessos` continua CERTO. **ACTUALIZAÇÃO 2 (mesmo dia): o nó JÁ PODE.** `Config.EventStore` passou a ser a porta `EventStorePort` (interface + `Healthy`/`Streams`) e `AOS_EVENTSTORE_NATS` liga o nó ao cluster. PROVADO com o cluster real: DUAS réplicas do nó sobre o MESMO Event Store arrancam ambas — o que o guard de AOS-285 antes recusava, e bem — e disputam o mesmo `expected_seq` com uma só a ganhar (`TestAOS100_DuasReplicasDoNoSobreOMesmoEventStore`). O guard do WORM MANTÉM-SE e tem teste próprio. **O que ainda prende ESTE deferimento é o `aos-orq`**, que abre o seu WAL por `abrir(wal)` e não tem via de configuração para o substrato replicado; enquanto assim for, dois `aos-orq` continuam a NÃO ser seguros e o sensor `TestLimite_EventStoreDeReferenciaNaoArbitraEntreProcessos` continua CERTO — ele mede o store de REFERÊNCIA, que não mudou | AOS-100, AOS-285 | Arquitecto de Plataforma | Existir um Event Store genuinamente partilhado/replicado entre processos (NATS JetStream, tabela de stack) em que o `expected_seq` seja atómico entre escritores | ABERTO |
...
| packages/cmd/aos-orq/main.go | DEFERIDO | 2 |
...
- O servidor da API exige **certificado de cliente** verificado (`tls.VerifyClientCertIfGiven` +
  ...
  (as 10 rotas `planoControlo` da tabela de `packages/cmd/aos/planos.go`), **escopado** (não impõe cert às rotas de dados/sondas), **ADITIVO**
