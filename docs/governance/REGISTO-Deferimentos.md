...
| Última actualização | 2026-09-26 (DEF-806 re-escopado e DEF-807 fechado em substância pelo ADR-034, AOS-069) |
...
| DEF-218 | DEFERIDO | packages/cmd/aos/broker_vault_env.go | O cliente Vault do broker está CONFIGURADO mas a troca fica PENDENTE: ligar ao vivo exige bundle PDP assinado + identidade infra com `cap:http.post`, senão o default-deny NEGA a troca e nenhum turno de modelo executa (o próprio risco do desafio A3). **PRÉ-CONDIÇÕES ACRESCENTADAS (AOS-324, AOS-330 e AOS-331):** o AOS-330 fecha a divergência entre o namespace em que a política decide e o namespace do path do Vault, sem a qual «autorizado» e «o que o Vault serve» não são a mesma coisa. **(AOS-331):** o eixo provider decide QUEM troca para QUE provedor, não para ONDE a credencial vai ser apresentada — um principal autorizado a trocar para `stripe` podia pedir a troca com um `ResourceValue` de outro host, e nada relacionava as duas coisas. O wiring TEM de declarar a allowlist de hosts por provedor (`broker.WithGateProviderHosts`) e assertar que a postura selada diz `enforced`; sem isso o eixo fica em `unset` — estado declarado, mas sem imposição. **(AOS-324):** o wiring TEM de declarar a política de provedores (`broker.WithClassProviders`) e assertar que o campo `provider_policy` selado em cada `credential.exchange.issued` diz `enforced`. **CORRECÇÃO (AOS-342): `enforced` NÃO CHEGA como pré-condição.** A postura é função da nulidade do mapa de política, não do seu conteúdo: `{"payments": {"*"}}` diz `enforced` e deixa passar qualquer provedor, e um mapa vazio diz `enforced` e nega tudo. A asserção tem de ser sobre o **par** — `provider_policy=enforced` **E** `provider_policy_shape=by-class`, ambos selados no mesmo evento e observáveis no banner de arranque antes da primeira troca. Assertar só a postura fica verde sobre exactamente a confusão de deputado que o AOS-324 fechou. Sem isso o eixo *Provider* fica sem imposição por conjunto — um principal autorizado a trocar para um provedor obtém material de qualquer outro presente no Vault — e ligar o broker nesse estado é reabrir a confusão de deputado que o AOS-324 fechou por configuração. **PRÉ-CONDIÇÃO ACRESCENTADA (AOS-069, 2026-09-26):** a troca do broker constrói a sua própria `Call` com `Taint` FIXO em trusted (`packages/platform/broker/exchange.go`, `CallContext{Taint: taintTrusted}`). Com o ADR-034 o taint da autorização é o rótulo do CONTEXTO do run que pediu a acção, e com o TaintGate armado é ele que separa uma call pedida depois de conteúdo não-confiável; um trusted fixo contorná-lo-ia para toda a capability privilegiada que a troca mediasse. Latente enquanto `broker.New` não tiver chamador de produção; o wiring TEM de levar à troca o rótulo do contexto do run (ou declarar por ADR porque a troca é control-plane puro e não pode ser pedida por um modelo) | AOS-264/AOS-265; eixo D4/AOS-156 | Responsável de Segurança | Autoridade de identidade real provisionada (D4) | ABERTO |
...
| DEF-405 | DEFERIDO | packages/cmd/aos/bootstrap.go | **Superfície de INVOCAÇÃO da promoção pelo operador.** A ratificação de promoção (AOS-206) é submissível pela rota externa `POST /promote` (`packages/cmd/aos/promotion_api.go` `handlePromote`, registada em `planos.go`, desde `caa08ea`/2026-08-11, EPIC-20 W6/AOS-275) — a caracterização anterior — de que o binário não teria endpoint HTTP nem subcomando CLI de submissão — CADUCOU, e `packages/cmd/aos/bootstrap.go:2337` já o declara («a submissao de ratificacoes DEIXOU de ser deferida — ha rota externa»). O que continua ABERTO é o gatilho de saída AOS-096 (reavaliado abaixo contra o gatilho, não contra a rota): um pipeline de promoção/canary a montante que submeta ratificações ao controller automaticamente, que `packages/cmd/aos/bootstrap.go:2340` declara continuar FORA do nó | AOS-096 | Responsável de Segurança | Existir pipeline de promoção/canary (AOS-096) que submeta ratificações ao controller | ABERTO |
...
| DEF-701 | NUNCA-EM-PRODUCAO | packages/substrate/sandbox/driver.go | **REESCRITA a 2026-09-06 por AOS-344/AOS-356(d): a redacção anterior era FALSA, e SOBRESTIMAVA o risco.** Dizia que «o driver de referência NÃO cria jail nem impõe as invariantes de isolamento». Impõe: `driver_fake.go` `Create` recusa fail-closed sem `NoHostSocket`/`NoSharedNetNS`/`NoSharedPIDNS` (`ErrHostSocketForbidden`, `ErrSharedNamespaceForbidden`) e bloqueia traversal, symlink para fora e metacaractere de shell ANTES de qualquer resolução; o próprio `driver.go` — o ficheiro ANCORADO por esta entrada — dizia o contrário do registo. É a classe que DEF-814 nomeia: engana para falso alarme, não para falso conforto. O QUE CONTINUA VERDADE, e é a razão de a entrada não fechar: o jail é modelado IN-PROCESS, em memória, e a sua fronteira é o processo do nó — não há fronteira ao nível do kernel, e um resultado fabricado por ele é selado na hash-chain WORM como se fosse um efeito real. É o único dos três drivers que falha ABERTO (`firecracker` e `gvisor` sem URL de executor devolvem `ErrDriverUnavailable`). **GATILHO DE SAÍDA JÁ OCORREU** (AOS-344): `deploy/server/model-tools/tools.json` declara um bloco `sandbox` para a tool `doc_read` e o compose de produção monta-o. A resposta foi tirar ao `fake` a porta de produção — `AOS_SANDBOX_DRIVER` sem valor deixa de o eleger e o default do compose passa a `gvisor` — pelo que a ausência de fronteira deixa de ser alcançável por omissão | AOS-064, AOS-068, AOS-103, AOS-344 | Responsável de Segurança | Composição de um driver com fronteira ao nível do kernel no nó (AOS-103): o gatilho original — catálogo de tools não-vazio — já ocorreu e foi respondido pela guarda de produção de AOS-344, não pela entrega da fronteira | MITIGADO |
| DEF-702 | NUNCA-EM-PRODUCAO | packages/substrate/sandbox/driver_fake.go | Driver fake dos testes: corre no processo do nó, não no host — e a diferença importa. **PRECISADO a 2026-09-06 por AOS-344.** `driver_fake.go` NÃO executa processos (`os/exec`, `net.`, `http.` não ocorrem no ficheiro) e o `readHost` não tem chamador a partir do jail: é um VFS em memória. O dano não é escape de sandbox — é que o resultado é FABRICADO e selado no WORM como efeito real, e o driver sucede em SILÊNCIO onde os outros dois falham fechado. Idem DEF-701 quanto ao gatilho, que JÁ OCORREU e foi respondido pela guarda de produção (`AOS_SANDBOX_DRIVER` ausente deixa de eleger o `fake`) | AOS-064, AOS-344 | Responsável de Segurança | Idem DEF-701 | MITIGADO |
...
| DEF-803 | STUB | packages/control-plane/orchestrator/orchestrator.go | Decomposição goal→DAG é um stub: o grafo tem um nó único. **REAVALIADO a 2026-08-30 por AOS-281 (DoD) e MANTIDO ABERTO.** A reavaliação mudou o que falta, não o estado: até aqui não havia sequer um ESCRITOR LEGÍTIMO do grafo — o `GraphBuilder` não detinha fencing token e, sobre um run existente, construía cego (`ErrLogAhead`). O ADR-023 dá-lhe dono (posse por lease) e via de construção re-hidratada (`orchestrator.NewGraphBuilderFromLog`), pelo que a condição NECESSÁRIA para uma decomposição multi-tarefa real passa a existir. Não é a condição SUFICIENTE: continua a faltar a decomposição em si, que é de AOS-025 e não deste ticket. **TICKETADO a 2026-09-09 como AOS-388** (EPIC-19, «Decomposer LLM de produção: goal → PlanDocument multi-nó, ponta-a-ponta no aos-orq»): é a decomposição real que faltava — o Decomposer de produção que satisfaz `planner.Decomposer` mais o wiring no `aos-orq`. O DEF-803 FECHA quando AOS-388 aterrar em código. **FECHADO-RESIDUAL a 2026-09-10:** AOS-388 aterrou — o Decomposer de produção (`decompose.LLMDecomposer`) + o wiring goal→DAG multi-nó no `aos-orq` (T1/T2-A) e o fix do ramo papéis-que-expandem (`AOS-393`) entregam a decomposição multi-tarefa real, provada por EXECUÇÃO (fixture de expansão ⇒ `spawn` + `materializado: nos=2`). O `orchestrator.Submit` (`NewMinimalGraph`, nó único) permanece como o esqueleto NÃO-PRODUTIVO de AOS-012 **sem chamador de produção** — o marcador STUB fica como CONTRASTE, não como dívida (é o que este estado exprime). O modelo LLM VIVO é ortogonal e segue em `AOS-391` (T2-B); nada disto entra no nó `cmd/aos` (ADR-018) | AOS-025, AOS-388 | Arquitecto de Plataforma | Necessidade de grafos multi-tarefa com detecção de deadlock; entregue por AOS-388 | FECHADO-RESIDUAL |
...
| DEF-806 | DIFERIDO | packages/kernel/agent-runtime/loop.go | Separação de planos por handle (dual-LLM/CaMeL, opção A do ADR-034), RE-ESCOPADA a efeitos parametrizados por dados untrusted: a AUTORIZAÇÃO já é derivada do contexto (opção C, ADR-034 — uma tool call pedida depois de conteúdo untrusted sai untrusted e o TaintGate nega-a se for privilegiada), mas o conteúdo untrusted continua a entrar INLINE no tail que o modelo lê, pelo que um efeito cujos argumentos venham desse conteúdo não tem separação | AOS-069 | Responsável de Segurança | Entrada no nó de uma tool de EFEITO parametrizada por dados untrusted (até lá `web_post` fica fora do snapshot dos planos) — ADR-034 §5.4 | ABERTO |
| DEF-807 | DIFERIDO | packages/kernel/agent-runtime/model.go | Marcador de contraste: `ToolInvocation.AuthorizationTaint` (autorização como string convencionada, preenchível pela fronteira do modelo) SAIU com o ADR-034 — o taint da autorização é cunhado pelo loop a partir do contexto e escrito no `CallContext`, e um teste estrutural impede que volte um campo de autoridade à saída do modelo. Resíduo: `CallContext.Taint` continua a ser uma string no contrato do RM, escrita por quem constrói a `Call` (no caminho das tool calls do modelo, o loop) | AOS-069 | Responsável de Segurança | Opção A do ADR-034 (autoridade cunhada a partir da `PlannerView`) ou um tipo de autorização mintado no contrato do RM | FECHADO-RESIDUAL |
...
| DEF-812 | DEFERIDO | packages/cmd/aos/posture_banner.go | **REG: catalogo, host MCP e TOFU nao sao construidos pelo no.** `bootstrap.go` compoe `emptyCatalog{}` e o `referenceRevalidator(wormForChain)` com trust store VAZIO. `registry.New` tem um unico chamador nao-teste (`promotion/pipeline.go`), que por sua vez nao tem chamador nenhum; `mcp.NewHost` e `tofu.NewMonitor` tem zero. O que CORRE e o congelamento por run (`toolset`) e a revalidacao por chamada, ligados na cadeia do Reference Monitor (`packages/integration/secured.go`) — e, desde AOS-381, tanto a revalidacao por chamada como as mudancas do trust store selam no WORM UNICO do no (durável sse `AOS_WORM_PATH` definido), nao num MemStore volatil; o ápice recusa fail-closed uma revalidacao selada noutro store. O catalogo vazio continua default-deny, pelo que nenhuma tool executa. Ao contrario do ORQ/SCH, NAO ha ADR que o declare deliberado — ver N-DEF-812 | POR ATRIBUIR | Arquitecto de Plataforma | Decisao do dono sobre a forma: compor o REG no no, ou emitir ADR que o declare fora do grafo de build no molde do ADR-023 | ABERTO |
...
| DEF-816 | DOCUMENTAL | analises/12_Auditoria_ES_SBX_Adversarial.md | **«O servidor recusa um `STREAM.CREATE` divergente» é AFIRMAÇÃO DE DOCUMENTAÇÃO NÃO MEDIDA.** O doc de `CreateStream` declara que «um stream já existente com configuração diferente é recusado pelo servidor — deliberadamente não se faz update às cegas», e essa recusa é a ÚNICA coisa que impede uma mudança de `num_replicas`/`deny_delete`/`deny_purge` por baixo de um log vivo. NENHUMA das oito adendas de `docs/reports/medicao-jetstream-arbitragem-2026-08-31.md` a mediu, e não há teste que a exercite: é uma propriedade do servidor NATS, assumida. Compõe com a assimetria que AOS-357(c) documenta — só o `Placement` é lido de volta da configuração armazenada; `NumReplicas`, `DenyDelete` e `DenyPurge` chegam em `StreamConfigLida` e não são lidos por ninguém —, pelo que se o servidor NÃO recusar, nada no nó dá por isso. Registado por AOS-357 a 2026-09-06. Risco contido hoje: o substrato replicado não é exercitado em CI (DEF-816 partilha essa condição com o inventário de `scripts/ci/dormencia.sh`) | AOS-357, AOS-358 | Arquitecto de Plataforma | Uma medição contra um cluster NATS real que confirme a recusa — ou, se ela não existir, a leitura de volta de `NumReplicas`/`DenyDelete`/`DenyPurge` no molde do que a soberania já faz | ABERTO |
...
| DEF-906 | DOCUMENTAL | analises/09_Auditoria_RT_RM_Adversarial.md | **Backstop de wall-clock para `paused` e `waiting_on_tool` — MECANISMO ENTREGUE (AOS-419), ALCANCE POR FECHAR.** MEDIDO na altura: com `humanTTL` e `wallClock` ligados e o relógio injectado avançado 87 600 h, `state.Machine.CheckDeadlines` não transitava nenhum dos dois estados — o `switch` só tinha ramo para `waiting_on_human` e `running`, e a tabela de AOS-017 nem sequer dava a esses dois estados aresta para terminal nenhum. A segunda via também não cobria: a guarda de entrada do disjuntor (`liveness.CountsAsActiveWork`, só `running`) devolve cedo, pelo que o sinal wall-clock ABSOLUTO de `tecnica/08` §6 era recolhido e nunca avaliado. FECHADO NO MECANISMO por AOS-419: as arestas `waiting_on_tool → timed_out` e `paused → timed_out` existem e `CheckDeadlines` transita-as fail-closed no MESMO tecto de `WithRunWallClock` (razão `suspension_wall_clock_exceeded`), com o nó a abrir as máquinas já com esse tecto. FICA POR LIGAR o ALCANCE: um run que se suspende e larga a posse sai do registo de em-curso do nó e o varrimento de AOS-252 deixa de lhe tocar — fechá-lo é varrer os streams SUSPENSOS, no molde do approval_sweeper. Auditoria §3.2 | AOS-419 | Arquitecto de Plataforma | Varrimento de deadlines que alcance os runs SUSPENSOS que largaram a posse (o último critério de AOS-419); fechado isso, a linha passa a FECHADO-RESIDUAL | MITIGADO |
...
| DEF-908 | DOCUMENTAL | analises/10_Auditoria_ORQ_SCH_PDP_Adversarial.md | **Despromoção automática por anomalia (AOS-090) sem chamador.** `autonomy.NewController` tinha ZERO chamadores; o nó compunha apenas o `LevelRegistry`. **COMPOSTO por AOS-090/ADR-025 (2026-09-12):** o TRIP do disjuntor multi-sinal (AOS-080) demove a CLASSE via `Controller.OnAnomaly` (`autonomiaAnomalias`), fechando os dois críticos medidos (demoção de instância elevava o efectivo / abria elevação por forja); a PROMOÇÃO liga-se pela `ReliabilitySource` de produção sobre o registo de desfecho pós-efeito (`tool.call.outcome`, novo no RM). **RESIDUAIS (declarados em ADR-025):** promoção automática só abaixo de L4; não-durável através de reinício; override-rate por proxy de escalada; drift/override-spike por ligar (fontes não compostas); agregação sobre histórico por fazer. | AOS-090 | Arquitecto de Plataforma | Metade de segurança (demoção) COMPOSTA; promoção medível abaixo de L4. Residuais em ADR-025 seguem como follow-up | FECHADO-RESIDUAL |
| DEF-909 | DOCUMENTAL | analises/10_Auditoria_ORQ_SCH_PDP_Adversarial.md | **Soberania por board inerte no caminho de EFEITO (AC#2 de AOS-094).** `pdp.WithBoardRegions` não tem chamador de produção, pelo que `applySovereignty` retorna imediato, a obrigação `region` nunca é emitida e o `enforceRegion` do Reference Monitor é inalcançável. Os outros quatro CA de AOS-094 ESTÃO cobertos por outra camada composta e assinada (a allowlist `(board, modelo, região)` do model-gateway, AOS-058) e o read-path por AOS-182/205; o que fica inerte é a cadeia PDP-emite→PEP-impõe para tool calls. Residual adjacente: `Resource.Region` é um campo de metadados declarado pelo operador, sem atestação **FECHADO-RESIDUAL por AOS-407 (2026-09-17):** a claim `board` passou a ir ASSINADA no token NHI (afirmada pelo IdP no mint; herdada pelos filhos), o hook de identidade propaga-a ao `rm.Principal` (antes apagava-a — a terceira peça, que este registo não nomeava), e o nó liga ao PDP a autoridade board→região como resolvedor VIVO (`SetBoardRegions`, ligação tardia: o PDP abre antes de a autoridade existir). A cadeia PDP-emite→PEP-impõe está provada no nó composto, com mutação nas duas metades. **Residual:** a `resource_region` continua declarada pelo operador em `AOS_MODEL_TOOLS` e não assinada — o arranque passa a RECUSAR uma região vazia ou fora do mapa de boards, mas a atestação da região pertence ao REG (DEF-812). **Verificado em produção a 2026-09-18 (v0.1.22):** NHI com `board:prod` da claim do IdP e 5 tool calls de um run real autorizadas com a obrigação `region=eu-west` e executadas. | AOS-094 | Arquitecto de Plataforma | Compor `WithBoardRegions` no nó E dar ao `Resource.Region` uma fonte de verdade — sem a segunda, a primeira compara a região autorizada do board contra um campo auto-declarado | FECHADO-RESIDUAL |
...
| packages/kernel/agent-runtime/loop.go | DIFERIDO | 1 |
| packages/kernel/agent-runtime/model.go | DIFERIDO | 1 |
...
> default-deny — o nó não executa tools, não as executa mal. E desde **AOS-381** o «que corre»
> ganhou a ressalva que faltava: a revalidação por chamada e as mudanças do trust store SELAM no
> **WORM único do nó** (durável sse `AOS_WORM_PATH` definido; o `aos audit-trail` lê as partições
> `registry.truststore`/`registry.revalidation`), não num `audit.NewMemStore()` volátil que nenhum
> leitor lia — o ápice (`integration.NewSecuredRuntime`) recusa fail-closed uma revalidação selada
> noutro store. O que não existe é o catálogo event-sourced, o host MCP e o TOFU.
>
...
recusa uma linha de registo sem âncora no código. Esta nota fica como o registo do que existiu —
o achado, o que custou, e o que o fechou.

**O achado**, de **2026-08-27**, nasceu de uma correcção que não corrigiu o que dizia corrigir.

O SLI `mediation_overhead_p95` derivava dos spans `execute_tool`. Há três produtores desse nome e
nenhum media a decisão isolada: o do worker envolve gate fenced + `ledger.Apply`, e o do Reference
Monitor **parece** ser só a mediação mas `Monitor.evaluate` chama `m.dispatch` ANTES de devolver a
decisão, pelo que o span só fecha depois de a tool correr. O filtro introduzido em AOS-085 escolheu
a janela mais estreita (a do monitor) — melhoria real de selecção de amostras — mas o número
continuava a ser um **tecto superior do custo de uma tool call mediada**.

**O que custou, medido.** Com o filtro activo, 2026-08-27: **1 amostra, `3,047 s`** contra um SLO de
`15 ms`. Antes do filtro: 2 amostras, `4,017 s`. E em produção a **2026-09-15/16**, o run
`run-delegado-1789519407` — dois turnos, UMA tool call `doc_read` em gVisor — acendeu
`mediation_overhead_high` e `mediation_overhead_p95_high`, ambos `critical` e ambos com rota RB-04
(«Falha de PDP»), a `1,21099128e+09` ns contra `1,5e+07` ns, streak até 4. Durante todo esse período,
o disparo do RB-04 **não** significava mediação degradada: significava que tinha havido uma tool call.

**O que o fechou — AOS-398, ratificado em ADR-026.** O Reference Monitor passou a medir a janela da
decisão (política, obrigações e selo pré-efeito, EXCLUINDO o despacho) e a publicá-la no span em
`aos.mediation.decision_latency_ns`; o SLI deriva desse atributo e **não** cai para a latência do
span quando ele falta. O `tecnica/19` §7 (S-02f) listava `EXEC` dentro do orçamento dos 15 ms, em
